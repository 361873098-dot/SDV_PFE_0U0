...
#define CRYPTO_SW_MAJOR_VERSION_HSE_C             4
#define CRYPTO_SW_MINOR_VERSION_HSE_C             0
#define CRYPTO_SW_PATCH_VERSION_HSE_C             2
...
#define CRYPTO_OPERATIONMODE_FINISH_MASK_U8             ((uint8)0x04U)
    ...
    #define CRYPTO_SHE_MASTER_ECU_KEY_HANDLE_U32        ((uint32)0x00010000U)
        ...
        #define CRYPTO_ENDIANESS_CONVERSION_LENGTH  (200U)
    #endif /* (STD_ON == CRYPTO_SPT_ECC) */
#endif /* (STD_ON == CRYPTO_KEYS_EXIST) */
...
typedef struct
{
    ...
    uint32              au32OutLength[HSE_NUM_OF_CHANNELS_PER_MU];                  /* Array that will be filled with the length information from user output */
        ...
        (STD_ON == CRYPTO_SPT_XCBCMAC) || \
        (STD_ON == CRYPTO_SPT_GMAC)    || \
        (STD_ON == CRYPTO_SPT_HMAC)    || \
        (STD_ON == CRYPTO_SPT_SIPHASH)
    ...
    #endif /* (STD_ON == CRYPTO_SPT_CMAC) || (STD_ON == CRYPTO_SPT_XCBCMAC) || (STD_ON == CRYPTO_SPT_GMAC) || (STD_ON == CRYPTO_SPT_HMAC) || (STD_ON == CRYPTO_SPT_SIPHASH */
    #if (STD_ON == CRYPTO_SPT_ECC)
    uint8               au8KeyBigEndForm[CRYPTO_ENDIANESS_CONVERSION_LENGTH];       /* Temporary buffer used for endianness conversion */
    #endif /* (STD_ON == CRYPTO_SPT_ECC) */
...
static inline Std_ReturnType Crypto_Hse_LoadCustomKey
(
    uint8               u8MuInstance,
    uint8               u8MuChannel,
    hseSrvDescriptor_t* pHseSrvDescriptor,
...
)
{
    ...
    const uint32 CoreId = OsIf_GetCoreID();

    if(Address > 0U)
    {
        /* If CRYPTO_ITCM_ADDR_END_U32 == 0U, the device only has general TCM memory, treated as DTCM */
#if ((CRYPTO_ITCM_ADDR_END_U32 != CRYPTO_ITCM_ADDR_START_U32) && (CRYPTO_ITCM_ADDR_END_U32 != 0x00000000U))
        /* Check if the address is in ITCM range */
        if((0U == (((HOST_ADDR)(CRYPTO_ITCM_ADDR_END_U32)) / Address)) && (0U < (((HOST_ADDR)(CRYPTO_ITCM_ADDR_END_U32)) % Address)))
        {
            RetVal = (HOST_ADDR)(Address + Crypto_aItcmAddrOffset[CoreId]);
        }
        /* Check if the address is in DTCM range */
        else 
#endif /* (CRYPTO_ITCM_ADDR_END_U32 > 0) */
		if((Address >= ((HOST_ADDR)(CRYPTO_DTCM_ADDR_START_U32))) && (Address < ((HOST_ADDR)(CRYPTO_DTCM_ADDR_END_U32))))
        {
...
)
{
    ...
    static const uint8 Crypto_au8EccNistSecp521Oid[CRYPTO_NIST_SECP_384_521_SIZE_U8]   = {0x2B, 0x81, 0x04, 0x00,0x23};
...
{
    uint8   u8RetEccCurveId = HSE_EC_CURVE_NONE;
#if ((STD_ON == CRYPTO_SPT_EC25519_ED25519) || (STD_ON == CRYPTO_SPT_EC25519_CURVE25519) || (STD_ON == CRYPTO_SPT_EC25519_ED25519PH))
    boolean bCompareResult;
#if ((STD_ON == CRYPTO_SPT_EC25519_ED25519) || (STD_ON == CRYPTO_SPT_EC25519_ED25519PH))
...
#endif /* ((STD_ON == CRYPTO_SPT_EC25519_ED25519) || (STD_ON == CRYPTO_SPT_EC25519_ED25519PH)) */
    ...
    (void)pCurveOid;
    (void)u32CurveOidLength;
#endif /* ((STD_ON == CRYPTO_SPT_EC25519_ED25519) || (STD_ON == CRYPTO_SPT_EC25519_CURVE25519) || (STD_ON == CRYPTO_SPT_EC25519_ED25519PH)) */
...
)
{
    ...
    /* Key bit length is pre-defined for ECC curves */
    switch (u16CurveId)
    {
#if ((STD_ON == CRYPTO_SPT_EC25519_ED25519) || (STD_ON == CRYPTO_SPT_EC25519_ED25519PH))
...
)
{
    ...
    if (CRYPTO_INVALID_KEY_IDX_U32 != u32CryptoKeyIdx)
    {
        ...
        if (CRYPTO_INVALID_KEY_ELEMENT_IDX_U32 != u32KeyMaterialKeyElemIdx)
        {
            ...
            if ((NULL_PTR != pJob->jobRedirectionInfoRef) && Crypto_Util_IsRedirectionSet(pJob->jobRedirectionInfoRef->redirectionConfig, CRYPTO_REDIRECT_CONFIG_SECONDARY_INPUT))
            {
                ...
                Crypto_Hse_apMuState[u8MuInstance]->au32MacVerifyInputLenInBytes[u8MuChannel] = ((uint32)(*jobPrimitiveIOUpdate.pSecondaryInputLength));
            }
            else
        ...
        if (CRYPTO_INVALID_KEY_ELEMENT_IDX_U32 != u32KeyMaterialKeyElemIdx)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        /* In case of redirection of secondary input, the Crypto driver will take the secondaryInputLength parameter from the redirected key element, where it is measured in bytes
                           Update the length and store its value in bits. */
                        pFastCMACSrv->tagBitLength = (uint8)(*jobPrimitiveIOUpdate.pSecondaryInputLength << 3UL) ;
                    }
                    else
#endif /* (STD_ON == CRYPTO_ENABLE_REDIRECTION_SUPPORT) */
                    {
...
)
{
    hseImportKeySrv_t*                         pImportKeyReq;
    Std_ReturnType                             RetVal;
    const Crypto_KeyImportType*  pImportKeyCustomStruct = (const Crypto_KeyImportType*)pKeyMaterial;
    ...
    /* Authentication */
    if ((CRYPTO_AUTH_SCHEME_MAC  == pImportKeyCustomStruct->Authentication.SelAuthenticationScheme) ||
        (CRYPTO_AUTH_SCHEME_SIGN == pImportKeyCustomStruct->Authentication.SelAuthenticationScheme)
    )
    {
        ...
        pImportKeyReq->keyContainer.pAuth[0]   = Crypto_Hse_AddrHandler(CRYPTO_HSE_PTR_TO_HOST_ADDR(pImportKeyCustomStruct->Authentication.pAuthentication[0]));
        pImportKeyReq->keyContainer.pAuth[1]   = Crypto_Hse_AddrHandler(CRYPTO_HSE_PTR_TO_HOST_ADDR(pImportKeyCustomStruct->Authentication.pAuthentication[1]));
        ...
        pImportKeyReq->keyContainer.pKeyContainer   = Crypto_Hse_AddrHandler(CRYPTO_HSE_PTR_TO_HOST_ADDR(pImportKeyCustomStruct->Authentication.pKeyContainer));
        pImportKeyReq->keyContainer.keyContainerLen = pImportKeyCustomStruct->Authentication.u16KeyContainerLength;
    }

    /* KeyData*/
    pImportKeyReq->pKeyInfo                = Crypto_Hse_AddrHandler(CRYPTO_HSE_PTR_TO_HOST_ADDR(pImportKeyCustomStruct->KeyData.pKeyInfo));
    pImportKeyReq->pKey[0]                 = Crypto_Hse_AddrHandler(CRYPTO_HSE_PTR_TO_HOST_ADDR(pImportKeyCustomStruct->KeyData.pKey[0]));
    pImportKeyReq->pKey[1]                 = Crypto_Hse_AddrHandler(CRYPTO_HSE_PTR_TO_HOST_ADDR(pImportKeyCustomStruct->KeyData.pKey[1]));
    pImportKeyReq->pKey[2]                 = Crypto_Hse_AddrHandler(CRYPTO_HSE_PTR_TO_HOST_ADDR(pImportKeyCustomStruct->KeyData.pKey[2]));
...
)
{
    ...
    if (CRYPTO_KE_INVALID_CURVE_ID_U8 != u8CurveId)
    {
        ...
        if ((u32pubKeyByteLen == u32KeyLength) && 
            (CRYPTO_ENDIANESS_CONVERSION_LENGTH >= (u32pubKeyByteLen + u8Index)))
        {
...
)
{
    ...
    if (CRYPTO_KE_INVALID_CURVE_ID_U8 != u8CurveId)
    {
           ...
           )
        {
            ...
            /* Check for big endianness conversion buffer offsets to avoid overflow */
            if((CRYPTO_ENDIANESS_CONVERSION_LENGTH >= (u32pubKeyByteLen + u8Index)) &&
               (CRYPTO_ENDIANESS_CONVERSION_LENGTH >= (u32pubKeyByteLen + u32PrivKeyByteLen + u8Index)))
            {
                /* Copy the key in a buffer used for big endian conversion */
                Crypto_Util_MemCpy8(&Crypto_Hse_apMuState[u8MuInstance]->au8KeyBigEndForm[u8Index], pKey, u32KeyLength);
        ...
        {
            /* Check for big endianness conversion buffer offsets to avoid overflow */
            if((CRYPTO_ENDIANESS_CONVERSION_LENGTH >= (u32PrivKeyByteLen + u8Index)) &&
                (CRYPTO_ENDIANESS_CONVERSION_LENGTH >= (u32PrivKeyByteLen + u32pubKeyByteLen + u8Index)))
            {
                /* Copy the key in a buffer used for big endian conversion */
                Crypto_Util_MemCpy8(&Crypto_Hse_apMuState[u8MuInstance]->au8KeyBigEndForm[u8Index], pKey, u32KeyLength);
...
)
{
    ...
    switch (u8AlgoFamily)
    {
...
#if (STD_ON == CRYPTO_SPT_ECC)
        case (uint8)CRYPTO_ALGOFAM_ECCNIST:
			/* The singnature (r,s) length for ECDSA is double the length of the private key used to generate the signature. 
			   Use output buffer length comparision with 2*UsedKeyLen where UsedKeyLen=u32JobKeyLength */
            if ((2UL*u32JobKeyLength) <=  Crypto_Hse_apMuState[u8MuInstance]->au32OutLength[u8MuChannel])
            {
				...
				/* The singnature (r,s) length for ECDSA is double the length of the private key used to generate the signature. 
			       Assign u32JobKeyLength to au32OutLength as it contains the key length. */
                Crypto_Hse_apMuState[u8MuInstance]->au32OutLength[u8MuChannel] = u32JobKeyLength;
        ...
        case (uint8)CRYPTO_ALGOFAM_ED25519:
        /* Walk through */
        case (uint8)CRYPTO_ALGOFAM_CUSTOM_ED448:
        /*Intentional Fall-Through*/
        case CRYPTO_ALGOFAM_CUSTOM_ED25519PH:
            if ((2UL*u32JobKeyLength) <= Crypto_Hse_apMuState[u8MuInstance]->au32OutLength[u8MuChannel])
            {
                ...
                /*Check which one of the 3 cases is active, if it's CRYPTO_ALGOFAM_CUSTOM_ED25519PH, then the input needs pre-hashing*/
                if(CRYPTO_ALGOFAM_CUSTOM_ED25519PH == u8AlgoFamily)
                {
...
)
{
    ...
    if(CRYPTO_INVALID_KEY_IDX_U32 != u32CryptoKeyIdx)
    {
        u32SipHashKeyElemIdx  = Crypto_Util_GetKeyElementIdx (u32CryptoKeyIdx, CRYPTO_KEY_MATERIAL_U32);
...
)
{
    ...
    if(CRYPTO_INVALID_KEY_ELEMENT_IDX_U32 != u32KeyMaterialKeyElemIdx)
    {
        ...
        switch (u8AlgoFam)
        {
            ...
            case (uint8)CRYPTO_ALGOFAM_ED25519:
            /* fall-through */
            case (uint8)CRYPTO_ALGOFAM_CUSTOM_ED25519PH:
            /* fall-through */
            case (uint8)CRYPTO_ALGOFAM_CUSTOM_ED448:
...
)
{
...
#else
	const hseKeyDeriveSrv_t*        pDeriveKeySrv;
#endif /* (STD_ON == CRYPTO_SPT_KDF_ANS_X963) || (STD_ON == CRYPTO_SPT_PBKDF2) || (STD_ON == CRYPTO_ENABLE_TLS12_DERIVE_SUPPORT) */
    ...
    const uint32                    u32TargetOutputDataKeyElemIdx = Crypto_Util_GetKeyElementIdx(u32TargetKeyIdx, CRYPTO_KE_KEYDERIVATION_OUTPUT_DATA);
          uint32                    u32TargetMasterKeyHandle      = CRYPTO_INVALID_KEY_ELEMENT_IDX_U32;
#endif /* (STD_ON == CRYPTO_ENABLE_TLS12_DERIVE_SUPPORT) */
          uint32                    u32KeyHandle;
	...
	/* Assigning FALSE to pbReqSentOverChannel to cover the cases invalid element index and non-supported key drive algorithms */
    *pbReqSentOverChannel = FALSE;
	...
	if( CRYPTO_INVALID_KEY_ELEMENT_IDX_U32 != (u32PasswordKeyElemIdx | u32DerivAlgoKeyElemIdx | u32TargetKeyMaterialElemIdx | u32DerivSecondAlgoKeyElemIdx | u32DerivSaltKeyElemIdx))
    {
        ...
        switch (u8KeyDeriveAlgo)
        {
            ...
            case CRYPTO_ALGOFAM_CUSTOM_TLS12_GENERATE_VERIFY_SERVER:
				...
				{
				    u32TargetMasterKeyHandle  = Crypto_aKeyElementList[u32TargetMasterKeyElemIdx].u32HseKeyHandle;
				}
    ...
    for (u8Index = 0U; u8Index < HSE_NUM_OF_CHANNELS_PER_MU; u8Index++)
    {
           ...
           )
        {
            ...
            if ((CRYPTO_RET_OPERATION_TIMEOUT != RetVal) &&
                (CRYPTO_RET_INVALID_PARAM     != RetVal) &&
                ((Std_ReturnType)E_OK != RetVal)
               )
            {
    ...
    if (HSE_IP_INVALID_MU_CHANNEL_U8 != u8MuChannel)
    {
        ...
        switch (u8KeyType)
        {
            ...
            case HSE_KEY_TYPE_ECC_PAIR:
                ...
                if ((Crypto_KeyFormatType)CRYPTO_KE_FORMAT_BIN_CUSTOM_STRUCT == eKeyElementFormat)
                {
                    RetVal = Crypto_Hse_LoadCustomKey(u8MuInstance,
                                                      u8MuChannel,
                                                      pHseSrvDescriptor,
                                                      pKey,
                                                      u32CryptoKeyElementIdx);
...
(
    ...
    const uint8*  pResult,
#endif
#if ((STD_ON == CRYPTO_ENABLE_ENC_AUTH_KEY_EXPORT) || (STD_ON == CRYPTO_SPT_SHE_KEY_EXPORT))
    uint32*       pResultLength
#else
    const uint32* pResultLength
#endif
...
*                  CRYPTO_E_BUSY: No MU channels available to process the requested job operation.
*/
Std_ReturnType Crypto_Hse_CheckKey
(
    const uint8  u8MuInstance,
    const uint32 u32CryptoKeyIdx
)
{
    Std_ReturnType      RetVal = (Std_ReturnType)E_OK;
#if (STD_ON == CRYPTO_SPT_KEY_VERIFY)
    uint32              u32HseKeyHandle;
    const uint8         u8InputBuffer[64] = { 0U };
    uint8               u8MuChannel;
    uint8               u8KeyType;
    hseKeyInfo_t        HseKeyInfoDummy;
    hseSrvDescriptor_t* pHseSrvDescriptor;
    Crypto_KeyFormatType eKeyFormat;
    ...
    {   
        RetVal = (Std_ReturnType)E_NOT_OK;
        u32HseKeyHandle = Crypto_aKeyElementList[u32KeyMaterialKeyElemIdx].u32HseKeyHandle;
        ...
        if ((uint32)HSE_INVALID_KEY_HANDLE != u32HseKeyHandle)
        {
            ...
            if (HSE_IP_INVALID_MU_CHANNEL_U8 != u8MuChannel)
            {
                u8KeyType  = Crypto_aKeyElementList[u32KeyMaterialKeyElemIdx].u8HseKeyType;
                eKeyFormat = Crypto_aKeyElementList[u32KeyMaterialKeyElemIdx].eCryptoKeyFormat;
                ...
                if ((uint8)HSE_KEY_TYPE_SHE == u8KeyType)
                {
                    if ((Crypto_KeyFormatType)CRYPTO_KE_FORMAT_BIN_SHEKEYS == eKeyFormat)
                    {
                        ...
                        pHseSrvDescriptor->hseSrv.verifyKeyReq.keyVerAlgo   = HSE_KEY_VER_SHA512;
                        pHseSrvDescriptor->hseSrv.verifyKeyReq.tagLen       = 64U;
                        pHseSrvDescriptor->hseSrv.verifyKeyReq.pTag         = Crypto_Hse_AddrHandler(CRYPTO_HSE_PTR_TO_HOST_ADDR(u8InputBuffer));  
                        if ((Std_ReturnType)CRYPTO_E_KEY_EMPTY == Crypto_Hse_SendMsg(u8MuInstance, u8MuChannel, pHseSrvDescriptor, NULL_PTR))
                        {
                            /* The HSE slot information is not ok. Mark the key as invalid. */
                            Crypto_ClearKeyValidFlag(u32CryptoKeyIdx);
                        }
                        else
                        {   
                            /* Return E_OK for further operations if the key is valid*/
                            RetVal = (Std_ReturnType)E_OK;
                        }
                    }
                }
                else
                {
                    ...
                    pHseSrvDescriptor->srvId                          = HSE_SRV_ID_GET_KEY_INFO;
                    pHseSrvDescriptor->hseSrv.getKeyInfoReq.keyHandle = u32HseKeyHandle;
                    pHseSrvDescriptor->hseSrv.getKeyInfoReq.pKeyInfo = Crypto_Hse_AddrHandler(CRYPTO_HSE_PTR_TO_HOST_ADDR(&HseKeyInfoDummy));
                    /* Send the request to HSE, synchronously */
                    if ((Std_ReturnType)E_OK != Crypto_Hse_SendMsg(u8MuInstance, u8MuChannel, pHseSrvDescriptor, NULL_PTR))
                    {
                         /* The HSE slot information is not ok. Mark the key as invalid. */
    ...
    (void)u8MuInstance;
    (void)u32CryptoKeyIdx;
#endif /* (STD_ON == CRYPTO_SPT_KEY_VERIFY) */
