...
#define PFE_GET_IFC_MASK(id) ((uint64_t)1ULL << (uint8_t)id)
...
#define SUM_WRAP_U32(A, B) ((uint32_t)(((uint64_t)(A) + (B)) & UINT32_MAX))
...
struct pfe_l2br_domain_tag
{
    ...
    uint8_t cur_item;                          /*!< The current interface list item */
    union /* Placeholder union for future use */
    {
        pfe_ct_phy_if_id_t id;
        pfe_phy_if_t *phy_if;
    } cur_crit_arg;                             /*!< Current criterion argument */
    uint64_t ifaces;                            /*!< List of associated interfaces as masks */
};
...
struct pfe_l2br_tag
{
    ...
    uint32_t dmem_vlan_hash_base;               /*!< Address within classifier memory where the vlan hash table is located */
    ...
    uint32_t curr_domain;                       /*!< The current domain list item */
    #ifdef PFE_CFG_L2BR_STATICS_ENABLE
    uint32_t curr_static_ent;                   /*!< The current static entry list item */
    #endif /* PFE_CFG_L2BR_STATICS_ENABLE */
    union
    {
        uint16_t vlan;
...
static void pfe_l2br_create_mandatory_domains(const pfe_class_t *class, pfe_l2br_t **bridge, uint16_t def_vlan, uint16_t def_aging_time);
...
static errno_t pfe_l2br_set_static_entry(pfe_l2br_t *bridge, uint16_t vlan, const pfe_mac_addr_t mac, uint32_t new_fw_list, pfe_l2br_static_entry_t **static_entry);
static errno_t pfe_l2br_domain_flush_by_if_static(const pfe_l2br_domain_t *domain, pfe_l2br_t *bridge, uint32_t iface_bitflag);
#endif /* PFE_CFG_L2BR_STATICS_ENABLE */
...
static errno_t pfe_l2br_domain_flush_by_if_dynamic_static(const pfe_l2br_domain_t *domain, pfe_l2br_t *bridge, pfe_l2br_table_iterator_t *l2t_iter, pfe_l2br_table_entry_t *entry, uint32_t iface_bitflag);
static errno_t pfe_l2br_add_vlan_to_collision_space(pfe_l2br_domain_t *domain, l2br_vlan_hash_entry_t *vlan_new_entry, uint8_t prev_entry_pos);
...
static void pfe_vlan_write_to_class(const pfe_l2br_t *bridge, uint32_t pos, const l2br_vlan_hash_entry_t *class_entry)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        if (EOK != pfe_class_write_dmem(bridge->class,
                                        -1,
                                        ADDR_BASE_OFFSET(bridge->dmem_vlan_hash_base, (uint64_t)pos * sizeof(l2br_vlan_hash_entry_t)),
...
static void pfe_l2br_vlan_action_to_entry(const pfe_l2br_domain_t *domain, l2br_vlan_hash_entry_t *entry)
{
    ...
    (void)autolibc_memcpy(&entry->entry.val, &tmp64, sizeof(uint64_t));
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        else
        {
            ...
            if (0U != (vlan_current_entry.flags & MAC_VALID_FLAG))
            {
                ...
                {
                    pos = (uint8_t) (oal_ntohs(vlan_current_entry.col_ptr) & UINT8_MAX);
                    pfe_vlan_read_from_class(domain->bridge, pos, &vlan_current_entry);
...
static errno_t pfe_l2br_add_vlan_to_collision_space(pfe_l2br_domain_t *domain,
                                                    ...
                                                    uint8_t prev_entry_pos)
{
    l2br_vlan_hash_entry_t vlan_current_entry, vlan_tmp_entry;  
    errno_t ret = EOK;
    uint8_t hash_size = 0U;
    uint8_t coll_space = 0U;
    uint8_t pos = prev_entry_pos;
    ...
    if (ret == EOK)
    {
        ...
        if (pos < hash_size)
        {
            ...
            vlan_current_entry.col_ptr = (uint16_t)pos << 8;  /* swap endianess (U16) */;
...
errno_t pfe_l2br_delete_vlan_hash_entry(pfe_l2br_domain_t *domain)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        if (domain->vlan == oal_ntohs(vlan_current_entry.vlan))
        {
            ...
            {
                next_entry_pos = (uint8_t ) (oal_ntohs(vlan_current_entry.col_ptr) & UINT8_MAX);
...
errno_t pfe_l2br_domain_destroy(pfe_l2br_domain_t *domain)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        if (EOK == ret)
        {
            ...
            const int64_t domain_subscript = isa_release(&domain->bridge->domains, domain);
            if ((domain_subscript >= 0) && (domain_subscript < (int64_t)domain->bridge->curr_domain))
            {
...
static errno_t pfe_l2br_domain_flush_by_if_dynamic_static(const pfe_l2br_domain_t *domain, pfe_l2br_t *bridge, pfe_l2br_table_iterator_t *l2t_iter, pfe_l2br_table_entry_t *entry, uint32_t iface_bitflag)
{
...
#endif /* PFE_CFG_L2BR_STATICS_ENABLE */
    {
        ...
        while (EOK == ret_query)
        {
            entry_vlan = (uint16_t)(pfe_l2br_table_entry_get_vlan(entry) & UINT16_MAX);
            entry_action_data.val = (uint32_t)(pfe_l2br_table_entry_get_action_data(entry) & UINT32_MAX);
...
errno_t pfe_l2br_domain_flush_by_if(const pfe_l2br_domain_t *domain, const pfe_phy_if_t *iface)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        iface_bitflag = (uint32_t)1UL << (uint32_t)pfe_phy_if_get_id(iface);
...
pfe_phy_if_t *pfe_l2br_domain_get_first_if(pfe_l2br_domain_t *domain, pfe_l2br_domain_if_get_crit_t crit, void *arg)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        switch (domain->cur_crit)
        {
            ...
            case L2BD_IF_BY_PHY_IF_ID:
                phy_idx = (uint32_t) arg;
                PfeDevAssert(phy_idx <= (uint32_t)PFE_PHY_IF_ID_MAX);
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        switch (bridge->cur_crit_ent)
        {
            ...
            case L2SENT_CRIT_BY_VLAN:
                bridge->cur_static_ent_crit_arg.vlan = (uint16_t)(((addr_t)arg1) & UINT16_MAX);
                break;
 ...
 * @return      EOK if possible to get index-based statistic, otherwise E_NOT_OK
 */
        ...
        {   
            ...
            if(index_entry == count)
            {
                ...
                if (PFE_L2BR_TABLE_VLAN == entry.type)
                {
                    ...
                    stat->action_data = (uint32_t)(entry.u.vlan_entry.action_data & UINT32_MAX);
...
errno_t pfe_l2br_get_domain_stats(const pfe_l2br_t *bridge, pfe_ct_vlan_stats_t *stat, uint8_t vlan_index)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        const uint16_t offset = (uint16_t)sizeof(pfe_ct_vlan_stats_t) * (uint16_t)vlan_index;
        ...
        for(i = 0; i < num_of_pes; i++)
        {
            ...
            const uint32_t ingress_add = oal_ntohl(stats.ingress);
            const uint32_t egress_add = oal_ntohl(stats.egress);
            const uint32_t ingress_bytes_add = oal_ntohl(stats.ingress_bytes);
            const uint32_t egress_bytes_add = oal_ntohl(stats.egress_bytes);
