...
[!IF "node:exists(CryptoKeyElements)"!][!//
    [!LOOP "node:CryptoKeyElements/CryptoKeyElement/*"!][!//
        ...
        [!ENDIF!][!//
        [!IF "node:value(./UseHseKey) = 'true'  and node:exists(./EncryptedKeyExport) and node:value(./EncryptedKeyExport) = 'true' and (node:ref(./HseKeyCatalogGroupRef)/KeyType = 'AES' or node:ref(./HseKeyCatalogGroupRef)/KeyType = 'HMAC')"!][!//
            [!INDENT "0"!]
    ...
    [!LOOP "node:CryptoKeyElements/CryptoKeyElement/*"!][!//
        ...
        [!INDENT "8"!]
            ...
            [!IF "ecu:get(concat('Crypto.HseSptEncAuthKeyExport.', string(node:fallback(./../../../CryptoGeneral/HseFwType,'PREMIUM')))) = 'STD_ON'"!][!//
                ...
                [!IF "node:value(./UseHseKey) = 'true'  and node:value(./EncryptedKeyExport) = 'true' and (node:ref(./HseKeyCatalogGroupRef)/KeyType = 'AES' or node:ref(./HseKeyCatalogGroupRef)/KeyType = 'HMAC')"!][!//
