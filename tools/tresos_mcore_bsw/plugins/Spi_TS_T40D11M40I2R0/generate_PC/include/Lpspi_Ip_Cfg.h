...
[!ENDIF!]
[!IF "(node:exists(SpiAutosarExt/SpiEnableLlceLpspiSupport)) and (contains(ecu:get('Spi.SpiGeneral.SpiPhyUnit.SpiPhyUnitMapping'), 'LPSPI'))"!][!//
...
#define LPSPI_IP_SW_PATCH_VERSION_CFG                2
...
[!IF "starts-with(node:value(SpiPhyUnitMapping),'LPSPI')"!]
