...
[!CODE!][!WS "0"!]#include "S32G274A_FLEXRAY.h"[!CR!][!ENDCODE!][!//
    [!ELSEIF "contains(as:modconf("Resource")[1]/ResourceGeneral/ResourceSubderivative, 's32g3')"!][!//
[!CODE!][!WS "0"!]#include "S32G399A_FLEXRAY.h"[!CR!][!ENDCODE!][!//
    [!ELSE!][!//
[!CODE!][!WS "0"!]#error "The driver supports for S32G platform only. Please configuring with S32G2 or S32G3 derivative"[!CR!][!ENDCODE!][!//
    [!ENDIF!][!//
[!ENDIF!][!//
[!ENDNOCODE!][!//
[!IF "var:defined('postBuildVariant')"!]
    [!LOOP "variant:all()"!]
        [!WS "0"!]#include "Flexray_43_LLCE_Ip_[!"."!]_PBcfg.h"
    [!ENDLOOP!]
[!ELSE!]
    [!WS "0"!]#include "Flexray_43_LLCE_Ip_PBcfg.h"
[!ENDIF!]
...
#define FLEXRAY_IP_CFG_SW_MAJOR_VERSION                 1
#define FLEXRAY_IP_CFG_SW_MINOR_VERSION                 0
#define FLEXRAY_IP_CFG_SW_PATCH_VERSION                 7
...
[!IF "var:defined('postBuildVariant')"!]
    [!LOOP "variant:all()"!]
        ...
        [!WS "0"!]    #error "Flexray_43_LLCE_Ip_Cfg.h and Flexray_Ip_[!"."!]_PBcfg.h have different Vendor IDs"
        [!WS "0"!]#endif
        ...
        [!WS "0"!]    #error "Software Version Numbers of Flexray_43_LLCE_Ip_Cfg.h and Flexray_Ip_[!"."!]_PBcfg.h are different"
        [!WS "0"!]#endif
        ...
        [!WS "0"!]    #error "Autosar Version Numbers of Flexray_43_LLCE_Ip_Cfg.h and Flexray_Ip_[!"."!]_PBcfg.h are different"
        [!WS "0"!]#endif
        [!WS "0"!]
    [!ENDLOOP!]
[!ELSE!]
[!WS "0"!]/* Check if current file and Flexray_43_LLCE_Ip_PBcfg.h header file are of the same vendor */
    [!WS "0"!]#if(FLEXRAY_IP_CFG_VENDOR_ID != FLEXRAY_IP_PBCFG_VENDOR_ID)
    [!WS "0"!]    #error "Flexray_43_LLCE_Ip_Cfg.h and Flexray_43_LLCE_Ip_PBcfg.h have different Vendor IDs"
    [!WS "0"!]#endif
    [!WS "0"!]/* Check if current file and Flexray_43_LLCE_Ip_PBcfg.h header file are of the same Autosar version */
    ...
    [!WS "0"!]    #error "Software Version Numbers of Flexray_43_LLCE_Ip_Cfg.h and Flexray_43_LLCE_Ip_PBcfg.h are different"
    [!WS "0"!]#endif
    [!WS "0"!]/* Check if current file and Flexray_43_LLCE_Ip_PBcfg.h header file are of the same software version */
    ...
    [!WS "0"!]    #error "Autosar Version Numbers of Flexray_43_LLCE_Ip_Cfg.h and Flexray_43_LLCE_Ip_PBcfg.h are different"
    [!WS "0"!]#endif
    [!WS "0"!]
[!ENDIF!]
[!WS "0"!][!CR!]
    ...
    /* Check if current file and Mcal.h header file are of the same Autosar version */
        ...
        #error "AutoSar Version Numbers of Flexray_43_LLCE_Ip_Cfg.h and Mcal.h are different"
    #endif
#endif
...
*                                       DEFINES AND MACROS
==================================================================================================*/
#define FLEXRAY_HW_VERSION    (0xA568U)
...
#define FLEXRAY_43_LLCE_SET_USER_ACCESS_ALLOWED_AVAILABLE      (STD_OFF)
    ...
    [!LOOP "node:order(FrMultipleConfiguration,'node:name(.)')"!]
        ...
        [!LOOP "node:order(FrController/*,'FrCtrlIdx','node:name(.)')"!]
            ...
            [!IF "var:defined('postBuildVariant')"!]
                ...
                [!ENDLOOP!]
            [!ELSE!]
                [!WS "4"!]FLEXRAY_IP_STATEALLOC_FRMC[!"$FrConfigIdx"!]_CTRL[!"$FrCtrlIdx"!]_[!"string($CfgVariant)"!][!IF "($FrCtrlIdx < ($FrNumCtrlSupported -1))"!] \[!ENDIF!]
            [!ENDIF!]
        [!ENDLOOP!]
        [!WS "0"!][!CR!]
        ...
        [!LOOP "node:order(FrController/*,'FrCtrlIdx','node:name(.)')"!]
            ...
            [!IF "var:defined('postBuildVariant')"!]
                ...
                [!ENDLOOP!]
            [!ELSE!]
                [!WS "4"!]FLEXRAY_IP_CFG_FRMC[!"$FrConfigIdx"!]_CTRL[!"$FrCtrlIdx"!]_[!"string($CfgVariant)"!][!IF "($FrCtrlIdx < ($FrNumCtrlSupported -1))"!] \[!ENDIF!]
            [!ENDIF!]
        [!ENDLOOP!]
    [!ENDLOOP!]

#define FLEXRAY_NUMBER_MB           (256U)
    ...
    [!SELECT "FrGeneral"!]
        ...
        [!WS "0"!]#define FLEXRAY_43_LLCE_PREPARE_LPDU_SUPPORT        ([!IF "'true' = ./FrPrepareLPduSupport"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])
...
#endif /* FLEXRAY_43_LLCE_IP_CFG_H */
