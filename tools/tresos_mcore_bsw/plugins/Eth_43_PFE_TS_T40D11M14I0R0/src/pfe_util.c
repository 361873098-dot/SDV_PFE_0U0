...
static bool_t pfe_util_check_new_fw_features(pfe_util_t *util, errno_t *ret, uint32_t features_idx)
{
    ...
    *ret = pfe_pe_get_fw_feature_entry(&util->pe[0U], features_idx, &entry);
    ...
    else
    {
        ...
        (void)pfe_fw_feature_set_ll_data(&util->fw_features[features_idx], entry, (uint8_t)util->pe_num);
...
static errno_t pfe_util_create_pe(uint32_t pe_num, addr_t cbus_base_va, pfe_util_t *util)
{
    ...
    {
        pe = pfe_pe_create(cbus_base_va, PE_TYPE_UTIL, (uint8_t)count, &util->pe[count], &util->miflock_pe);
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        if ((int64_t)pe_idx >= (int64_t)(util->pe_num))
        {
            ret = EINVAL;
        }
        else
...
errno_t pfe_util_isr(pfe_util_t *util)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        oal_mutex_lock(PFE_UTIL_PE_MUTEX_05);
        pfe_pe_lock_family(util->pe);
        ...
        pfe_pe_unlock_family(util->pe);
        oal_mutex_unlock(PFE_UTIL_PE_MUTEX_05);
