...
static __attribute__((cold)) errno_t pfe_hif_chnl_create_mcal_aux(pfe_hif_chnl_t *chnl, addr_t cbus_base_va, uint32_t id, const pfe_bmu_t *bmu);
...
static __attribute__((hot)) errno_t pfe_hif_chnl_add_vlan_tag(pfe_hif_chnl_t *chnl, void **buf_va, pfe_ct_hif_rx_hdr_t **hif_hdr_ptr, uint32_t *vlan_header);
#if defined(PFE_CFG_NULL_ARG_CHECK)
static __attribute__((hot)) errno_t pfe_hif_chnl_rx_va_null_arg_check(pfe_hif_chnl_t *chnl, void **buf_va, uint32_t *len, bool_t *lifm, void **meta);
#endif
#endif /* PFE_HIF_CHNL_CFG_RX_BUFFERS_ENABLED */
...
static inline void *rx_pool_get(rx_pool_t *pool)
{
    ...
    {
        const uint32_t buf_offset = (uint32_t)(pool->get_idx) * pool->buf_size;
...
static inline void *rx_pool_get_meta_buf(const rx_pool_t *pool, const void *buf_va)
{
    ...
    const uint16_t index = (uint16_t)((OFFSET_ADDR_BASE((addr_t)buf_va, pool->base_addr) / pool->buf_size) & UINT16_MAX);
    const uint32_t buffer_offset = (uint32_t)index * ETH_43_PFE_META_BUF_SIZE;
 ...
 * @return          EOK if success, error code otherwise
 */
__attribute__((cold)) errno_t pfe_hif_chnl_create_mcal(pfe_hif_chnl_t *chnl, addr_t cbus_base_va, uint32_t id, const pfe_bmu_t *bmu)
{
    errno_t ret = EOK;
 ...
 * @return          EOK if success, error code otherwise
 */
__attribute__((cold)) errno_t pfe_hif_chnl_create_minihif(pfe_hif_chnl_t *chnl, addr_t cbus_base_va, uint32_t id, pfe_hif_ring_t *rx_ring, pfe_hif_ring_t *tx_ring)
{
 ...
 */
__attribute__((pure, hot)) bool_t pfe_hif_chnl_can_accept_tx_num(const pfe_hif_chnl_t *chnl, uint16_t number)
{
    bool_t accept_tx;
    uint32_t fill_level;
    uint32_t ring_len;
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        PfeDevAssert(ring_len >= fill_level);
        accept_tx = ((ring_len - fill_level) > number);
    }
    return accept_tx;
}
...
static __attribute__((hot)) errno_t pfe_hif_chnl_add_vlan_tag(pfe_hif_chnl_t *chnl, void **buf_va, pfe_ct_hif_rx_hdr_t **hif_hdr_ptr, uint32_t *vlan_header)
{
    ...
    {
        if ((oal_ntohl((*hif_hdr_ptr)->flags) & (flags_map[chnl->id])) != (uint32_t)0U)
        {
...
__attribute__((cold)) errno_t pfe_hif_chnl_init(pfe_hif_chnl_t *chnl)
{
    ...
    static const uint32_t common_hif_id = (ETH_43_PFE_COMMON_HIF == PFE_PHY_IF_ID_HIF_NOCPY)
                                          ? PFE_HIF_CHNL_NOCPY_ID
                                          : (uint32_t)(ETH_43_PFE_COMMON_HIF - PFE_PHY_IF_ID_HIF0);
...
static __attribute__((cold)) errno_t pfe_hif_chnl_send_dummy_frame(pfe_hif_chnl_t *chnl, void *tx_buf_va, send_dummy_frame_mode_t mode)
{
    ...
    switch (mode)
    {
        ...
        case DUMMY_FRAME_IHC_SELF:
            ...
            tx_hdr->chid = (uint8_t)(chnl->id & UINT8_MAX);
            break;
