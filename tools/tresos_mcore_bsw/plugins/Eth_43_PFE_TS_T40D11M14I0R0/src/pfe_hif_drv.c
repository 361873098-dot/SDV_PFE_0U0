...
static void pfe_hif_drv_invoke_Tx_conf(pfe_hif_drv_t *hif_drv);
...
static void pfe_hif_drv_notify_clients(pfe_hif_drv_t *hif_drv, uint32_t tx_clients, pfe_hif_drv_client_t *client);
static void pfe_hif_drv_notify_rx_clients(uint32_t rx_clients, pfe_hif_drv_t *hif_drv);
...
static errno_t pfe_hif_drv_client_transmit_packet_buffers(pfe_hif_drv_client_t *client, const hif_drv_sg_list_t *const sg_list);
static errno_t pfe_hif_drv_put_data_to_tx_conf_queue(pfe_hif_drv_t *hif_drv, pfe_hif_drv_client_t *client, uint32_t *tx_clients, pfe_hif_tx_meta_t *tx_metadata);

#ifdef PFE_CFG_IEEE1588_SUPPORT
static errno_t pfe_hif_drv_cfg_IEEE1588(pfe_hif_drv_client_t *client, pfe_ct_hif_tx_hdr_t *tx_hdr, pfe_hif_tx_meta_t *tx_metadata, const hif_drv_sg_list_t *const sg_list, void *ref_ptr);
static void pfe_hif_drv_process_ts(uint32_t rx_len, pfe_ct_hif_rx_hdr_t *hif_hdr_ptr, pfe_hif_drv_client_t *client, void *current_buffer_va);
#endif /*PFE_CFG_IEEE1588_SUPPORT*/
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        if ((uint8_t)client_data->phy_if_id >= HIF_CLIENTS_MAX)
        {
        ...
        else
        {
            ...
            else
            {
                ...
                client_tmp->id_mask = ((uint32_t)1U << (uint8_t)client_data->phy_if_id);
...
#if (TRUE == PFE_CFG_HIF_IRQ_ENABLED)
        /* Notify client(s) about new confirmations */
        pfe_hif_drv_notify_clients(hif_drv, tx_clients, client);
#endif /* PFE_CFG_HIF_IRQ_ENABLED */
...
static errno_t pfe_hif_drv_client_transmit_packet_buffers(pfe_hif_drv_client_t *client, const hif_drv_sg_list_t *const sg_list)
{
    ...
    for (ii = 0U; ii < sg_list->size; ii++)
    {
        ...
        if (unlikely(EOK != err))
        {
            /* We need somehow reset the TX BD Ring because HIF header has already been enqueued. */
...
#ifdef PFE_CFG_IEEE1588_SUPPORT
            err = pfe_hif_drv_cfg_IEEE1588(client, tx_hdr, tx_metadata, sg_list, ref_ptr);
#endif /* PFE_CFG_IEEE1588_SUPPORT */

            tx_metadata->ref_ptr = ref_ptr;

            /*  Transmit particular packet buffers */
            err = pfe_hif_drv_client_transmit_packet_buffers(client, sg_list);
...
#if (FALSE == HIF_CFG_IRQ_TRIGGERED_TX_CONFIRMATION)
                /* Invoke TX confirmation job */
                pfe_hif_drv_invoke_Tx_conf(hif_drv);
#endif /* HIF_CFG_IRQ_TRIGGERED_TX_CONFIRMATION */
#endif /* HIF_CFG_DETACH_TX_CONFIRMATION_JOB */
...
static errno_t pfe_hif_drv_attach_channels(pfe_hif_drv_t *hif_drv)
{
    ...
    else
    {
        ...
        /* Attach channel out-of-buffers event handler */
            ret = pfe_hif_chnl_set_event_cbk(hif_drv->channel, HIF_CHNL_EVT_RX_OOB, NULL, NULL);
            if (EOK != ret)
            {
                NXP_LOG_ERROR("Could not register RX OOB handler\n");
                pfe_hif_drv_init_err_handler(hif_drv, 7U);
            }
        }
    }
