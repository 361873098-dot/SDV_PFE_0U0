...
#define USDHC_IP_AR_RELEASE_MINOR_VERSION_C     4
#define USDHC_IP_AR_RELEASE_REVISION_VERSION_C  0
#define USDHC_IP_SW_MAJOR_VERSION_C             4
#define USDHC_IP_SW_MINOR_VERSION_C             0
#define USDHC_IP_SW_PATCH_VERSION_C             2
...
#define EEP_START_SEC_VAR_CLEARED_BOOLEAN
#include "Eep_MemMap.h"
...
#define EEP_STOP_SEC_VAR_CLEARED_BOOLEAN
#include "Eep_MemMap.h"
...
static uint32    Usdhc_Ip_u32TempState;
...
ISR(uSDHC0_Isr)
{
    ...
    /* Get gate locker */
    u32GateLocker = (uint32)Sema42_Ip_GetGateLocker(USDHC_MCORE_SEMA4_HARDWARE_INSTANCE, USDHC_MCORE_SEMA4_HARDWARE_CHANNEL);

    if (u32DomainId == u32GateLocker)         /* If the gate was locked by the HLD before */
    {
#endif
                           ...
                           )
{
    ...
    if (Data != NULL_PTR)
    {
        ...
        if (Data->rxData != NULL_PTR)
        {
            ...
            (void)Cache_Ip_CleanByAddr( CACHE_IP_DATA,
                                        (boolean)TRUE,
                                        CacheStartAddress,
...
Usdhc_Ip_StatusType uSDHC_Ip_ProcessFastBoot(uint32 instance,const Usdhc_Ip_BootConfigType * UsdhcBootConfig, uint32 numberBlocks, uint32 * buffer)
{
    ...
    if (STATUS_USDHC_IP_SUCCESS == Usdhc_Ip_eTransferStatus)
    {
        ...
        MixCtrlReg |= USDHC_MIX_CTRL_MSBSEL_MASK | USDHC_MIX_CTRL_DTDSEL_MASK | USDHC_MIX_CTRL_BCEN_MASK | USDHC_ENABLE_AUTO_CMD12_FLAG;
        ...
        if (DmaMode != USDHC_DMA_MODE_NO_DMA)
        {
            ...
            Usdhc_Ip_eTransferStatus = uSDHC_Ip_SetAdmaTableConfig(Base, DmaMode, Usdhc_Ip_State[instance], buffer, (numberBlocks * USDHC_IP_DEFAULT_BLOCK_SIZE));
        }
    }
                                  ...
                                  )
{
    ...
    Usdhc_Ip_u32TempState = (uint32)(Base->SYS_CTRL & Mask);
    MCAL_FAULT_INJECTION_POINT(EEP_FIP_FR_CHECK_WHILE_LOOP_CTRL_REG);

    /* Waiting for reset operation complete */
    while (0U != Usdhc_Ip_u32TempState)
    {
                                 ...
                                 )
{
    ...
    if (STATUS_USDHC_IP_SUCCESS == Status)
    {
        ...
        Base->INT_STATUS_EN = 0U;
        /* Disable all interrupt signal */
        Base->INT_SIGNAL_EN = 0U;
                                          ...
                                          )
{
    ...
    Usdhc_Ip_u32TempState = (uint32)(uSDHC_Ip_GetPresentStatusFlags(Instance) & (uint32)USDHC_PRES_STATE_SDSTB_MASK);

    /* Label for checking test timeout */
    MCAL_FAULT_INJECTION_POINT(EEP_CHECK_WHILE_LOOP_PRESENT_STATUS_FLAG_1);
    ...
    Usdhc_Ip_u32TempState = (uint32)(uSDHC_Ip_GetPresentStatusFlags(Instance) & ((uint32)USDHC_PRES_STATE_CIHB_MASK | (uint32)USDHC_PRES_STATE_CDIHB_MASK));

    /* Label for checking test timeout */
    MCAL_FAULT_INJECTION_POINT(EEP_CHECK_WHILE_LOOP_PRESENT_STATUS_FLAG_2);
    ...
    if (TRUE == IsLowVoltage)
    {
        ...
        Usdhc_Ip_u32TempState = (uint32)(uSDHC_Ip_GetPresentStatusFlags(Instance) & ((uint32)USDHC_DATA0_LINE_LEVEL));

        /* Label for checking test timeout */
        MCAL_FAULT_INJECTION_POINT(EEP_CHECK_WHILE_LOOP_PRESENT_STATUS_FLAG_3);
                                        ...
                                        )
{
    ...
    *NearestFrequency = (BusClock / FinalPrescaler) / FinalDivisor;
    ...
    MCAL_FAULT_INJECTION_POINT(EEP_CHECK_WHILE_LOOP_IP_SET_BUS_CLOCK);
...
Usdhc_Ip_StatusType uSDHC_Ip_SetCardActive(uint32 Instance)
{
    ...
    Usdhc_Ip_u32TempState = (uint32)(Base->SYS_CTRL & USDHC_SYS_CTRL_INITA_MASK);

    MCAL_FAULT_INJECTION_POINT(EEP_CHECK_WHILE_LOOP_IP_SET_CARD_ACTIVE);
                                    ...
                                    )
{
    ...
    MCAL_FAULT_INJECTION_POINT(EEP_CHECK_WHILE_LOOP_TRANFER_PROCESS);
...
Usdhc_Ip_StatusType uSDHC_Ip_Abort(uint32 Instance)
{
    ...
    if (STATUS_USDHC_IP_PENDING == Usdhc_Ip_eTransferStatus)
    {
        ...
        MCAL_FAULT_INJECTION_POINT(EEP_CHECK_WHILE_LOOP_ABORT);
