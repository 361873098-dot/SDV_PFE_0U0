...
#define CRYPTO_UTIL_SW_MAJOR_VERSION_C              4
#define CRYPTO_UTIL_SW_MINOR_VERSION_C              0
#define CRYPTO_UTIL_SW_PATCH_VERSION_C              2
...
/**
* @brief           A function to obtain the redirected key info.
* @details         Using the redirection configuration, this function will obtain
*                  the Key Id and Key element Id and store them to be used in the
*                  caller function.
*
* @param[in]       pJob: Pointer to the job
* @param[out]      pSelectedKeyIdRedirectInfo: Pointer to the SelectedKeyIdRedirectInfo
* @param[out]      pSelectedKeyElementIdRedirectInfo: Pointer to the SelectedKeyElementIdRedirectInfo
...
*
*/
static inline void Crypto_Util_KeyRedirectInfo
(
    const Crypto_JobType*                         pJob,
    uint32 * const                                pSelectedKeyIdRedirectInfo,
    uint32 * const                                pSelectedKeyElementIdRedirectInfo,
    const Crypto_InputOutputRedirectionConfigType redirectionConfiguration
)
{
    /* Assign inputKeyId and inputKeyElementId for the selected IO. */
    switch (redirectionConfiguration)
    {
        case CRYPTO_REDIRECT_CONFIG_PRIMARY_INPUT:
            *pSelectedKeyIdRedirectInfo        = pJob->jobRedirectionInfoRef->inputKeyId;
            *pSelectedKeyElementIdRedirectInfo = pJob->jobRedirectionInfoRef->inputKeyElementId;
            break;

        case CRYPTO_REDIRECT_CONFIG_SECONDARY_INPUT:
            *pSelectedKeyIdRedirectInfo        = pJob->jobRedirectionInfoRef->secondaryInputKeyId;
            *pSelectedKeyElementIdRedirectInfo = pJob->jobRedirectionInfoRef->secondaryInputKeyElementId;
            break;

        case CRYPTO_REDIRECT_CONFIG_TERTIARY_INPUT:
            *pSelectedKeyIdRedirectInfo        = pJob->jobRedirectionInfoRef->tertiaryInputKeyId;
            *pSelectedKeyElementIdRedirectInfo = pJob->jobRedirectionInfoRef->tertiaryInputKeyElementId;
            break;

        case CRYPTO_REDIRECT_CONFIG_PRIMARY_OUTPUT:
            *pSelectedKeyIdRedirectInfo        = pJob->jobRedirectionInfoRef->outputKeyId;
            *pSelectedKeyElementIdRedirectInfo = pJob->jobRedirectionInfoRef->outputKeyElementId;
            break;

        case CRYPTO_REDIRECT_CONFIG_SECONDARY_OUTPUT:
            *pSelectedKeyIdRedirectInfo        = pJob->jobRedirectionInfoRef->secondaryOutputKeyId;
            *pSelectedKeyElementIdRedirectInfo = pJob->jobRedirectionInfoRef->secondaryOutputKeyElementId;
            break;
...
/**
* @brief           A function to assign the Crypto element array and size locations.
* @details         Using the redirection configuration and the pRedirectedKeyElemIdx
*                  previously obtained caller function as array index, this
*                  function will update the corresponding IO pointers.
*
* @param[out]      pJobPrimitiveIOUpdate: Pointer to the structure of inputs and outputs to be redirected
* @param[in]       pRedirectedKeyElemIdx: Pointer to the RedirectedKeyElemIdx
* @param[in]       redirectionConfiguration: Used to select the settings for the redirected input or output
* @returns         void
*
*
...
*
*/
static inline void Crypto_Util_AssignElementArray
(
    Crypto_JobPrimitiveIOUpdateType*              pJobPrimitiveIOUpdate,
    const uint32 *                                pRedirectedKeyElemIdx,
    const Crypto_InputOutputRedirectionConfigType redirectionConfiguration
)
{
    /* Check if the RedirectedKeyElemIdx is valid */
    if(CRYPTO_INVALID_KEY_ELEMENT_IDX_U32 != *pRedirectedKeyElemIdx)
    {
        /* Update the Crypto element aray and actual size for the current IO. */
        switch (redirectionConfiguration)
        {
            case CRYPTO_REDIRECT_CONFIG_PRIMARY_INPUT:
                pJobPrimitiveIOUpdate->pInput                 = Crypto_aKeyElementList[*pRedirectedKeyElemIdx].pCryptoElementArray;
                pJobPrimitiveIOUpdate->pInputLength           = Crypto_aKeyElementList[*pRedirectedKeyElemIdx].pu32CryptoElementActualSize;
                break;

            case CRYPTO_REDIRECT_CONFIG_SECONDARY_INPUT:
                pJobPrimitiveIOUpdate->pSecondaryInput        = Crypto_aKeyElementList[*pRedirectedKeyElemIdx].pCryptoElementArray;
                pJobPrimitiveIOUpdate->pSecondaryInputLength  = Crypto_aKeyElementList[*pRedirectedKeyElemIdx].pu32CryptoElementActualSize;
                break;

            case CRYPTO_REDIRECT_CONFIG_TERTIARY_INPUT:
                pJobPrimitiveIOUpdate->pTertiaryInput         = Crypto_aKeyElementList[*pRedirectedKeyElemIdx].pCryptoElementArray;
                pJobPrimitiveIOUpdate->pTertiaryInputLength   = Crypto_aKeyElementList[*pRedirectedKeyElemIdx].pu32CryptoElementActualSize;
                break;

            case CRYPTO_REDIRECT_CONFIG_PRIMARY_OUTPUT:
                pJobPrimitiveIOUpdate->pOutput                = Crypto_aKeyElementList[*pRedirectedKeyElemIdx].pCryptoElementArray;
                /* Update the Crypto element actual size to max size. */
                *(Crypto_aKeyElementList[*pRedirectedKeyElemIdx].pu32CryptoElementActualSize) = Crypto_aKeyElementList[*pRedirectedKeyElemIdx].u32CryptoKeyElementMaxSize;
                pJobPrimitiveIOUpdate->pOutputLength          = Crypto_aKeyElementList[*pRedirectedKeyElemIdx].pu32CryptoElementActualSize;
                break;

            case CRYPTO_REDIRECT_CONFIG_SECONDARY_OUTPUT:
                pJobPrimitiveIOUpdate->pSecondaryOutput       = Crypto_aKeyElementList[*pRedirectedKeyElemIdx].pCryptoElementArray;
                /* Update the Crypto element actual size to max size. */
                *(Crypto_aKeyElementList[*pRedirectedKeyElemIdx].pu32CryptoElementActualSize) = Crypto_aKeyElementList[*pRedirectedKeyElemIdx].u32CryptoKeyElementMaxSize;
                pJobPrimitiveIOUpdate->pSecondaryOutputLength = Crypto_aKeyElementList[*pRedirectedKeyElemIdx].pu32CryptoElementActualSize;
                break;
...
)
{
    ...
    /* Get the key Id and key element Id for the current IO. */
    Crypto_Util_KeyRedirectInfo(pJob, &u32SelectedKeyIdRedirectInfo, &u32SelectedKeyElementIdRedirectInfo, redirectionConfiguration);
    ...
    if(CRYPTO_INVALID_KEY_IDX_U32 != u32RedirectedKeyIdx)
    {
        ...
        Crypto_Util_AssignElementArray(pJobPrimitiveIOUpdate, &u32RedirectedKeyElemIdx, redirectionConfiguration);
    }
    else
    {
