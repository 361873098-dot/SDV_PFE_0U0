...
extern volatile uint32_t *pfeRxCtr;
extern volatile uint32_t *pfeTxCtr;
extern volatile uint32_t *pfeTxErrorCtr;
extern uint32_t ipv4_string_to_int(const char *p_ip_addr);
...
void * _sbrk (ptrdiff_t __incr)
{
    ...
    uint32_t *prev_heap;
    static uint32_t *HeapEndPtr = (uint32_t *)&_heapStart;
    void * Status;
...
static err_t pfe_netif_output(struct netif *netif, struct pbuf *p)
{
    ...
    eth_type = (*((uint8_t*)p->payload+12) << 8) | *((uint8_t*)p->payload+13);
...
static err_t pfe_netif_init(struct netif *netif)
{
    ...
    uint8_t ctrl_idx = netif->name[1] - '0';

    for (uint8_t j = 0U; j < NETIF_MAX_HWADDR_LEN; j++)
    {
...
{
    uint32_t netmask_num = lwip_htonl(ipv4_string_to_int(LWIP_MASK));
    uint8_t i;
    struct netif *netif;
#if LWIP_IPV4
