...
{
#if (FALSE == PFE_HIF_CFG_USE_BD_POLLING)
    hal_write32(0U, base_va + HIF_RX_CHn_START(channel_id));
#else
    (void)base_va;
    (void)channel_id;
#endif /* PFE_HIF_CFG_USE_BD_POLLING */
...
{
#if (FALSE == PFE_HIF_CFG_USE_BD_POLLING)
    hal_write32(0U, base_va + HIF_TX_CHn_START(channel_id));
#else
    (void)base_va;
    (void)channel_id;
#endif /* PFE_HIF_CFG_USE_BD_POLLING */
...
{
    uint32_t reg = hal_read32(base_va + HIF_LTC_MAX_PKT_CHn_ADDR(channel_id));

    reg &= ~(HIF_LTC_MAX_PKT_CHN_MASTER_UP(1));
    reg |= HIF_LTC_MAX_PKT_CHN_MASTER_UP(value);

    hal_write32(reg, base_va + HIF_LTC_MAX_PKT_CHn_ADDR(channel_id));
}
