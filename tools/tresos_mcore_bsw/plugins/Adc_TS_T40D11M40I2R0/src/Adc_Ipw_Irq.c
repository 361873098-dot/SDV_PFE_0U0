...
#define ADC_IPW_SW_PATCH_VERSION_IRQ_C               2
/*==================================================================================================
*                                     FILE VERSION CHECKS
==================================================================================================*/
...
static inline void Adc_Ipw_EndSoftwareConv(Adc_HwUnitType Unit,
        ...
        defined(ADC_UNIT_0_INJECTED_ISR_USED) || defined(ADC_UNIT_1_INJECTED_ISR_USED) || defined(ADC_UNIT_2_INJECTED_ISR_USED) || \
        defined(ADC_UNIT_0_DMA_TRANSFER_USED) || defined(ADC_UNIT_1_DMA_TRANSFER_USED) || defined(ADC_UNIT_2_DMA_TRANSFER_USED) || \
        ((defined(ADC_CTU_FIFO_0_OF_ISR_USED) || defined(ADC_CTU_FIFO_1_OF_ISR_USED)) && (ADC_IPW_CTU_AVAILABLE == STD_ON)) \
    )
#if (ADC_SETCHANNEL_API == STD_ON)
...
*
*/
/*TODO: this function duplicates Adc_Ipw_IsChannelEnable from Adc_Ipw.c. Duplicated to allow inline */
                                                  ...
                                                  uint32 CoreId)
{
    ...
    for (Ch = 0U; Ch < ChannelCount; Ch++)
    {
        ...
        /* Get the result */
        /*TODO: Should support ADC_CONV_TYPE_CTU by generation in Group Config */
                                                                   ...
                                                                   boolean RuntimeChanUpdated)
{
    ...
    if (TRUE == GroupPtr->AdcOptimizeDmaStream)
    {
        ...
        if (CurrentIter != GroupSamples)
        {
            ...
            NumsCurrentSample = (Adc_StreamNumSampleType)(GroupSamples - (Adc_StreamNumSampleType)CurrentIter);
        }
        else
        {
...
#endif /* ADC_DMA_SUPPORTED */
    {
           ...
           )
        {
            /* Increment ResultIndex with modulo 'NumSamples' to remember that another sample is completed */
             Adc_axGroupStatus[Group].ResultIndex = (Adc_StreamNumSampleType)(Adc_axGroupStatus[Group].ResultIndex + (Adc_StreamNumSampleType)1U);
        }
                                               ...
                                               uint32 CoreId)
{
#if ( \
     defined(ADC_UNIT_0_INJECTED_ISR_USED) || defined(ADC_UNIT_1_INJECTED_ISR_USED) || defined(ADC_UNIT_2_INJECTED_ISR_USED) || \
     (FEATURE_ADC_HAS_INJ_EXT_TRIGGER) || \
     ((ADC_HARDWARE_NORMAL_CONVERSIONS_USED == STD_ON) && ((FEATURE_ADC_HAS_EXT_TRIGGER) || \
    ...
    if (TRUE == InjectedFlag)
    {
        ...
        {
            /* Conversion triggered by CTU */
            Ctu_Ip_DisableTrigger(ADC_IPW_CTU_UNIT_USED, (uint8)TriggerSrc);
            /* Make sure that value is loaded to double-buffer THCRx register */
                                           ...
                                           uint32 CoreId)
{
    ...
    const uint32 AdcDmaLogicChId = (uint32) AdcIpwConfigPtr->Mapping.AdcDmaChannelLogicId[Unit];
#endif /* ADC_DMA_SUPPORTED */
#if (defined(ADC_DMA_SUPPORTED) || ((ADC_IPW_CTU_TRIGGER_MODE_SUPPORTED == STD_ON) && (ADC_IPW_CTU_AVAILABLE == STD_ON)))
    Adc_HwUnitType PhysicalHwUnitId = Adc_apxCfgPtr[CoreId]->AdcIpwConfigPtr->AdcPhysicalIdArr[Unit];
#endif /* defined(ADC_DMA_SUPPORTED) || .... */
#if ((ADC_IPW_CTU_TRIGGER_MODE_SUPPORTED == STD_ON) && (ADC_IPW_CTU_AVAILABLE == STD_ON))
    uint16 ConvData[CTU_IP_FIFO_DEPTH];
#endif /* ((ADC_IPW_CTU_TRIGGER_MODE_SUPPORTED == STD_ON) && (ADC_IPW_CTU_AVAILABLE == STD_ON)) */
#if (ADC_OPTIMIZE_ONESHOT_HW_TRIGGER == STD_OFF)
    boolean RuntimeChanUpdated = FALSE;
    uint32 CurrentSampleCount = 0U;
#endif /* (ADC_OPTIMIZE_ONESHOT_HW_TRIGGER == STD_OFF) */
...
#if (ADC_IPW_CTU_AVAILABLE == STD_ON)
            /* Read all result Fifo to make sure FIFO is empty after get the result */
            /* Assumes that there is no incoming HW trigger while ISR Handler is executing */
            /* Adc Hw Unit can pass as input of Ctu_Ip_GetFifoData because it is associated with Fifo Index */
            /* Physical Unit 0 means FIFO0, Physical Unit 1 means FIFO1 */
            Ctu_Ip_GetFifoData(ADC_IPW_CTU_UNIT_USED, PhysicalHwUnitId, CTU_IP_RESULT_ALIGN_RIGHT_UNSIGNED, CTU_IP_FIFO_DEPTH, ConvData);
            /* Avoid compiler warning because data is only read to empty the result Fifo */
            (void) ConvData[CTU_IP_FIFO_DEPTH - 1U];
#endif /* ADC_IPW_CTU_AVAILABLE == STD_ON */
...
#endif /* (defined(ADC_DMA_SUPPORTED) && defined(ADC_UNIT_2) && defined(ADC_UNIT_2_DMA_TRANSFER_USED)) */
...
#if (ADC_HW_TRIGGER_API == STD_ON)
#if (ADC_IPW_CTU_AVAILABLE == STD_ON)
#ifdef ADC_CTU_FIFO_0_OF_ISR_USED
void Adc_Ipw_CtuOverflowFifoAdc0(void)
{
...
#endif /* ADC_CTU_FIFO_0_OF_ISR_USED */

#ifdef ADC_CTU_FIFO_1_OF_ISR_USED
void Adc_Ipw_CtuOverflowFifoAdc1(void)
{
    const volatile uint32 CoreId = Adc_GetCoreID();
...
#endif /* ADC_CTU_FIFO_1_OF_ISR_USED */
#endif /* (ADC_IPW_CTU_AVAILABLE == STD_ON) */
#endif /* (ADC_HW_TRIGGER_API == STD_ON) */
