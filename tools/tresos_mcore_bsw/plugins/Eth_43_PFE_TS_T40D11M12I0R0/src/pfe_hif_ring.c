...
#if !defined(PFE_CFG_DETACHED_MINIHIF) /* Only for MCAL. Minihif uses memory provided by user */
...
__attribute__((cold)) static errno_t pfe_hif_ring_attach_std(pfe_hif_ring_t *ring, void *ring_va, void *wb_ring_va, uint32_t length, bool_t is_rx);
#if !defined(PFE_CFG_HIF_NOCPY_SUPPORT)
#if !defined(PFE_CFG_DETACHED_MINIHIF)
    __attribute__((cold)) static errno_t pfe_hif_ring_create_std(pfe_hif_ring_t *ring, uint32_t length, bool_t rx);
#endif /* PFE_CFG_DETACHED_MINIHIF */
#endif /* PFE_CFG_HIF_NOCPY_SUPPORT */
/* When attached driver uses HIF_NOCPY and configure normal HIF for minihif purpose, we have to define PFE_CFG_USE_MINIHIF */
#if (!defined(PFE_CFG_HIF_NOCPY_SUPPORT)) || (defined(PFE_CFG_USE_MINIHIF))
...
#endif /* PFE_CFG_HIF_NOCPY_SUPPORT || PFE_CFG_USE_MINIHIF */
#if defined(PFE_CFG_HIF_NOCPY_SUPPORT)
    #if !defined(PFE_CFG_DETACHED_MINIHIF)
    __attribute__((cold)) static errno_t pfe_hif_ring_create_nocpy(pfe_hif_ring_t *ring, uint32_t length, bool_t rx);
    #endif /* PFE_CFG_DETACHED_MINIHIF */
    __attribute__((cold)) static errno_t pfe_hif_ring_attach_nocpy(pfe_hif_ring_t *ring, void *ring_va, uint32_t length, bool_t is_rx);
...
__attribute__((pure, hot)) bool_t pfe_hif_ring_is_below_wm(const pfe_hif_ring_t *ring)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        if (pfe_hif_ring_get_fill_level(ring) >= (ring->length / 2U))
        {
            is_below_wm = TRUE;
        }
        else
...
__attribute__((pure, hot)) uint32_t pfe_hif_ring_get_fill_level(const pfe_hif_ring_t *ring)
{
    ...
    {
        NXP_LOG_ERROR("NULL argument received\n");
        fill_level = (int32_t)0xFFFFFFFFU;
    }
    else
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
...
#endif /* PFE_CFG_HIF_NOCPY_SUPPORT */
        {
            fill_level = (int32_t)ring->write_idx - (int32_t)ring->read_idx;
            if (fill_level < 0)
            {
                fill_level += 2 * (int32_t)ring->length;
            }
        }
    }
    return (uint32_t)fill_level;
}
...
__attribute__((hot)) static inline errno_t pfe_hif_ring_enqueue_buf_std(pfe_hif_ring_t *ring, const void *buf_pa, uint32_t length, bool_t lifm)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        else
        {
            ...
            ring->bd_write.wr_bd->buflen = (uint16_t)length;
 ...
 * @param[in]      nocpy If TRUE then HIF NOCPY variant is requested
 * @return         EOK if success, error code otherwise
 * @note           Must not be preempted by any of the remaining API functions
 */
/* TODO: delete this function and call pfe_hif_ring_attach instead */
__attribute__((cold)) errno_t pfe_hif_ring_create(pfe_hif_ring_t *ring, uint32_t length, bool_t rx, bool_t nocpy)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        if (TRUE == nocpy)
        {
            ret_val = pfe_hif_ring_create_nocpy(ring, length, rx);
        }
#else
        if (FALSE == nocpy)
        {
            ret_val = pfe_hif_ring_create_std(ring, length, rx);
        }
#endif /* PFE_CFG_HIF_NOCPY_SUPPORT */
        else
        {
            NXP_LOG_ERROR("HIF variant mismatch\n");
            ret_val = EACCES;
        }
    }
...
__attribute__((cold)) static errno_t pfe_hif_ring_create_nocpy(pfe_hif_ring_t *ring, uint32_t length, bool_t rx)
{
    ...
    /*  Assign static memory for buffer descriptors */
    if (TRUE == rx)
    {
        if (unlikely(length > ETH_43_PFE_MAX_RXBD_CNT))
        {
            NXP_LOG_ERROR("Rx BD memory is too small\n");
        }
        else
        {
    ...
    {
        if (unlikely(length > ETH_43_PFE_MAX_TXBD_CNT))
        {
            NXP_LOG_ERROR("Tx BD memory is too small\n");
        }
        else
        {
    ...
    if (NULL != ring_va)
    {
        (void)autolibc_memset(ring_va, 0, (length * sizeof(pfe_hif_nocpy_bd_t)));
        /*  Initialize ring structure, includes some checks */
        ret_val = pfe_hif_ring_attach_nocpy(ring, ring_va, length, rx);
    }
...
__attribute__((cold)) static errno_t pfe_hif_ring_create_std(pfe_hif_ring_t *ring, uint32_t length, bool_t rx)
{
    ...
    else
    {
        ...
        /*  Initialize ring structure, includes some checks */
        ret_val = pfe_hif_ring_attach_std(ring, ring_va, wb_ring_va, length, rx);
    }
...
/**
 * @brief          Attach to existing ring
 * @brief          Will create ring instance but not allocate ring memory. This is useful
 *                 as alternative to pfe_hif_ring_create() when memory for the BD rings
 *                 has already been allocated and user don't need to do the allocation by
 *                 to be done by the ring driver.
 * @param[inout]   ring Pointer to BD ring
 * @param[in]      ring_va Pointer to memory allocated for BD ring (virtual)
 * @param[in]      wb_ring_va Pointer to memory allocated for WB BD ring (virtual)
 * @param[in]      rx If TRUE the ring is RX, if FALSE the the ring is TX
 * @param[in]      nocpy If TRUE then HIF NOCPY variant is requested
 * @param[in]      init_bd If TRUE then ring memory will be initialized
...
__attribute__((cold)) errno_t pfe_hif_ring_attach(pfe_hif_ring_t *ring, void *ring_va, void *wb_ring_va, uint32_t length, bool_t is_rx, bool_t nocpy, bool_t init_bd)
{
    ...
    if (TRUE == nocpy)
    {
        NXP_LOG_ERROR("Nocpy support is not available\n");
        ret_val = EINVAL;
    }
    else
#endif
#if defined(PFE_CFG_NULL_ARG_CHECK)
    if (unlikely((NULL == ring) || (NULL == ring_va) || ((FALSE == nocpy) && (NULL == wb_ring_va))))
    {
        NXP_LOG_ERROR("NULL argument received\n");
        ret_val = EINVAL;
    }
...
#if defined(PFE_CFG_HIF_NOCPY_SUPPORT) /* NOCOPY INTERFACE */ /* FIXME cannot be #if #else - must support minihif */
    if (TRUE == nocpy)
    {
        ret_val = pfe_hif_ring_attach_nocpy(ring, ring_va, length, is_rx);
        if ((EOK == ret_val) && (TRUE == init_bd))
        {
...
#endif
    {
        ret_val = pfe_hif_ring_attach_std(ring, ring_va, wb_ring_va, length, is_rx);
#if defined(PFE_CFG_HIF_NOCPY_SUPPORT)
        /* We can get here only if initializing ring for minihif, init_bd must be FALSE */
        if (TRUE == init_bd)
        {
            NXP_LOG_ERROR("Driver is configured for standard HIF. Cannot initialize BD ring for HIF_NOCPY\n");
            ret_val = EINVAL;
        }
#else
        if ((EOK == ret_val) && (TRUE == init_bd))
...
__attribute__((cold)) static void pfe_hif_ring_init_bd_nocpy(const pfe_hif_ring_t *ring)
{
    ...
    /*  Invalidate the cache over the first descriptor so next attempt to access
        it (enqueue/dequeue) will result in a cache miss. Further descriptors are
        treated using the prefetch approach. */
    /* TBD_MINI ??? is it needed here?: oal_mm_cache_inval(ring->base_va, ring->base_pa, sizeof(pfe_hif_nocpy_bd_t));*/
}
#endif /* PFE_CFG_HIF_NOCPY_SUPPORT */
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        desc_en_value = ring->rd_wb_bd->control.info.desc_en;
    }
    return (desc_en_value > 0U) ? TRUE : FALSE;
}
