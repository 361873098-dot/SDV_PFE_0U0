...
[!NOCODE!]
        ...
        [!SELECT "(node:ref($FrIfCluster)/FrIfController/*[node:ref(FrIfFrCtrlRef)=node:ref($FrController)])"!]
            [!IF "$FrController = node:path(node:ref(FrIfFrCtrlRef))"!]
                ...
                [!SELECT "node:ref($FrIfCluster)/../.."!]
                    [!VAR "FrIfConfigIndex" = "position()"!]
                [!ENDSELECT!]
                [!WS "0"!]static const Fr_CCLpduInfoType FrIfMC[!"$FrIfConfigIndex"!]_Clst[!"num:i(node:ref($FrIfCluster)/FrIfClstIdx)"!]_Ctrl[!"num:i(node:ref($FrController)/FrCtrlIdx)"!]_LPduInfoCfgSet_[!"string($CfgVariant)"!][] =
                [!WS "0"!]{
                    ...
                    [!IF "node:name(node:ref(node:ref(node:ref(FrIfVBTriggeringRef)/FrIfFrameStructureRef)/FrIfPdusInFrame/*[position()=1]/FrIfPduRef)/FrIfPduDirection)='FrIfRxPdu'"!]
                        ...
                        [!IF "node:value(node:ref(FrIfVBTriggeringRef)/FrIfChannel)='FRIF_CHANNEL_A'"!]
                            ...
                            [!IF "text:contains(string($FIFOASlots), $MacTmpFrIfSlotId)"!]
                                ...
                                [!ENDIF!][!// FIFO A
                                [!VAR "MacNotFifo" = "false()"!]
                                [!WS "4"!]{FR_FIFOA_BUFFER, 0U, (boolean)FALSE, 0U, (boolean)FALSE}[!//
                        ...
                        [!IF "node:value(node:ref(FrIfVBTriggeringRef)/FrIfChannel)='FRIF_CHANNEL_B'"!]
                            ...
                            [!IF "text:contains(string($FIFOBSlots), $MacTmpFrIfSlotId)"!]
                                ...
                                [!ENDIF!][!// FIFO B
                                [!VAR "MacNotFifo" = "false()"!]
                                [!WS "4"!]{FR_FIFOB_BUFFER, 0U, (boolean)FALSE, 0U, (boolean)FALSE}[!//
                                [!VAR "MacComma" = "concat(', /* LPdu ', num:i(FrIfLPduIdx),' */')"!]
    ...
    [!MACRO "PSRRegisters", "CfgIdx" = "0", "CfgVariant" = "", "FrIfCluster" = "", "FrController" = ""!]
        ...
        [!WS "1"!] *  parameters returned by the Fr_ReadCCConfig function */
        [!WS "0"!]static const uint32 FrMC[!"num:i($CfgIdx)"!]_Ctrl[!"num:i(node:ref($FrController)/FrCtrlIdx)"!]_CCReadBackParamSet_[!"string($CfgVariant)"!][] =
        [!WS "0"!]{
        ...
        [!WS "0"!]static const Fr_CCLowLevelConfigSetType FrMC[!"num:i($CfgIdx)"!]_Ctrl[!"num:i(node:ref($FrController)/FrCtrlIdx)"!]_CCLowLevelCfgSet_[!"string($CfgVariant)"!] =
        [!WS "0"!]{
        [!WS "0"!][!// PCR0
...
==================================================================================================*/
#define FR[!"$postBuildVariantName"!]_PBCFG_VENDOR_ID_C                      43
#define FR[!"$postBuildVariantName"!]_PBCFG_AR_RELEASE_MAJOR_VERSION_C       4
#define FR[!"$postBuildVariantName"!]_PBCFG_AR_RELEASE_MINOR_VERSION_C       4
#define FR[!"$postBuildVariantName"!]_PBCFG_AR_RELEASE_REVISION_VERSION_C    0
#define FR[!"$postBuildVariantName"!]_PBCFG_SW_MAJOR_VERSION_C               4
#define FR[!"$postBuildVariantName"!]_PBCFG_SW_MINOR_VERSION_C               0
#define FR[!"$postBuildVariantName"!]_PBCFG_SW_PATCH_VERSION_C               2

/*==================================================================================================
*                                       FILE VERSION CHECKS
==================================================================================================*/
/* Check if current file and Fr.h header file are of the same vendor */
#if(FR[!"$postBuildVariantName"!]_PBCFG_VENDOR_ID_C != FR_VENDOR_ID)
...
#if((FR[!"$postBuildVariantName"!]_PBCFG_AR_RELEASE_MAJOR_VERSION_C    != FR_AR_RELEASE_MAJOR_VERSION) || \
    (FR[!"$postBuildVariantName"!]_PBCFG_AR_RELEASE_MINOR_VERSION_C    != FR_AR_RELEASE_MINOR_VERSION) || \
    (FR[!"$postBuildVariantName"!]_PBCFG_AR_RELEASE_REVISION_VERSION_C != FR_AR_RELEASE_REVISION_VERSION))
...
#if((FR[!"$postBuildVariantName"!]_PBCFG_SW_MAJOR_VERSION_C != FR_SW_MAJOR_VERSION) || \
    (FR[!"$postBuildVariantName"!]_PBCFG_SW_MINOR_VERSION_C != FR_SW_MINOR_VERSION) || \
    (FR[!"$postBuildVariantName"!]_PBCFG_SW_PATCH_VERSION_C != FR_SW_PATCH_VERSION))
...
#if(FR[!"$postBuildVariantName"!]_PBCFG_VENDOR_ID_C != FLEXRAY_IP_VENDOR_ID)
    #error "Fr[!"$postBuildVariantName"!]_PBcfg.c and Flexray_Ip.h have different Vendor IDs"
#endif
/* Check if current file and Flexray_Ip.h header file are of the same Autosar version */
#if((FR[!"$postBuildVariantName"!]_PBCFG_AR_RELEASE_MAJOR_VERSION_C    != FLEXRAY_IP_AR_RELEASE_MAJOR_VERSION) || \
    (FR[!"$postBuildVariantName"!]_PBCFG_AR_RELEASE_MINOR_VERSION_C    != FLEXRAY_IP_AR_RELEASE_MINOR_VERSION) || \
    (FR[!"$postBuildVariantName"!]_PBCFG_AR_RELEASE_REVISION_VERSION_C != FLEXRAY_IP_AR_RELEASE_REVISION_VERSION))
    #error "Software Version Numbers of Fr[!"$postBuildVariantName"!]_PBcfg.c and Flexray_Ip.h are different"
#endif
/* Check if current file and Flexray_Ip.h header file are of the same software version */
#if((FR[!"$postBuildVariantName"!]_PBCFG_SW_MAJOR_VERSION_C != FLEXRAY_IP_SW_MAJOR_VERSION) || \
    (FR[!"$postBuildVariantName"!]_PBCFG_SW_MINOR_VERSION_C != FLEXRAY_IP_SW_MINOR_VERSION) || \
    (FR[!"$postBuildVariantName"!]_PBCFG_SW_PATCH_VERSION_C != FLEXRAY_IP_SW_PATCH_VERSION))
    #error "Autosar Version Numbers of Fr[!"$postBuildVariantName"!]_PBcfg.c and Flexray_Ip.h are different"
#endif
    ...
    #if((FR[!"$postBuildVariantName"!]_PBCFG_AR_RELEASE_MAJOR_VERSION_C != DEM_AR_RELEASE_MAJOR_VERSION) || \
        (FR[!"$postBuildVariantName"!]_PBCFG_AR_RELEASE_MINOR_VERSION_C != DEM_AR_RELEASE_MINOR_VERSION))
...
#include "Fr_MemMap.h"
    ...
    [!LOOP "node:order(FrMultipleConfiguration,'node:name(.)')"!][!//Sort the multiple configurations by their name at first
        ...
        [!LOOP "node:order(FrController/*,'FrCtrlIdx','node:name(.)')"!][!//Sort the controllers by their name at first
            ...
            [!ELSE!]
                ...
                [!IF "$NeededBuffers > ecu:get('FR.AvailableBuffers')"!]
                    [!ERROR!]
                        [!WS "0"!]Error 035: Not enough resources for all LPdus, try enable the parameter FrPrepareLPduSupport which enables API function Fr_PrepareLPdu(), so more than one LPdu can share one Message Buf
                    [!ENDERROR!]
                [!ENDIF!]
            [!ENDIF!]
            ...
            [!IF "../../../FrGeneral/VendorSpecific/FrDisableDemReportErrorStatus = 'false'"!]
                ...
                [!WS "0"!]static const Fr_DemErrorType FrMC[!"$FrConfigIdx"!]_Ctrl[!"num:i(node:ref(node:path(.))/FrCtrlIdx)"!]_DemEventParameter_[!"string($CfgVariant)"!] =
                [!WS "0"!]{
        ...
        [!LOOP "node:order(./FrController/*,'FrCtrlIdx','node:name(.)')"!][!//Sort the controllers by their name at first
            ...
            [!WS "0"!]static const Fr_CtrlCfgType Fr_Controller_[!"$CtrlIdx"!] =
            [!WS "0"!]{
            [!WS "4"!][!"$CtrlIdx"!]U,
            [!WS "4"!]&Fr_MC[!"$FrConfigIdx"!]_Ctrl[!"num:i(node:ref($FrController)/FrCtrlIdx)"!]_Cfg_[!"string($CfgVariant)"!][!"$postBuildVariantName"!],
            [!WS "4"!]&Fr_MC[!"$FrConfigIdx"!]_Ctrl[!"num:i(node:ref($FrController)/FrCtrlIdx)"!]_StateAlloc_[!"string($CfgVariant)"!][!"$postBuildVariantName"!],
            [!SELECT "node:ref($FrControllersUsage)/.."!]
            ...
            [!WS "4"!]&FrMC[!"$FrConfigIdx"!]_Ctrl[!"num:i(node:ref(node:path(.))/FrCtrlIdx)"!]_CCLowLevelCfgSet_[!"string($CfgVariant)"!],
            [!WS "4"!]FrMC[!"$FrConfigIdx"!]_Ctrl[!"num:i(node:ref($FrController)/FrCtrlIdx)"!]_CCReadBackParamSet_[!"string($CfgVariant)"!],
            [!IF "../../../FrGeneral/VendorSpecific/FrDisableDemReportErrorStatus = 'false'"!]
                [!WS "4"!]&FrMC[!"$FrConfigIdx"!]_Ctrl[!"num:i(node:ref(node:path(.))/FrCtrlIdx)"!]_DemEventParameter_[!"string($CfgVariant)"!]
            [!ELSE!]
                [!WS "4"!]NULL_PTR
            [!ENDIF!]
            [!WS "0"!]};
        ...
        [!IF "num:i(count(../FrGeneral/FrEcucPartitionRef/*)) = '0'"!]
            [!IF "((../IMPLEMENTATION_CONFIG_VARIANT != 'VariantPostBuild') and (variant:size()>1)) or (../IMPLEMENTATION_CONFIG_VARIANT = 'VariantPostBuild')"!]
                [!WS "0"!]/* This structure puts together all information about controllers for Fr_Config[!"$postBuildVariantName"!][!WS "1"!] multiple configuration */
                [!WS "0"!]const Fr_ConfigurationType Fr_Config[!"$postBuildVariantName"!] =
            [!ELSE!]
                [!WS "0"!]/* This structure puts together all information about controllers for Fr_Config multiple configuration */
                [!WS "0"!]static const Fr_ConfigurationType Fr_Config =
            [!ENDIF!]
            [!WS "0"!]{
                [!WS "4"!]{
            [!VAR "FrCtrlIdxMax" = "num:i('0')"!]
            [!LOOP "node:order(./FrController/*,'FrCtrlIdx','node:name(.)')"!]
                [!VAR "FrController" = "node:path(.)"!]
                [!VAR "CtrlIdx" = "num:i(node:value(./FrCtrlIdx))"!]
                [!IF "$CtrlIdx > $FrCtrlIdxMax"!]
                    [!VAR "FrCtrlIdxMax" = "$CtrlIdx"!]
                [!ENDIF!]
            [!ENDLOOP!]
            [!FOR "CtrlIdx" = "num:i('0')" TO "$FrCtrlIdxMax"!]
                [!IF "node:exists(./FrController/*[FrCtrlIdx = $CtrlIdx]) = 'true'"!]
                    [!WS "0"!][!//Store the reference to the multiple cluster using this FrController
                    [!SELECT "./FrController/*[FrCtrlIdx = $CtrlIdx]"!]
                        [!VAR "FrControllersUsage" = "node:path(node:refs('ASPathDataOfSchema:/AUTOSAR/EcucDefs/FrIf/FrIfConfig/FrIfCluster/FrIfController/FrIfFrCtrlRef')[node:path(node:ref(.)) = node:path(node:current())]/..)"!][!//Store the path of the controllers cluster for further accesses
                    [!ENDSELECT!]
                    [!SELECT "node:ref($FrControllersUsage)/../../../.."!]
                        [!VAR "FrIfConfigIndex" = "position()"!]
                    [!ENDSELECT!]
                    [!WS "8"!]&Fr_Controller_[!"$CtrlIdx"!][!IF "$CtrlIdx < $CtrlConfigCount"!],[!ENDIF!][!CR!]
                [!ELSE!]
                    [!WS "8"!]0U[!IF "$CtrlIdx < $CtrlConfigCount"!],[!ENDIF!][!CR!]
                [!ENDIF!]
            [!ENDFOR!][!//FrController
                [!WS "4"!]},
                [!WS "4"!]0U
        ...
        [!ELSE!]
            [!LOOP "../FrGeneral/FrEcucPartitionRef/*"!]
                ...
                [!IF "((../../../IMPLEMENTATION_CONFIG_VARIANT != 'VariantPostBuild') and (variant:size()>1)) or (../../../IMPLEMENTATION_CONFIG_VARIANT = 'VariantPostBuild')"!]
                    [!WS "0"!]/* This structure puts together all information about controllers for Fr_Config[!"$postBuildVariantName"!]_[!"node:ref(.)/@name"!][!WS "1"!] multiple configuration */
                    [!WS "0"!]const Fr_ConfigurationType Fr_Config[!"$postBuildVariantName"!]_[!"node:ref(.)/@name"!] =
                [!ELSE!]
                    [!WS "0"!]/* This structure puts together all information about controllers for Fr_Config_[!"node:ref(.)/@name"!][!WS "1"!] multiple configuration */
                    [!WS "0"!]static const Fr_ConfigurationType Fr_Config_[!"node:ref(.)/@name"!] =
                [!ENDIF!]
                [!WS "0"!]{
                    [!WS "4"!]{
        ...
        [!IF "((../IMPLEMENTATION_CONFIG_VARIANT != 'VariantPostBuild') and (variant:size()>1)) or (../IMPLEMENTATION_CONFIG_VARIANT = 'VariantPostBuild')"!]
        [!ELSE!]
            [!WS "0"!]/* This structure puts together all information about controllers for Fr_PBCfgVariantPredefined multiple configuration */
            ...
            [!ENDIF!]
            [!WS "0"!]const Fr_ConfigurationType* Fr_PBCfgVariantPredefined[FR_ECUC_PARTITION] =
            [!WS "0"!]{
            [!IF "num:i(count(../FrGeneral/FrEcucPartitionRef/*)) = '0'"!]
                [!WS "4"!]&Fr_Config
            [!ELSE!]
                ...
                [!LOOP "node:order(as:modconf('EcuC')[1]/EcucHardware/*[1]/EcucCoreDefinition/*, './EcucCoreId')"!]
                    ...
                    [!LOOP "as:modconf('Fr')[1]/FrGeneral/FrEcucPartitionRef/*"!]
                        ...
                        [!ENDIF!]
                    [!ENDLOOP!]
                    [!WS "4"!][!IF "$FrPartMatchCounter = 1"!]&Fr_Config_[!"$Fr_Partition_name"!][!ELSE!]NULL_PTR[!ENDIF!][!IF "$coreIdx < $MaxCoreIdConfig"!],[!ENDIF!][!CR!]
                [!ENDLOOP!]
            [!ENDIF!]
            [!WS "0"!]};
            [!WS "0"!][!CR!]
