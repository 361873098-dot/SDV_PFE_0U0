...
static void pfe_class_check_pes(pfe_class_t *class)
{
    for (uint32_t i = 0U; i < class->pe_num; i++)
    {
    ...
    for (i = 0U; i < class->fw_features_count; i++)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                (void)pfe_fw_feature_set_ll_data(fw_features, entry, (uint8_t)(pfe_class_get_num_of_pes(class) & UINT8_MAX));
...
errno_t pfe_class_get_mmap(pfe_class_t *class, int32_t pe_idx, pfe_ct_class_mmap_t *mmap)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        if ((uint32_t)pe_idx >= class->pe_num)
        {
            ret = EINVAL;
        }
        else
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        PfeDevAssert(class->pe_num <= (uint32_t)INT32_MAX);
        ret = pfe_pe_gather_memcpy_from_dmem_to_host_32(class->pe,
                                                        (int32_t)class->pe_num,
                                                        dst_ptr,
                                                        (addr_t)src_addr,
                                                        buffer_len,
                                                        read_len);
