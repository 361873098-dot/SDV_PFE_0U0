 ...
 */
/* coverity[misra_c_2012_rule_8_13_violation] db cannot be const as the function modifies it when built in master mode. */
...
errno_t pfe_if_db_remove(pfe_if_db_t *db, uint32_t session_id, pfe_if_db_entry_t *entry)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        else
        {
            if (PFE_IF_DB_PHY == db->type)
            {
                if ((&pfe_if_db_phy_ifs[(uint32_t)PFE_PHY_IF_ID_EMAC0] <= entry) &&
                    (&pfe_if_db_phy_ifs[(uint32_t)PFE_PHY_IF_ID_MAX + 1U] > entry))
                {
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        else
        {
            ...
            else
            {
                ...
                PfeDevAssert((pfe_if_db_log_ifs.occupied_items_count > 0U) && (pfe_if_db_log_ifs.occupied_items_count <= (uint32_t)INT32_MAX));
