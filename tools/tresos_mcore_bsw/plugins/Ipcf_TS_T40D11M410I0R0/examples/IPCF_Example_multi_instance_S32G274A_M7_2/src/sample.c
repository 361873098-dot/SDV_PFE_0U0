...
#define WAIT_TIMEOUT 0x01000000
 ...
 * demo function a ping message has been received.
 */
...
void shm_demo(void)
{
	...
	while (1) {
		...
		while ((app.rx_count != app.expected_msgs)
				|| (app.expected_msgs == 0)) {
			(void)ipc_shm_poll_channels(IPCF_INSTANCE);
		}
		...
		for (i = 0; i < app.expected_msgs; i++) {
			...
			buf = ipc_shm_acquire_buf(IPCF_INSTANCE,
							chan_id, MAX_SAMPLE_MSG_LEN);
			...
			ipc_numtostr(i, ipc_strrchr(buf, '#'));
