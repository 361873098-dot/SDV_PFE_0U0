...
[!NOCODE!]
    ...
    [!IF "$IcuEcucPartitionRefCount != 0 "!]
        ...
        [!IF "node:exists(IcuChannel/*[(contains(node:path(node:ref(./IcuChannelRef)), 'IcuSiul2')) and (node:value(./IcuChannelEcucPartitionRef/*[1]) = $IcuEcucPartitionRefName)])"!]
            [!VAR "OutNumInstancesInPartition" = "$OutNumInstancesInPartition + 1"!]
        [!ENDIF!]   
        [!IF "node:exists(IcuChannel/*[(contains(node:path(node:ref(./IcuChannelRef)), 'IcuWkpu')) and (node:value(./IcuChannelEcucPartitionRef/*[1]) = $IcuEcucPartitionRefName)])"!]
            [!VAR "OutNumInstancesInPartition" = "$OutNumInstancesInPartition + 1"!]
        [!ENDIF!]
        [!// Count channels
        [!LOOP "IcuChannel/*"!]
            ...
            [!LOOP "./IcuChannelEcucPartitionRef/*"!]
                [!IF "node:value(.) = $IcuEcucPartitionRefName"!]
                    [!//Update variables storing number of Icu channels using either Ftm, Siul2 or Wkpu IP
                    ...
                    [!ELSEIF "contains($HardwareModule,'IcuWkpu')"!]
    ...
    [!ELSE!]     
        ...
        [!IF "node:exists(IcuChannel/*[(contains(node:path(node:ref(./IcuChannelRef)), 'IcuSiul2'))])"!]
            [!VAR "OutNumInstancesInPartition" = "$OutNumInstancesInPartition + 1"!]
        [!ENDIF!]
        [!IF "node:exists(IcuChannel/*[(contains(node:path(node:ref(./IcuChannelRef)), 'IcuWkpu'))])"!]
            [!VAR "OutNumInstancesInPartition" = "$OutNumInstancesInPartition + 1"!]
        [!ENDIF!]
