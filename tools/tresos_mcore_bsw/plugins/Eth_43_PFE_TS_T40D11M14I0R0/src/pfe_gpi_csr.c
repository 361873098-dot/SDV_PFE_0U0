...
void pfe_gpi_cfg_init(addr_t base_va, const pfe_gpi_cfg_t *cfg)
{
    ...
    hal_write32((uint32_t)(PFE_CFG_DDR_HDR_SIZE << 16) | cfg->lmem_header_size, ADDR_BASE_OFFSET(base_va, GPI_HDR_SIZE));
...
errno_t pfe_gpi_cfg_reset(addr_t base_va)
{
    ...
    uint32_t reg = hal_read32(ADDR_BASE_OFFSET(base_va, GPI_CTRL));
...
void pfe_gpi_cfg_qos_default_init(addr_t base_va)
{
    ...
    /* reset TPID */
    hal_write32(((uint32_t)IGQOS_TPID_DOT1Q << 16) | IGQOS_TPID_DOT1Q, ADDR_BASE_OFFSET(base_va, CSR_IGQOS_TPID));
    /* reset IGQOS CLASS */
...
{
    uint32_t reg = hal_read32(ADDR_BASE_OFFSET(base_va, CSR_IGQOS_CONTROL));
...
{
    bool_t ret = FALSE;
    uint32_t reg = hal_read32(ADDR_BASE_OFFSET(base_va, CSR_IGQOS_ENTRY_CMDSTATUS));
...
{
    uint32_t reg = hal_read32(ADDR_BASE_OFFSET(base_va, CSR_IGQOS_QOS));
...
{
    const addr_t reg_offset = CSR_IQGOS_ZONE_PROB((uint32_t)queue);
    uint32_t reg = hal_read32(ADDR_BASE_OFFSET(base_va, reg_offset));
...
void pfe_gpi_cfg_wred_set_thr(addr_t base_va, pfe_iqos_queue_t queue, pfe_iqos_wred_thr_t thr, uint16_t val)
{
    switch (thr)
    {
        ...
        {
            const addr_t reg_offset = CSR_IQGOS_FULL_THRESH((uint32_t)queue);
            hal_write32(val, ADDR_BASE_OFFSET(base_va, reg_offset));
            break;
        }
        case PFE_IQOS_WRED_MIN_THR:
        {
            const addr_t reg_offset = CSR_IQGOS_DROP_THRESH((uint32_t)queue);
            uint32_t reg = hal_read32(ADDR_BASE_OFFSET(base_va, reg_offset));
            reg = (reg & 0xFFFFUL) | ((uint32_t)val << 16); /* update upper 16 bits */
            hal_write32(reg, ADDR_BASE_OFFSET(base_va, reg_offset));
            break;
        }
        case PFE_IQOS_WRED_MAX_THR:
        {
            const addr_t reg_offset = CSR_IQGOS_DROP_THRESH((uint32_t)queue);
            uint32_t reg = hal_read32(ADDR_BASE_OFFSET(base_va, reg_offset));
            reg = (reg & 0xFFFF0000UL) | ((uint32_t)val); /* update lower 16 bits */
            hal_write32(reg, ADDR_BASE_OFFSET(base_va, reg_offset));
            break;
        }
        default:
...
void pfe_gpi_cfg_wred_get_thr(addr_t base_va, pfe_iqos_queue_t queue, pfe_iqos_wred_thr_t thr, uint16_t *val)
{
    switch (thr)
    {
        ...
        {
            const addr_t reg_offset = CSR_IQGOS_FULL_THRESH((uint32_t)queue);
            const uint32_t reg = hal_read32(ADDR_BASE_OFFSET(base_va, reg_offset));
            *val = (uint16_t)(reg & UINT16_MAX);
            break;
        }
        case PFE_IQOS_WRED_MIN_THR:
        {
            const addr_t reg_offset = CSR_IQGOS_DROP_THRESH((uint32_t)queue);
            const uint32_t reg = hal_read32(ADDR_BASE_OFFSET(base_va, reg_offset));
            *val = (uint16_t)((reg >> 16) & UINT16_MAX); /* upper 16 bits */
            break;
        }
        case PFE_IQOS_WRED_MAX_THR:
        {
            const addr_t reg_offset = CSR_IQGOS_DROP_THRESH((uint32_t)queue);
            const uint32_t reg = hal_read32(ADDR_BASE_OFFSET(base_va, reg_offset));
            *val = (uint16_t)(reg & UINT16_MAX);         /* lower 16 bits */
            break;
        }
        default:
        {
...
{
    uint32_t reg = hal_read32(ADDR_BASE_OFFSET(base_va, CSR_IGQOS_PORT_SHP_CONFIG));
 ...
 */
uint32_t pfe_gpi_cfg_get_stat_value(addr_t base_va, uint32_t stat_id)
{
    uint32_t stat_value;
#if defined(PFE_CFG_NULL_ARG_CHECK)
    if (unlikely(NULL_ADDR == base_va))
