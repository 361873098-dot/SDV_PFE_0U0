...
pfe_phy_if_t *pfe_phy_if_create(pfe_class_t *class, pfe_ct_phy_if_id_t id, const char_t *name)
{
    ...
    (void)class;
    (void)autolibc_memset(&req, 0, sizeof(pfe_platform_rpc_pfe_phy_if_create_arg_t));

    /*  Get remote phy_if instance */
    req.phy_if_id = id;
    ret = pfe_idex_master_rpc((uint32_t)PFE_PLATFORM_RPC_PFE_PHY_IF_CREATE, &req, (uint16_t)sizeof(req), NULL_PTR, 0U);
    if (EOK != ret)
    {
...
static bool_t pfe_phy_if_has_log_if_nolock(const pfe_phy_if_t *iface, const pfe_log_if_t *log_if)
{
    ...
    (void)autolibc_memset(&arg, 0, sizeof(pfe_platform_rpc_pfe_phy_if_has_log_if_arg_t));
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        ret = pfe_idex_master_rpc((uint32_t)PFE_PLATFORM_RPC_PFE_PHY_IF_HAS_LOG_IF, &arg, (uint16_t)sizeof(arg), NULL, 0U);
        if (EOK == ret)
        {
            val = TRUE;
        }
...
errno_t pfe_phy_if_set_op_mode(pfe_phy_if_t *iface, pfe_ct_if_op_mode_t mode)
{
    ...
    (void)autolibc_memset(&arg, 0, sizeof(pfe_platform_rpc_pfe_phy_if_set_op_mode_arg_t));
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        arg.phy_if_id = iface->id;
        arg.op_mode = mode;
        ret = pfe_idex_master_rpc((uint32_t)PFE_PLATFORM_RPC_PFE_PHY_IF_SET_OP_MODE, &arg, (uint16_t)sizeof(arg), NULL, 0U);
        if (EOK != ret)
        {
