...
static errno_t ipvx_ct_check_args_clear_buf(bool_t ipv6, const fci_msg_t *msg, uint16_t *fci_ret, void *reply_buf, uint32_t *reply_len);
static errno_t ipvx_ct_register(bool_t ipv6, const fci_msg_t *msg, uint16_t *fci_ret);
static uint16_t ipvx_ct_register_get_rtable_entries(bool_t ipv6,
                                                    const fci_msg_t *msg,
                                                    pfe_rtable_entry_t **entry,
                                                    pfe_rtable_entry_t **rep_entry);
static errno_t ipvx_ct_register_add_entry(void *client, uint16_t *fci_ret, pfe_rtable_entry_t **entry, pfe_rtable_entry_t **rep_entry);
static errno_t ipvx_ct_register_add_rep_entry(uint16_t *fci_ret, pfe_rtable_entry_t **entry, pfe_rtable_entry_t **rep_entry);
static errno_t ipvx_ct_deregister(bool_t ipv6, const fci_msg_t *msg, uint16_t *fci_ret);
static errno_t ipvx_ct_update(bool_t ipv6, const fci_msg_t *msg, uint16_t *fci_ret);
...
static void ipvx_ct_compile_fci_reply(  bool_t ipv6,
                                        ...
                                        void *reply_buf,
                                        uint32_t *reply_len);
static void ipvx_ct_query(bool_t ipv6, uint16_t *fci_ret, void *reply_buf, uint32_t *reply_len);
static void ipvx_ct_query_cont(bool_t ipv6, uint16_t *fci_ret, void *reply_buf, uint32_t *reply_len);
...
static void fci_connections_ipv4_cmd_to_5t(const fpp_ct_cmd_t *ct_cmd, pfe_5_tuple_t *tuple)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        tuple->proto = (uint8_t)(oal_ntohs(ct_cmd->protocol) & UINT8_MAX);
    }
}
...
static void fci_connections_ipv6_cmd_to_5t(const fpp_ct6_cmd_t *ct6_cmd, pfe_5_tuple_t *tuple)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        tuple->proto = (uint8_t)(oal_ntohs(ct6_cmd->protocol) & UINT8_MAX);
    }
}
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        const uint32_t reply_len_required = ipv6 ? sizeof(fpp_ct6_cmd_t) : sizeof(fpp_ct_cmd_t);
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        else
        {
            ...
            *reply_len = 0U;
            ...
            if (EOK != fci_connections_set_default_timeout((uint8_t)(oal_ntohs(timeout_cmd->protocol) & UINT8_MAX), oal_ntohl(timeout_cmd->timeout_value1)))
            {
