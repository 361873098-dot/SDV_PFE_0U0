...
{
    hal_write32((uint32_t)PFE_CFG_MEMORY_PHYS_TO_PFE((addr_t)rx_ring_pa & 0xffffffffU), ADDR_BASE_OFFSET(base_va, HIF_NOCPY_RX_BDP_ADDR));
}
...
{
    hal_write32((uint32_t)PFE_CFG_MEMORY_PHYS_TO_PFE((addr_t)tx_ring_pa), ADDR_BASE_OFFSET(base_va, HIF_NOCPY_TX_BDP_ADDR));
}
...
errno_t pfe_hif_nocpy_cfg_set_emac_timer_ownership(pfe_ct_phy_if_id_t emac, bool_t value)
{
    ...
    switch(emac) 
    {
        ...
        {
            const uint8_t emac_idx = (uint8_t)emac - (uint8_t)PFE_PHY_IF_ID_EMAC0;
            if(value) 
            {
            ...
            {
                hif_nocpy_timer_ownership &= (uint8_t)(~HIF_NOCPY_TIMER_OWNERSHIP_EMAC(emac_idx) & UINT8_MAX);
            }
            ret = EOK;
        }
        break;
