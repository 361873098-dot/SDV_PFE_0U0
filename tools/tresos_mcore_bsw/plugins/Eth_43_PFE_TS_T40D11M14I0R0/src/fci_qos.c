...
static errno_t policer_flow_cmd_action_register(uint16_t *fci_ret, fpp_qos_policer_flow_cmd_t *flow_cmd, pfe_iqos_flow_spec_t *gpi_flow, pfe_gpi_t *gpi);
static errno_t policer_flow_cmd_action_deregister(uint16_t *fci_ret, fpp_qos_policer_flow_cmd_t *flow_cmd, pfe_gpi_t *gpi);
static errno_t policer_shp_cmd_action_update(uint16_t *fci_ret, fpp_qos_policer_shp_cmd_t *shp_cmd, pfe_gpi_t *gpi, uint8_t shp_id);
static errno_t policer_shp_cmd_action_query(fpp_qos_policer_shp_cmd_t *reply_buf, uint16_t *fci_ret, fpp_qos_policer_shp_cmd_t *shp_cmd, pfe_gpi_t *gpi, uint8_t shp_id);
...
static errno_t policer_wred_cmd_action_update(uint16_t *fci_ret, fpp_qos_policer_wred_cmd_t *wred_cmd, pfe_gpi_t *gpi, fpp_iqos_queue_t queue);
static errno_t policer_wred_cmd_action_query(fpp_qos_policer_wred_cmd_t *reply_buf, uint16_t *fci_ret, fpp_qos_policer_wred_cmd_t *wred_cmd, pfe_gpi_t *gpi, fpp_iqos_queue_t queue);
static errno_t qos_queue_cmd_action_update(uint16_t *fci_ret, fpp_qos_queue_cmd_t *q, const fci_t *fci);
static errno_t qos_queue_cmd_action_query(fpp_qos_queue_cmd_t *reply_buf, uint16_t *fci_ret, fpp_qos_queue_cmd_t *q, const fci_t *fci);
static errno_t queue_mode_wred(uint16_t *fci_ret, fpp_qos_queue_cmd_t *q, const fci_t *fci, pfe_phy_if_t *phy_if);
static errno_t shaper_cmd_action_update(uint16_t *fci_ret, const fci_t *fci, fpp_qos_shaper_cmd_t *shp);
static errno_t shaper_cmd_action_query(fpp_qos_shaper_cmd_t *reply_buf, uint16_t *fci_ret, const fci_t *fci, fpp_qos_shaper_cmd_t *shp);
static errno_t set_shaper_cmd_props(uint16_t *fci_ret, const fci_t *fci, fpp_qos_shaper_cmd_t *shp, pfe_phy_if_t *phy_if);
static errno_t shaper_disconnect_and_disable(uint16_t *fci_ret, const fci_t *fci, fpp_qos_shaper_cmd_t *shp, pfe_phy_if_t *phy_if);
static errno_t scheduler_cmd_action_update(uint16_t *fci_ret, const fci_t *fci, fpp_qos_scheduler_cmd_t *sch);
static errno_t scheduler_cmd_action_query(fpp_qos_scheduler_cmd_t *reply_buf, uint16_t *fci_ret, const fci_t *fci, fpp_qos_scheduler_cmd_t *sch);
static bool_t check_scheduler_input(fpp_qos_scheduler_cmd_t **sch, uint8_t ii);
...
static errno_t fci_qos_scheduler_cmd_update_set_mode(const fci_t *fci, fpp_qos_scheduler_cmd_t **sch, uint16_t **fci_ret, pfe_phy_if_t *phy_if);
static errno_t fci_qos_scheduler_cmd_update_cfg_scheduler(const fci_t *fci, fpp_qos_scheduler_cmd_t **sch, uint16_t **fci_ret, pfe_phy_if_t *phy_if);
static errno_t fci_qos_scheduler_cmd_query_get_sch(const fci_t *fci, fpp_qos_scheduler_cmd_t *sch, uint16_t **fci_ret, fpp_qos_scheduler_cmd_t **reply_buf, pfe_phy_if_t *phy_if);
...
static errno_t get_wred_zone_probs(fpp_qos_queue_cmd_t *reply_buf, uint16_t *fci_ret, fpp_qos_queue_cmd_t *q, const fci_t *fci, pfe_phy_if_t *phy_if)
{
    errno_t ret = EOK;
    uint8_t cnt, ii;
...
static bool_t check_scheduler_input(fpp_qos_scheduler_cmd_t **sch, uint8_t ii)
{
    ...
    ret_value = ((0U == (((uint32_t)1U << ii) & (*sch)->input_en)) || ((*sch)->input_src[ii] == 255U));
...
static errno_t set_shaper_cmd_props(uint16_t *fci_ret, const fci_t *fci, fpp_qos_shaper_cmd_t *shp, pfe_phy_if_t *phy_if)
{
    ...
    if(FALSE == break_switch)
    {
        ...
        else
        {
            ...
            ret = pfe_tmu_shp_set_limits(fci->tmu, pfe_phy_if_get_id(phy_if), shp->id, (int32_t)oal_ntohl(shp->max_credit), (int32_t)oal_ntohl(shp->min_credit));
            if (EOK != ret)
            {
                /* FCI command has wrong data. Respond with FCI error code. */
