...
static errno_t routes_cmd_action_register(fci_msg_t *msg, uint16_t *fci_ret, fpp_rt_cmd_t *rt_cmd, pfe_ip_addr_t *ip, pfe_mac_addr_t *dst_mac);
static errno_t routes_cmd_query_cont(uint16_t *fci_ret, fpp_rt_cmd_t *reply_buf, uint32_t *reply_len, fci_rt_db_entry_t *rt_entry);
static errno_t routes_cmd_action_deregister( uint16_t *fci_ret, fpp_rt_cmd_t *rt_cmd, fci_rt_db_t *route_db);
...
static errno_t routes_cmd_query_cont(uint16_t *fci_ret, fpp_rt_cmd_t *reply_buf, uint32_t *reply_len, fci_rt_db_entry_t *rt_entry)
{
    ...
    if(FALSE == break_switch)
    {
        ...
        *reply_len = sizeof(fpp_rt_cmd_t);
        ...
        (void)autolibc_strncpy(reply_buf->output_device, pfe_phy_if_get_name(rt_entry->iface), (uint32_t)IFNAMSIZ-1U);
