...
static errno_t pfe_feature_mgr_get_val_class_util(const char *feature_name, uint8_t **val);
static errno_t pfe_feature_mgr_set_val_class_util(pfe_fw_feature_t *fw_feature_class, pfe_fw_feature_t *fw_feature_util, const uint8_t val);
static errno_t pfe_feature_mgr_handle_feature(const pfe_feature_mgr_t *fmgr, const char *feature_name, const uint8_t val);
...
static errno_t table_set_val_null_check(const char *feature_name, const char *table_el_name, uint8_t* val);
#endif
...
#endif
    {
        ...
        if (EOK == ret)
        {
            ...
            if (EOK == ret)
            {
                if (index == 0)
                {
                    const uint32_t mem_size = pfe_fw_feature_table_entry_allocsize(fw_feature_table_entry);
                    /* mem_size is limited to U16 by pfe_fw_feature_table_entry_allocsize */
                    ret = pfe_fw_feature_table_entry_set(fw_feature_table_entry, (void *) val, (uint16_t)(mem_size & UINT16_MAX));
                }
                else
                {
...
#endif
    {
        ...
        if (EOK == ret)
        {
            ...
            if (EOK == ret)
            {
                ...
                *size = (uint8_t)(pfe_fw_feature_table_entry_size(fw_feature_table_cfg) & UINT8_MAX);
            }
        }
    }
    return ret;
...
static errno_t pfe_feature_mgr_table_cfg_get_multiplicity(const char *feature_name, const char *table_el_name, uint8_t *count)
{
...
#endif
    {
        ...
        if (EOK == ret)
        {
            ...
            if (EOK == ret)
            {
                const uint32_t multiplicity = pfe_fw_feature_table_entry_multiplicity(fw_feature_table_cfg);
                /* multiplicity is limited by pfe_fw_feature_table_entry_multiplicity to U8 */
                *count = (uint8_t)(multiplicity & UINT8_MAX);
            }
        }
    }
    return ret;
...
static errno_t pfe_feature_mgr_table_cfg_get_payload(const char *feature_name, const char *table_el_name, uint8_t *payload)
{
...
#endif
    {
        ...
        if (EOK == ret)
        {
            ...
            if (EOK == ret)
            {
                const uint32_t mem_size = pfe_fw_feature_table_entry_allocsize(fw_feature_table_cfg);
                /* mem_size is limited to U16 by pfe_fw_feature_table_entry_allocsize */
                (void)pfe_fw_feature_table_entry_get(fw_feature_table_cfg, payload, (uint16_t)(mem_size & UINT16_MAX), FALSE);
            }
        }
    }
     return ret;
...
static errno_t pfe_feature_mgr_table_stats_get_size(const char *feature_name, const char *table_el_name, uint8_t *size)
{
...
#endif
    {
        ...
        if (EOK == ret)
        {
            ...
            {
                /* pfe_fw_feature_table_entry_size output is limited to U8 */
                *size = (uint8_t)(pfe_fw_feature_table_entry_size(fw_feature_table_stats) & UINT8_MAX);                
            }
        }
    }
    return ret;
...
static errno_t pfe_feature_mgr_table_stats_get_multiplicity(const char *feature_name, const char *table_el_name, uint8_t *count)
{
...
#endif
    {
        ...
        if (EOK == ret)
        {
            ...
            if (EOK == ret)
            {
                const uint32_t multiplicity = pfe_fw_feature_table_entry_multiplicity(fw_feature_table_stats);
                /* multiplicity is limited by pfe_fw_feature_table_entry_multiplicity to U8 */
                *count = (uint8_t)(multiplicity & UINT8_MAX);
            }
        }
...
static errno_t pfe_feature_mgr_table_stats_get_payload(const char *feature_name, const char *table_el_name, uint8_t *payload)
{
...
#endif
    {
        ...
        if (EOK == ret)
        {
            ...
            if (EOK == ret)
            {
                const uint32_t mem_size = pfe_fw_feature_table_entry_allocsize(fw_feature_table_stats);
                /* mem_size is limited to U16 by pfe_fw_feature_table_entry_allocsize */
                (void)pfe_fw_feature_table_entry_get(fw_feature_table_stats, payload, (uint16_t)(mem_size & UINT16_MAX), TRUE);
            }
        }
    }
    return ret;
