...
#define WKPU_IP_SW_PATCH_VERSION_C  2
...
#if (defined(WKPU_IP_SUPPORT_PULL_CONTROL) && (WKPU_IP_SUPPORT_PULL_CONTROL == STD_ON))
/* Table of base addresses for S32G_STDBY_GPR instances. */
S32G_STDBY_GPR_Type * const Wkpu_Ip_pStdbyGpr[] = IP_S32G_STDBY_GPR_BASE_PTRS;
#endif
...
#if (defined (MCAL_GPR_REG_PROT_AVAILABLE) && (STD_ON == WKPU_IP_ENABLE_USER_MODE_SUPPORT))
    #define Call_Wkpu_Ip_SetUserAccessAllowed(BaseAddr) OsIf_Trusted_Call1param(Wkpu_Ip_SetUserAccessAllowed,(BaseAddr))
#else
    #define Call_Wkpu_Ip_SetUserAccessAllowed(BaseAddr)
#endif

#if (STD_ON == WKPU_IP_ENABLE_USER_MODE_SUPPORT)
/**
 * @brief        Enables S32G_STDBY_GPR registers writing in User Mode by configuring REG_PROT
 * @details      Sets the UAA (User Access Allowed) bit of the S32G_STDBY_GPR IP allowing S32G_STDBY_GPR registers writing in User Mode
 *
 * @param[in]    GprBaseAddr
 *
 * @return       none
 *
...
void Wkpu_Ip_SetUserAccessAllowed(uint32 GprBaseAddr)
{
    SET_USER_ACCESS_ALLOWED((uint32)GprBaseAddr, GPR_PROT_MEM_U32);
}
...
#if (defined(WKPU_IP_SUPPORT_PULL_CONTROL) && (WKPU_IP_SUPPORT_PULL_CONTROL == STD_ON))
/*!
 * @brief Wakeup/Interrupt Pull Enable
 *
 * This function enable a Pull on the corresponding interrupt pads to pull an unconnected wakeup/interrupt input to a
 * specified voltage level (See the chip-specific WKPU information for the exact level of voltage). When enabling a PULL, user must
 * ensure that the wakeup or interrupt is disabled to prevent any unintended events.
 *
 * @param[in] base The WKPU peripheral base address
 * @param[in] channelMask The channel mask
 * @param[in] enable Enables or disables pull
 */
static inline void Wkpu_Ip_PullControl(WKPU_Type * const base,
                                       uint32 channelMaskLo,
#ifdef WKPU_IP_64_CH_USED
                                       uint32 channelMaskHi,
#endif
                                       boolean enable)
{
    if (enable)
    {
        base->WIPUER_WIPDER |= channelMaskLo;
#ifdef WKPU_IP_64_CH_USED
        base->WIPUER_WIPDER_64 |= channelMaskHi;
#endif
    }
    else
    {
...
/*!
 * @brief Set pull value of wake-up pads
 *
 * This function will set the WKPU field configuration controls the pull value of wake-up pads
 *
 * @param[in] base The WKPU peripheral base address
 * @param[in] channelMask The channel mask
 * @param[in] pullValue Selects pullup or pulldown on WKUP pad
 *                      TRUE : Pull_up
 *                      FALSE: Pull_down
 */
static inline void Wkpu_Ip_SetPullValue(S32G_STDBY_GPR_Type * const base,
                                       uint32 channelMaskLo,
                                       boolean pullValue)
{
    if (pullValue)
    {
        base->WKUP_PUS |= (channelMaskLo & WKPU_IP_GPR_WKUP_PUS_MASK);
    }
    else
    {
        base->WKUP_PUS &= ~(channelMaskLo & WKPU_IP_GPR_WKUP_PUS_MASK);
...
/*!
 * @brief Set pull override of wake-up pads
 *
 * This function will override the pull value of wake-up pads
 *
 * @param[in] base The WKPU peripheral base address
 * @param[in] pullOverride Enables or disables pull override
 */
static inline void Wkpu_Ip_SetPullOverride(S32G_STDBY_GPR_Type * const base,
                                           boolean pullOverride)
{

    /* Wake-up pad pull controls will be configured via WKPU or SIUL2 */
    base->WKUP_PUS = (base->WKUP_PUS & ~S32G_STDBY_GPR_WKUP_PUS_WKUP_PU_OVERRIDE_MASK) | S32G_STDBY_GPR_WKUP_PUS_WKUP_PU_OVERRIDE((pullOverride == TRUE) ? 0x01U : 0x00U);
}
#endif

#if (defined(WKPU_IP_SUPPORT_BOOT_MODE) && (WKPU_IP_SUPPORT_BOOT_MODE == STD_ON))
/*!
 * @brief select between a short boot or a full boot
 *
 *
 * @param[in] base The WKPU peripheral base address
 * @param[in] channelMaskLo The channel mask
 * @param[in] bootMode Wakeup Short or Long Boot Select
 */
static inline void Wkpu_Ip_SetBootMode(WKPU_Type * const base,
                                          uint32 channelMaskLo,
#ifdef WKPU_IP_64_CH_USED
                                          uint32 channelMaskHi,
#endif
...
#endif /* WKPU_IP_SUPPORT_BOOT_MODE */
...
Wkpu_Ip_StatusType Wkpu_Ip_Init (uint8 instance, const Wkpu_Ip_IrqConfigType* userConfig)
{
    WKPU_Type * base;
#if (defined(WKPU_IP_SUPPORT_PULL_CONTROL) && (WKPU_IP_SUPPORT_PULL_CONTROL == STD_ON))
    S32G_STDBY_GPR_Type * baseStdbyGpr;
#endif
    ...
    base = Wkpu_Ip_pBase[instance];
#if (defined(WKPU_IP_SUPPORT_PULL_CONTROL) && (WKPU_IP_SUPPORT_PULL_CONTROL == STD_ON))
    baseStdbyGpr = Wkpu_Ip_pStdbyGpr[instance];
    /* Setup User/Supervisor mode */
    Call_Wkpu_Ip_SetUserAccessAllowed((uint32)GPR_PROT_MEM_ADDR_U32);
    /* Set pull override of wake-up pads */
    Wkpu_Ip_SetPullOverride(baseStdbyGpr, userConfig->pullOverrideEn);
#endif
...
#if (defined(WKPU_IP_SUPPORT_PULL_CONTROL) && (WKPU_IP_SUPPORT_PULL_CONTROL == STD_ON))
        if (userConfig->pullOverrideEn)
        {
            switch((*userConfig->pChannelsConfig)[index].pullValue)
            {
                case WKPU_IP_HIGH_IMPEDANCE:
                    /* Disable Wakeup/Interrupt Pull */
                    Wkpu_Ip_PullControl(base, 
                                        channelMaskLo,
        #ifdef WKPU_IP_64_CH_USED
                                        channelMaskHi, 
        #endif 
                                        FALSE);
                    break;
                case WKPU_IP_PULL_UP:
                    /* Set pull up */
                    Wkpu_Ip_SetPullValue(baseStdbyGpr, channelMaskLo, TRUE);
                    /* Enable Wakeup/Interrupt Pull */
                    Wkpu_Ip_PullControl(base, 
                                        channelMaskLo,
        #ifdef WKPU_IP_64_CH_USED
                                        channelMaskHi, 
                                        ...
                                        TRUE);
                    break;
                case WKPU_IP_PULL_DOWN:
                    /* Set pull down */
                    Wkpu_Ip_SetPullValue(baseStdbyGpr, channelMaskLo, FALSE);
                    /* Enable Wakeup/Interrupt Pull */
                    Wkpu_Ip_PullControl(base, 
                                        channelMaskLo,
        #ifdef WKPU_IP_64_CH_USED
...
#endif /* WKPU_IP_SUPPORT_PULL_CONTROL == STD_ON */

#if (defined(WKPU_IP_SUPPORT_BOOT_MODE) && (WKPU_IP_SUPPORT_BOOT_MODE == STD_ON))
        /* Set Wakeup Boot Mode Register */
        Wkpu_Ip_SetBootMode(base,
                            channelMaskLo,
#ifdef WKPU_IP_64_CH_USED
                            channelMaskHi, 
#endif 
                            (*userConfig->pChannelsConfig)[index].bootSel);
#endif
...
#if (defined(WKPU_IP_SUPPORT_PULL_CONTROL) && (WKPU_IP_SUPPORT_PULL_CONTROL == STD_ON))
    /* Disable Wake-up pull Override */
    Wkpu_Ip_pStdbyGpr[instance]->WKUP_PUS = (Wkpu_Ip_pStdbyGpr[instance]->WKUP_PUS & ~S32G_STDBY_GPR_WKUP_PUS_WKUP_PU_OVERRIDE_MASK) | S32G_STDBY_GPR_WKUP_PUS_WKUP_PU_OVERRIDE(0x00U);
#endif
...
#if (defined(WKPU_IP_SUPPORT_PULL_CONTROL) && (WKPU_IP_SUPPORT_PULL_CONTROL == STD_ON))
            /* Pull is disabled */
            Wkpu_Ip_pBase[instance]->WIPUER_WIPDER &= ~u32ChannelMask;
            /* Pull down as default*/
            Wkpu_Ip_pStdbyGpr[instance]->WKUP_PUS &= ~(u32ChannelMask & WKPU_IP_GPR_WKUP_PUS_MASK);
#endif

#if (defined(WKPU_IP_SUPPORT_BOOT_MODE) && (WKPU_IP_SUPPORT_BOOT_MODE == STD_ON))
            /* Disable boot mode*/
            Wkpu_Ip_pBase[instance]->WBMSR &= ~u32ChannelMask;
#endif
        }
#ifdef WKPU_IP_64_CH_USED
        if((uint8)32U <= index)
...
#if (defined(WKPU_IP_SUPPORT_PULL_CONTROL) && (WKPU_IP_SUPPORT_PULL_CONTROL == STD_ON))
            /* Pull is disabled */
            Wkpu_Ip_pBase[instance]->WIPUER_WIPDER_64 &= ~u32ChannelMask;
#endif

#if (defined(WKPU_IP_SUPPORT_BOOT_MODE) && (WKPU_IP_SUPPORT_BOOT_MODE == STD_ON))
            /* Disable boot mode*/
            Wkpu_Ip_pBase[instance]->WBMSR_64 &= ~u32ChannelMask;
#endif
        }
#endif 
    }
...
#endif /* STD_ON == WKPU_IP_DEINIT_API */
...
boolean Wkpu_Ip_GetInputState(uint8 instance, uint8 hwChannel)
{
    ...
    if((uint8)32U > hwChannel)
    {
        ...
        u32regWkpuWRER = base->WRER & channelMask;
        if ((0x0U != u32regWkpuWISR) && (0x0U == u32regWkpuWRER))
        {
    ...
    else
    {
        ...
        u32regWkpuWRER = base->WRER_64 & channelMask;
        if ((0x0U != u32regWkpuWISR) && (0x0U == u32regWkpuWRER))
        {
