...
#define CDD_MCL_SW_PATCH_VERSION_C                2
...
#if (STD_ON == MCL_DMA_CRC_IS_AVAILABLE)
...
void Mcl_SetDmaChannelCrcList(const uint32 Channel, const Mcl_DmaChannelCrcListType List[], const uint32 ListDimension)
{
    ...
    if (TRUE != ChannelValidation)
    {
        (void)Det_ReportError((uint16)CDD_MCL_MODULE_ID, (uint8)Channel, MCL_DET_DMA_CRC, MCL_E_INVALID_CHANNEL);
    }
#endif

    if (FALSE != ChannelValidation)
    {
        RetStatus = Dma_Ip_SetLogicChannelCrcList(Channel, (Dma_Ip_LogicChannelCrcListType const *)List, ListDimension);
...
#endif /* #if (STD_ON == MCL_DMA_CRC_IS_AVAILABLE) */
