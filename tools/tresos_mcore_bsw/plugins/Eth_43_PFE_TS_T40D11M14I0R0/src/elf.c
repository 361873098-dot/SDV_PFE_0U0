...
/* ELF32 */
    ...
    static bool_t ELF32_Load(ELF_File_t *pElfFile,uint32_t *u32NamesSectionOffset,uint32_t *u32NamesSectionSize);
        ...
        static bool_t ELF32_ProgSectLoadActual(const ELF_File_t *pElfFile, uint32_t u32ProgIdx, addr_t AccessAddr);
...
static bool_t ELF64_SectLoad(const ELF_File_t *pElfFile, uint32_t u32SectIdx, addr_t AccessAddr, addr_t AllocSize)
{
    ...
    {   /* All OK */
        ...
        {   /* Fill with zeros */
            (void)autolibc_memset((void *)AccessAddr, 0, (uint32_t)ENDIAN_SW_8B(pElfFile->arSectHead64[u32SectIdx].sh_size));
            bSuccess = TRUE;
        }
        else
        {   /* Copy from file */
