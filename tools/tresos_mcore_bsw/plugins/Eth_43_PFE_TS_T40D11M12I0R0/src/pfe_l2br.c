...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        else
        {
            ...
            if (0U != (vlan_current_entry.flags & MAC_VALID_FLAG))
            {
                ...
                {
                    pos = (uint8_t) oal_ntohs(vlan_current_entry.col_ptr);
                    pfe_vlan_read_from_class(domain->bridge, pos, &vlan_current_entry);
                ...
                if ((vlan_current_entry.flags & MAC_VALID_FLAG) != 0U)
                {
                    ...
                    if (vlan_new_entry.vlan != vlan_current_entry.vlan)
                    {
                        ...
                        if (pos < hash_size)
                        {
                            ...
                            vlan_current_entry.col_ptr = oal_htons((uint16_t)pos);
...
errno_t pfe_l2br_delete_vlan_hash_entry(pfe_l2br_domain_t *domain)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        if (domain->vlan == oal_ntohs(vlan_current_entry.vlan))
        {
            ...
            {
                next_entry_pos = (uint8_t ) oal_ntohs(vlan_current_entry.col_ptr);
...
errno_t pfe_l2br_domain_destroy(pfe_l2br_domain_t *domain)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        if (EOK == ret)
        {
            ...
            domain_subscript = isa_release(&domain->bridge->domains, domain);
            if ((domain_subscript >= 0) && (domain_subscript < (int32_t)domain->bridge->curr_domain))
            {
...
errno_t pfe_l2br_static_entry_create(pfe_l2br_t *bridge, uint16_t vlan, const pfe_mac_addr_t mac, uint32_t new_fw_list)
{
...
#ifdef PFE_CFG_L2BR_STATICS_ENABLE
    {
        ...
        for(index = 0U; index < bridge->static_entries.occupied_items_count; index++)
        {
            ...
            {
                if ((static_ent_tmp->vlan == vlan) && (0 == autolibc_memcmp(static_ent_tmp->mac, mac, sizeof(pfe_mac_addr_t))))
                {
                    match = TRUE;
                    break;
                }
        ...
        {   
            ...
            if(index_entry == count)
            {
                ...
                if (PFE_L2BR_TABLE_VLAN == entry.type)
                {
                    ...
                    stat->action_data = (uint64_t)entry.u.vlan_entry.action_data;
...
errno_t pfe_l2br_get_domain_stats(const pfe_l2br_t *bridge, pfe_ct_vlan_stats_t *stat, uint8_t vlan_index)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        for(i = 0; i < num_of_pes; i++)
        {
            ...
            ret = pfe_class_read_dmem((void *)bridge->class, (int32_t)i, &stats, bridge->domain_stats_table_addr + offset, sizeof(pfe_ct_vlan_stats_t));
            if(ret != EOK)
            {
                break;
            }
            ...
            stat->ingress += oal_ntohl(stats.ingress);
            stat->egress += oal_ntohl(stats.egress);
            stat->ingress_bytes += oal_ntohl(stats.ingress_bytes);
            stat->egress_bytes += oal_ntohl(stats.egress_bytes);
        }
