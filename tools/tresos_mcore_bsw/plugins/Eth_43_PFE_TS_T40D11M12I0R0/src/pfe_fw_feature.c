...
#define UINT_8_SIZE     (sizeof(uint8_t))
#define UINT_16_SIZE    (sizeof(uint16_t))
#define UINT_32_SIZE    (sizeof(uint32_t))
...
{
    return (uint32_t)handle.tbl_curr->size * handle.tbl_curr->multiplicity;
}
...
errno_t pfe_fw_feature_table_entry_get(pfe_fw_tbl_handle_t handle, void *mem, uint16_t size, bool_t collect)
{
    ...
    if(FALSE == collect)
    {
        ...
        if ((EOK == ret) && (pfe_fw_feature_table_entry_size(handle) > 0))
        {
            for (idx = 0; idx < (size/pfe_fw_feature_table_entry_size(handle)); idx++)
            {
                ptr = & ((uint8_t *) mem) [idx * pfe_fw_feature_table_entry_size(handle)];
                switch (pfe_fw_feature_table_entry_size(handle))
                {
                    case UINT_16_SIZE:
                        *(uint16_t *)ptr = oal_ntohs(*(uint16_t *) ptr);
                    ...
                    case UINT_32_SIZE:
                        *(uint32_t *)ptr = oal_ntohl(*(uint32_t *) ptr);
                        break;
                    default:
                        /*Do nothing - Avoid MISRA rule 16.4 */
                        break;
                }
            }
        }
...
errno_t pfe_fw_feature_table_entry_set_by_idx(pfe_fw_tbl_handle_t handle, void *val, uint16_t idx)
{
    ...
    if( idx < pfe_fw_feature_table_entry_multiplicity(handle))
    {
        ...
        ret = handle.feature->dmem_write_func(handle.feature->dmem_rw_func_data, -1, (addr_t)oal_ntohl(handle.tbl_curr->data) + (addr_t)(idx * handle.tbl_curr->size), (void *)val, handle.tbl_curr->size);
    }
...
errno_t pfe_fw_feature_table_entry_get_by_idx(pfe_fw_tbl_handle_t handle, void *mem, uint16_t idx, bool_t collect)
{
    ...
    if( idx < pfe_fw_feature_table_entry_multiplicity(handle))
    {
        ...
        {
            ret = handle.feature->dmem_read_func(handle.feature->dmem_rw_func_data, pe_idx, mem, (addr_t)oal_ntohl(handle.tbl_curr->data) + (addr_t)(idx * handle.tbl_curr->size), handle.tbl_curr->size);
