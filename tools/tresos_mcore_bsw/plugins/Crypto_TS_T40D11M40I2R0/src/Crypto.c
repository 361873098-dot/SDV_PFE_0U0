...
#define CRYPTO_SW_MAJOR_VERSION_C              4
...
(
    ...
    uint32*       pu32PublicValueLengthPtr
#else
    const uint32* pu32PublicValueLengthPtr
#endif
);
...
#endif /* (STD_ON == CRYPTO_DEV_ERROR_DETECT) */
    {
        RetVal = Crypto_Ipw_CheckKey(u32PartitionId,u32CryptoKeyIdx);
        if ((Std_ReturnType)E_OK  == RetVal)
        {
...
)
{
...
#if (STD_ON == CRYPTO_DEV_ERROR_DETECT)
     /* Check whether the call is coming from an authorized core and the driver is initialized */
    if ((Std_ReturnType)E_NOT_OK == Crypto_CheckValidCoreAndDrvInitForNonCdoApis(u32PartitionId, CRYPTO_KEYGETSTATUS_ID))
    {
        /* Nothing to do. The reporting to DET was done from inside the Crypto_CheckValidCoreAndDrvInitForNonCdoApis() function */
    }
    ...
    {
        (void)Det_ReportError((uint16)CRYPTO_MODULE_ID, (uint8)CRYPTO_INSTANCE_ID, (uint8)CRYPTO_KEYGETSTATUS_ID, (uint8)CRYPTO_E_PARAM_HANDLE);
    }
    ...
    {
        (void)Det_ReportError((uint16)CRYPTO_MODULE_ID, (uint8)CRYPTO_INSTANCE_ID, (uint8)CRYPTO_KEYGETSTATUS_ID, (uint8)CRYPTO_E_PARAM_POINTER);
    }
    else
#endif /* (STD_ON == CRYPTO_DEV_ERROR_DETECT) */
    {
        ...
        if((Std_ReturnType)CRYPTO_E_BUSY != RetVal)
        {
            /* If the key is currently not valid, the status CRYPTO_KEYSTATUS_INVALID shall be reported*/
             if (((Std_ReturnType)E_OK  != RetVal) || !(Crypto_IsKeyValidFlagSet(u32CryptoKeyIdx)))
            {
            ...
            {
                *keyStatusPtr = CRYPTO_KEYSTATUS_VALID;
            }
            RetVal = (Std_ReturnType)E_OK;
        }
        else
