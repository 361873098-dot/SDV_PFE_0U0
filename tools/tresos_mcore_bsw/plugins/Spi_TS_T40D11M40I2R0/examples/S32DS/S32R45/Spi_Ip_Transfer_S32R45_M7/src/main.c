...
*                                      LOCAL CONSTANTS
==================================================================================================*/
#define SLAVE_EXTERNAL_DEVICE            (Spi_Ip_DeviceAttributes_SpiExternalDevice_Slave_Instance_3)
#define MASTER_EXTERNAL_DEVICE           (Spi_Ip_DeviceAttributes_SpiExternalDevice_Master_Instance_0)
...
int main(void)
{
    ...
    Spi_Ip_Init(&Spi_Ip_PhyUnitConfig_SpiPhyUnit_0_Instance_0);
    Spi_Ip_Init(&Spi_Ip_PhyUnitConfig_SpiPhyUnit_1_Instance_3);

    /* Update Framesize */
    Spi_Ip_UpdateFrameSize(&SLAVE_EXTERNAL_DEVICE, 8U);
    Spi_Ip_UpdateFrameSize(&MASTER_EXTERNAL_DEVICE, 8U);

    /* Update Transfered Bit order */
    Spi_Ip_UpdateLsb(&SLAVE_EXTERNAL_DEVICE, FALSE);
    Spi_Ip_UpdateLsb(&MASTER_EXTERNAL_DEVICE, FALSE);

    /* Update Transfer Mode */
    Spi_Ip_UpdateTransferMode(SLAVE_EXTERNAL_DEVICE.Instance, SPI_IP_INTERRUPT);
    ...
    Spi_Ip_AsyncTransmit(&SLAVE_EXTERNAL_DEVICE, TxSlaveBuffer, RxSlaveBuffer, NUMBER_OF_BYTES, NULL_PTR);

    /* Master starts transfer, async method is used */
    Spi_Ip_SyncTransmit(&MASTER_EXTERNAL_DEVICE, TxMasterBuffer, RxMasterBuffer, NUMBER_OF_BYTES, TIMEOUT);

    do
    {
        Counter --;
        Status = Spi_Ip_GetStatus(SLAVE_EXTERNAL_DEVICE.Instance);
    }
