        ...
        else
        {
            ...
            */
            ...
            else
            {
                ...
                tmp_entry = pfe_log_if_from_phy_if_binding_list_entry(iface->log_ifs.prNext);

                log_if_dmem_base = 0U;
                if (EOK != pfe_log_if_get_dmem_base(tmp_entry, &log_if_dmem_base))
                {
                    NXP_LOG_ERROR("Could not get DMEM base (%s, parent: %s)\n",
                            pfe_log_if_get_name(tmp_entry), iface->name);
                    ret = ENOEXEC;
                }
                else
                {
...
static bool_t pfe_phy_if_has_enabled_log_if_nolock(const pfe_phy_if_t *iface)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        {
            entry = pfe_log_if_from_phy_if_binding_list_entry(curItem);
            if (TRUE == pfe_log_if_is_enabled(entry))
            {
                ret = TRUE;
                break;
            }
...
static bool_t pfe_phy_if_has_loopback_log_if_nolock(const pfe_phy_if_t *iface)
{
    ...
    const pfe_log_if_t *entry;
    bool_t ret = FALSE;
    bool_t islogIfEnable = FALSE;
    bool_t isLoopbackEnable = FALSE;
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        {
            entry = pfe_log_if_from_phy_if_binding_list_entry(curItem);
            islogIfEnable = pfe_log_if_is_enabled(entry);
            isLoopbackEnable = pfe_log_if_is_loopback(entry);
            if ((TRUE == islogIfEnable)
                        && (TRUE == isLoopbackEnable))
            {
                ret = TRUE;
                break;
            }
...
static errno_t pfe_phy_if_release_entry(const pfe_log_if_t *log_if, pfe_phy_if_t *iface)
{
    LLIST_t *curItem;
    errno_t ret;

    /*  Remove entry from local list */
    curItem = pfe_log_if_get_phy_if_binding_list_entry(log_if);
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (EOK != pfe_log_if_set_next_dmem_ptr(prev_entry, next_dmem_ptr))
                    {
                        NXP_LOG_ERROR("Can't set next linked list pointer (%s, parent: %s)\n",
                                pfe_log_if_get_name(prev_entry), iface->name);
                        ret = ENOEXEC;
                    ...
                    else
                    {
                        ...
                        if (0U == next_dmem_ptr)
                        {
                            NXP_LOG_INFO("Removing default logical interface (%s, parent: %s). Will be replaced by %s.\n",
                                    pfe_log_if_get_name(log_if), iface->name, pfe_log_if_get_name(prev_entry));

                            pfe_phy_if_check_next_pointer_of_deleted_entry(iface, prev_entry, &log_if_dmem_base);
                        }
                    }
                }
                ...
                if (EOK == ret)
                {
                    ...
                    else
                    {
                        ...
                        ret = pfe_phy_if_release_entry(log_if, iface);
...
static void pfe_phy_if_update_op_mode_nolock(pfe_phy_if_t *iface, pfe_ct_if_op_mode_t mode)
{
    ...
    if_bitmap = (uint32_t)(1U << (uint8_t)iface->id);
    if ((IF_OP_VLAN_BRIDGE == mode) || (IF_OP_L2L3_VLAN_BRIDGE == mode))
    {
        br_mode = TRUE;
    }
                ...
                else
                {
                    if (FALSE == pfe_emac_is_broad(addr))
                    {
                        ...
                        if (EOK != ret)
                        {
                            ...
                            temp_ret = pfe_mac_db_add_addr(&iface->mac_db, addr, owner);
...
errno_t pfe_phy_if_get_stats(pfe_phy_if_t *iface, pfe_ct_phy_if_stats_t *stat)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        ret = pfe_class_gather_read_dmem(iface->class, &pfe_phy_if_stats, (iface->dmem_base + offset), sizeof(pfe_phy_if_stats), sizeof(pfe_ct_phy_if_stats_t));
        ...
        {
            /* Store statistics */
            stat->discarded += oal_ntohl(pfe_phy_if_stats[i].discarded);
            stat->egress    += oal_ntohl(pfe_phy_if_stats[i].egress);
            stat->ingress   += oal_ntohl(pfe_phy_if_stats[i].ingress);
            stat->malformed += oal_ntohl(pfe_phy_if_stats[i].malformed);
        }
