...
typedef struct
{
    const Spi_Ip_ConfigType *SpiIpConfig;
#ifdef LPSPI_IP_LLCE_SUPPORT
#if (STD_ON == LPSPI_IP_LLCE_SUPPORT)
    ...
    Spi_Ip_FastTransferType *SpiFastTransferCfg; /* config structure for a sequence transfer with Dma Fast */
#endif
#endif
...
typedef struct
{
    const Spi_Ip_ExternalDeviceType *SpiExternalDeviceConfig;
#ifdef LPSPI_IP_LLCE_SUPPORT
#if (STD_ON == LPSPI_IP_LLCE_SUPPORT)
