 ...
 * FILE VERSION CHECKS
 */
/* Check if ipc-hw-s32gen1.c file and ipc-shm.h file are of the same vendor */
#if (IPC_HW_PLATFORM_VENDOR_ID_C != IPC_SHM_VENDOR_ID)
	#error "ipc-hw-s32gen1.c and ipc-shm.h have different vendor IDs"
#endif
/* Check if ipc-hw-s32gen1.c file and ipc-shm.h file are of the same Autosar version */
	...
	#error "AutoSar Version Numbers of ipc-hw-s32gen1.c and ipc-shm.h are different"
#endif
/* Check if ipc-hw-s32gen1.c file and ipc-shm.h file are of the same software version */
	...
	#error "Software Version Numbers of ipc-hw-s32gen1.c and ipc-shm.h are different"
#endif

/* Check if ipc-hw-s32gen1.c file and ipc-hw.h file are of the same vendor */
#if (IPC_HW_PLATFORM_VENDOR_ID_C != IPC_HW_VENDOR_ID)
	#error "ipc-hw-s32gen1.c and ipc-hw.h have different vendor IDs"
#endif
/* Check if ipc-hw-s32gen1.c file and ipc-hw.h file are of the same Autosar version */
	...
	#error "AutoSar Version Numbers of ipc-hw-s32gen1.c and ipc-hw.h are different"
#endif
/* Check if ipc-hw-s32gen1.c file and ipc-hw.h file are of the same software version */
	...
	#error "Software Version Numbers of ipc-hw-s32gen1.c and ipc-hw.h are different"
#endif

/* Check if ipc-hw-s32gen1.c file and ipc-hw-platform.h file are of the same vendor */
#if (IPC_HW_PLATFORM_VENDOR_ID_C != IPC_HW_PLATFORM_VENDOR_ID)
	#error "ipc-hw-s32gen1.c and ipc-hw-platform.h have different vendor IDs"
#endif
/* Check if ipc-hw-s32gen1.c file and ipc-hw-platform.h file are of the same Autosar version */
	...
	#error "AutoSar Version Numbers of ipc-hw-s32gen1.c and ipc-hw-platform.h are different"
#endif
/* Check if ipc-hw-s32gen1.c file and ipc-hw-platform.h file are of the same software version */
	...
	#error "Software Version Numbers of ipc-hw-s32gen1.c and ipc-hw-platform.h are different"
#endif
...
/* Validate and get core index if core type is m7 */
...
/* Validate and get core index if core type is a53 */
						...
						const struct ipc_shm_cfg *cfg)
{
	...
	if (cfg->remote_core.type == IPC_CORE_A53) {
		...
		if ((cfg->remote_core.index >= IPC_CORE_INDEX_0)
				&& (cfg->remote_core.index <= IPC_CORE_INDEX_2)) {
						...
						const struct ipc_shm_cfg *cfg)
{
	...
	if (cfg->local_core.type == IPC_CORE_A53) {
		...
		if ((cfg->local_core.index >= IPC_CORE_INDEX_0)
				&& (cfg->local_core.index <= IPC_CORE_INDEX_2)) {
						...
						const struct ipc_shm_cfg *cfg)
{
	...
	if ((cfg->inter_core_tx_irq == cfg->inter_core_rx_irq)
			&& (cfg->inter_core_tx_irq != IPC_IRQ_NONE)) {
		err = -IPC_SHM_E_INVAL;
	} else {
		err = IPC_SHM_E_OK;
		if ((cfg->inter_core_tx_irq == IPC_IRQ_NONE)
				|| ((cfg->inter_core_tx_irq >= (sint16)INT0_IRQn)
					&& (cfg->inter_core_tx_irq <= (sint16)INT2_IRQn))) {
			ipc_hw_priv[instance].mscm_tx_irq = cfg->inter_core_tx_irq;
			ipc_hw_priv[instance].msi_tx_irq
				= (uint8)cfg->inter_core_tx_irq - 1u;
		} else {
			err = -IPC_SHM_E_INVAL;
		}
		if ((cfg->inter_core_rx_irq == IPC_IRQ_NONE)
				|| ((cfg->inter_core_rx_irq >= (sint16)INT0_IRQn)
					&& (cfg->inter_core_rx_irq <= (sint16)INT2_IRQn))) {
			ipc_hw_priv[instance].mscm_rx_irq = cfg->inter_core_rx_irq;
			ipc_hw_priv[instance].msi_rx_irq
				= (uint8)cfg->inter_core_rx_irq - 1u;
		} else {
			err = -IPC_SHM_E_INVAL;
		}
	}
...
/* generic cache flush */
static void ipc_hw_flush_cache(uintptr data_addr, uintptr data_size)
{
