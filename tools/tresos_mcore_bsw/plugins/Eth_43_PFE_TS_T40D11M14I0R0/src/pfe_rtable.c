...
#define ADDU32_WRAP(X,Y)       (uint32_t)(((uint64_t)(X)+(Y)) & UINT32_MAX)
...
static const pfe_isa_definition_t pfe_rtable_entries_isa_def =
{
    ...
    .item_size = (uint32_t)sizeof(pfe_rtable_entry_t),
...
static pfe_rtable_t *pfe_rtable_configure_table(pfe_class_t *class, pfe_rtable_t *rtable, uint32_t pool_size);
...
static uint32_t pfe_rtable_entry_get_hash(pfe_rtable_entry_t *entry, pfe_ipv_type_t ipv_type, uint32_t hash_mask)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        if (IPV4 == ipv_type)
        {
            ...
            sport = entry->phys_entry->ipv.v4.sip ^ ((uint32_t)(entry->phys_entry->sport) << 16);
            ...
            temp = ADDU32_WRAP(temp, (uint32_t)oal_ntohs(entry->phys_entry->dport));
            ret = (temp & hash_mask);

        }
        else if (IPV6 == ipv_type)
        {
            uint32_t crc_ipv6 = 0U;
            int32_t jj;
...
void pfe_rtable_entry_set_out_inner_vlan(pfe_rtable_entry_t *entry, uint16_t vlan)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        if (NULL != entry->rtable)
        {
            oal_mutex_lock(PFE_RTABLE_LOCK_MUTEX_08);
        }
        entry->phys_entry->actions |= oal_htonl(RT_ACT_ADD_VLAN1_HDR);
        if (NULL != entry->rtable)
        {
            oal_mutex_unlock(PFE_RTABLE_LOCK_MUTEX_08);
        }
    }
}
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (NULL != entry->rtable)
                {
                    oal_mutex_lock(PFE_RTABLE_LOCK_MUTEX_09);
                }
                entry->phys_entry->actions |= oal_htonl(RT_ACT_ADD_PPPOE_HDR);
                if (NULL != entry->rtable)
                {
                    oal_mutex_unlock(PFE_RTABLE_LOCK_MUTEX_09);
                }
            }
        }
    }
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ret = (uint8_t)(oal_ntohs(entry->phys_entry->conntrack_stats_index) & UINT8_MAX);
    }
    return ret;
}
...
static errno_t pfe_rtable_add_entry_id(pfe_rtable_t *rtable, pfe_rtable_entry_t *entry, pfe_rtable_phys_entry_infor_t *phys_entry_temp)
{
    ...
    index = (uint16_t)pfe_rtable_get_free_stats_index(rtable);
...
static errno_t pfe_rtable_del_entry_nolock_htable(pfe_rtable_t *rtable, pfe_rtable_entry_t *entry)
{
    ...
    {
        (void)pfe_rtable_clear_stats(rtable, (uint8_t)(oal_ntohs(entry->temp_phys_entry.conntrack_stats_index) & UINT8_MAX));
        pfe_rtable_free_stats_index((uint8_t)(oal_ntohs(entry->temp_phys_entry.conntrack_stats_index) & UINT8_MAX));
    }
...
static uint32_t pfe_rtable_create_stats_table(pfe_class_t *class, uint16_t conntrack_count)
{
    ...
    size = (uint32_t)(((uint64_t)conntrack_count * sizeof(pfe_ct_conntrack_stats_t)) & UINT32_MAX);
    /* Allocate DMEM */
...
pfe_rtable_t *pfe_rtable_create(pfe_class_t *class, pfe_l2br_t *bridge, pfe_class_table_sizes_t table_params)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        rtable->htable_base_pa = (addr_t)oal_mm_virt_to_phys_contig((void *)table_params.htable_base_va);
        rtable->htable_size = table_params.htable_size;
        PfeDevAssert(rtable->htable_size != 0U);
        rtable->htable_end_va = ADDR_BASE_OFFSET(rtable->htable_base_va, ((uint64_t)rtable->htable_size * sizeof(pfe_ct_rtable_entry_t)) - 1U);
        rtable->htable_end_pa = ADDR_BASE_OFFSET(rtable->htable_base_pa, ((uint64_t)rtable->htable_size * sizeof(pfe_ct_rtable_entry_t)) - 1U);
        ...
        rtable->pool_end_va = ADDR_BASE_OFFSET(rtable->pool_base_va, ((uint64_t)rtable->pool_size * sizeof(pfe_ct_rtable_entry_t)) - 1U);
        rtable->pool_end_pa = ADDR_BASE_OFFSET(rtable->pool_base_pa, ((uint64_t)rtable->pool_size * sizeof(pfe_ct_rtable_entry_t)) - 1U);
...
pfe_rtable_entry_t *pfe_rtable_get_first(pfe_rtable_t *rtable, pfe_rtable_get_criterion_t crit, void *arg)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        switch (rtable->cur_crit)
        {
            ...
            case RTABLE_CRIT_BY_ROUTE_ID:
                (void)autolibc_memcpy(&rtable->cur_crit_arg.route_id, arg, sizeof(uint32_t));
                break;

            case RTABLE_CRIT_BY_ID5T:
                (void)autolibc_memcpy(&rtable->cur_crit_arg.id5t, arg, sizeof(uint32_t));
                break;
...
errno_t pfe_rtable_get_stats(const pfe_rtable_t *rtable, pfe_ct_conntrack_stats_t *stat, uint8_t conntrack_index)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        while(i < pfe_class_get_num_of_pes(rtable->class))
        {
            ...
            ret = pfe_class_read_dmem((void *)rtable->class, (int32_t)i, &stats, ADDR_BASE_OFFSET(rtable->conntrack_stats_table_addr, offset), sizeof(pfe_ct_conntrack_stats_t));
            if (EOK != ret)
            {
                break;
            }
