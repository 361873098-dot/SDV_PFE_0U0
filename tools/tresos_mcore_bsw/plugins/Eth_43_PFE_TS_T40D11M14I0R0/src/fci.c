...
static errno_t execute_fci_cmd(fci_msg_t *msg, uint16_t *fci_ret, uint32_t **reply_buf_ptr, uint32_t **reply_buf_len_ptr);
#ifdef  PFE_CFG_MULTI_INSTANCE_SUPPORT
static errno_t fci_authorize_and_lock( uint32_t **reply_buf_len_ptr, bool_t *floating_lock, uint16_t *fci_ret, bool_t *fci_cmd_execute, pfe_ct_phy_if_id_t sender_phy_if_id);
static errno_t fci_check_credentials(fci_vars_t *vars, uint16_t *fci_ret, uint32_t **reply_buf_len_ptr, uint16_t **reply_retval_ptr);
#endif /* PFE_CFG_MULTI_INSTANCE_SUPPORT */
...
static errno_t fci_check_credentials(fci_vars_t *vars, uint16_t *fci_ret, uint32_t **reply_buf_len_ptr, uint16_t **reply_retval_ptr)
{
    ...
    if (FALSE == fci_cmd_execute)
    {
        ...
        *reply_retval_ptr = (uint16_t *)vars->p_rep_msg->msg_cmd.payload;
...
errno_t fci_process_ipc_message(fci_msg_t *msg, fci_msg_t *rep_msg)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        ret = pfe_idex_master_rpc((uint32_t)PFE_PLATFORM_RPC_PFE_FCI_PROXY, &proxy_cmd, (uint16_t)sizeof(proxy_cmd), &proxy_rep, (uint16_t)sizeof(proxy_rep));
