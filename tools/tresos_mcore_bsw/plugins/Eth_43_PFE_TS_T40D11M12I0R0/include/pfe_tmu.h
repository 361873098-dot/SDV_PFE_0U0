...
typedef struct
{
    uint32_t level;         /* Fill level*/
    uint32_t drops;         /*Number of droped packets*/
    uint32_t tx;            /*Transmitted packets*/
    uint32_t mode;          /*< Queue mode:
                            0 == Disabled. Queue will drop all packets.
                            1 == Default. HW implementation-specific. Normally not used.
                            2 == Tail drop
                            3 == WRED */

    uint32_t min;       /*< Minimum threshold. Value is specific for mode:
    ...
    uint32_t max;       /*< Maximum threshold. Value is specific for mode:
                            - Disabled, Default: n/a
