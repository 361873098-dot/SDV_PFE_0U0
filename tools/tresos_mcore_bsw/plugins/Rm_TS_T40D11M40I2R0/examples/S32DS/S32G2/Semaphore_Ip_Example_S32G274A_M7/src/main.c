...
#define SEMA42_INSTANCE                     (0UL)
#define SEMA42_GATE                         (1U)
/*==================================================================================================
*                                      LOCAL CONSTANTS
==================================================================================================*/
...
int main(void) 
{
    ...
    Sema42_Ip_LockGate(SEMA42_INSTANCE,SEMA42_GATE,XRDC_DOMAIN0);
    ...
    Sema42_Ip_UnlockGate(SEMA42_INSTANCE,SEMA42_GATE);
...
void SyncDataFromCache(uint32 Address, uint32 Length)
{
#ifdef CACHE_IP_IS_AVAILABLE
    #if (STD_ON == CACHE_IP_IS_AVAILABLE)
    Cache_Ip_InvalidateByAddr(CACHE_IP_DATA, (uint32)Address, (uint32)Length);
...
void DMA_StartTransfer(uint8 DMA_Channel, uint32 Src_Addr, uint32 Dest_Addr)
{
    ...
    Dma_Ip_SetLogicChannelCommand(DMA_Channel, DMA_IP_CH_CLEAR_ERROR);

    /* DMA Logic Address Configuration */
    Dma_Ip_LogicChannelTransferListType DmaCh0_TransferList[DMA_CH0_CONFIG_LIST_DIMENSION] =
    {
        {DMA_IP_CH_SET_SOURCE_ADDRESS, (uint32)Src_Addr},
        {DMA_IP_CH_SET_DESTINATION_ADDRESS, (uint32)Dest_Addr},
    };

    /* Set Logic Channel 0 -> Source and Destination Address */
    Dma_Ip_SetLogicChannelTransferList(DMA_Channel, DmaCh0_TransferList, DMA_CH0_CONFIG_LIST_DIMENSION);

    /* Start transfer for logic Channel 0 */
    Dma_Ip_SetLogicChannelCommand(DMA_Channel, DMA_IP_CH_SET_SOFTWARE_REQUEST);
}
...
boolean DMA_WaitCptTransfer(uint8 DMA_Channel)
{
    Dma_Ip_LogicChannelStatusType DmaTransferStatus;
    uint32 TestTime = 0U;
    ...
    do
    {
        Dma_Ip_GetLogicChannelStatus(DMA_Channel, &DmaTransferStatus);
        TestTime++;
