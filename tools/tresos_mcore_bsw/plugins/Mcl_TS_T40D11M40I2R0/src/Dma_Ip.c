...
#define DMA_IP_SW_PATCH_VERSION_C                2
...
static Dma_Ip_ReturnType Static_Dma_Ip_LogicChannelCrcInit(const uint32 LocHwVers,const uint32 LocHwInst, const uint32 LocHwCh, const uint32 LogicChId, const Dma_Ip_CrcConfigType * pxCrcConfig);
#endif
...
static Dma_Ip_ReturnType Static_Dma_Ip_SetLogicChannelCrcConfig(const uint32 LogicCh, const Dma_Ip_CrcConfigType * const CrcConfig)
{
    ...
    LocList[3U].Value = (CrcConfig->EnInitSel == TRUE) ? 1U : 0U;
    LocList[4U].Param = DMA_IP_CH_SET_CRC_EN_LOGIC;
    LocList[4U].Value = (CrcConfig->EnLogic == TRUE) ? 1U : 0U;

    /* Load values to CRC channels registers */
...
Dma_Ip_ReturnType Dma_Ip_Init(const Dma_Ip_InitType * const DmaInit)
{
...
#if (STD_ON == DMA_CRC_IP_REG_PROT_AVAILABLE)
    uint32 LocDmaCrcHwInstAddr = 0U;
#endif /* DMA_CRC_IP_REG_PROT_AVAILABLE */
#endif /* DMA_IP_REG_PROT_AVAILABLE */
#endif /* (STD_ON == DMA_IP_USER_MODE_SUPPORT_IS_AVAILABLE) */
#endif /* MCAL_ENABLE_USER_MODE_SUPPORT */
    ...
    if(DMA_IP_STATUS_SUCCESS == LocStatus)
    {
        ...
        #if (STD_ON == DMA_IP_REG_PROT_AVAILABLE)
        #if (STD_ON == DMA_CRC_IP_REG_PROT_AVAILABLE )
            LocDmaCrcHwInstAddr = (uint32)DMA_CRC_IP_GEC_BASE_ADDR((uint32)(Dma_Ip_pxInit->ppxLogicInstConfigArray[LocCfgIdx]->LogicInstId.HwInstId));
            OsIf_Trusted_Call1param(Mcl_Dma_SetUserAccessAllowed, LocDmaCrcHwInstAddr);
        #endif /* DMA_IP_MP_REG_PROT_AVAILABLE  */
        #endif /* DMA_IP_REG_PROT_AVAILABLE */
        #endif /* (STD_ON == DMA_IP_USER_MODE_SUPPORT_IS_AVAILABLE) */
        #endif /* MCAL_ENABLE_USER_MODE_SUPPORT */
            HwAccDmaInst_SetCrcConfig(&Dma_Ip_pxInit->ppxLogicInstConfigArray[LocCfgIdx]->LogicInstId, (const Dma_Ip_LogicInstanceConfigType*)Dma_Ip_pxInit->ppxLogicInstConfigArray[LocCfgIdx]);
        ...
        for(LocCfgIdx = 0U; LocCfgIdx < DMA_IP_NOF_CFG_LOGIC_CHANNELS; LocCfgIdx++)
        {
            ...
            pLocChState = Dma_Ip_InitHwChannelStatePointer(LocHwVers, LocHwInst, LocHwCh, Dma_Ip_pxInit->ppxChStateArray[LocCfgIdx]);
...
Dma_Ip_ReturnType Dma_Ip_Deinit(void)
{
    ...
    #if (STD_ON == DMA_IP_DMACRC_IS_AVAILABLE)
            HwAccDmaInst_SetCrcConfig(&(Dma_Ip_pxInit->ppxLogicInstConfigArray[LocCfgIdx]->LogicInstId), (const Dma_Ip_LogicInstanceConfigType*)Dma_Ip_pxInit->pxLogicInstanceResetConfig);
    #endif
        }
        /* Exit exclusive area to protect DMA_MP_CSR and DMA_CRC_GEC*/
        ...
        if(NULL_PTR != LocChState)
        {
            ...
            else
            {
                ...
                if(DMA_IP_STATUS_SUCCESS == LocStatus)
                {
                    ...
                    {
                        LocStatus = Static_Dma_Ip_LogicChannelCrcInit(LocHwVers, LocHwInst, LocHwCh, LocLogicChId->LogicChId, Dma_Ip_pxInit->ppxLogicChannelConfigArray[LogicCh]->pxCrcConfig);
                    }
                    else
                    {
                        /* Do nothing */
...
#if (STD_ON == DMA_IP_DMACRC_IS_AVAILABLE)
Dma_Ip_ReturnType Dma_Ip_SetLogicChannelCrcList(const uint32 LogicCh, const Dma_Ip_LogicChannelCrcListType List[], const uint32 ListDimension)
{
    Dma_Ip_ReturnType LocStatus = DMA_IP_STATUS_SUCCESS;
    const Dma_Ip_LogicChannelIdType * LocLogicChId;
    Dma_Ip_HwChannelStateType * LocChState;
...
static void Static_Dma_Ip_SetLogicChannelCrcList(const uint32 LocHwVers, const uint32 LogicCh, const Dma_Ip_LogicChannelCrcListType List[], const uint32 ListDimension)
{
    ...
    for(LocParamIdx = 0U; LocParamIdx < ListDimension; LocParamIdx++)
    {
        ...
        HwAccDmaCh_SetCrcParam((uint32)List[LocParamIdx].Param, LocHwVers, LocHwInst, LocHwCrcChId, List[LocParamIdx].Value);
    }
}
...
static Dma_Ip_ReturnType Static_Dma_Ip_LogicChannelCrcInit(const uint32 LocHwVers,const uint32 LocHwInst, const uint32 LocHwCh, const uint32 LogicChId, const Dma_Ip_CrcConfigType * pxCrcConfig)
{
    ...
    uint8 LocHwCrcChId = pxCrcConfig->HwCrcChId;
    ...
    LocStatus = Static_Dma_Ip_SetLogicChannelCrcConfig(LogicChId, pxCrcConfig);
    return LocStatus;
}
#endif /* STD_ON == DMA_IP_DMACRC_IS_AVAILABLE */
...
Dma_Ip_ReturnType Dma_Ip_GetLogicChannelParam(const uint32 LogicCh, const Dma_Ip_LogicChannelInfoParamType Param, uint32 * const Value)
{
    ...
    if(LocStatus == DMA_IP_STATUS_SUCCESS)
    {
        ...
        LocLogicChId = (const Dma_Ip_LogicChannelIdType *)&(Dma_Ip_pxInit->ppxLogicChannelConfigArray[LogicCh]->LogicChId);
    #if (STD_ON == DMA_IP_DMACRC_IS_AVAILABLE)
        pxCrcConfig = (Dma_Ip_pxInit->ppxLogicChannelConfigArray[LogicCh]->pxCrcConfig);
    #endif
        LocHwVers = LocLogicChId->HwVersId;
        LocHwInst = LocLogicChId->HwInstId;
    #if (STD_ON == DMA_IP_DMACRC_IS_AVAILABLE)
