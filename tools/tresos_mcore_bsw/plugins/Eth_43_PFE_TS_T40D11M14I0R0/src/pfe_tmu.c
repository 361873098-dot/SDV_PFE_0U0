...
typedef struct
{
    ...
    uint32_t min;               /*Min threshold (number of packets)*/
    uint32_t max;               /*Max threshold (number of packets)*/
    uint16_t sum;               /*Sum of queue lengths*/
...
static errno_t pfe_tmu_set_queue_mode(const pfe_tmu_t *tmu, pfe_ct_phy_if_id_t phy, uint8_t queue, pfe_tmu_queue_params parameters);
 ...
 */
static bool_t is_hif_by_id(pfe_ct_phy_if_id_t id, uint8_t *err051211_hif_idx)
{
    bool_t ret = FALSE;
    uint8_t hif_idx;
...
static errno_t pfe_tmu_set_queue_mode(const pfe_tmu_t *tmu, pfe_ct_phy_if_id_t phy, uint8_t queue, pfe_tmu_queue_params parameters )
{
    ...
    if (EOK == ret_val)
    {
        switch (parameters.mode)
        {
            case TMU_Q_MODE_TAIL_DROP:
            {
                ret_val = pfe_tmu_q_mode_set_tail_drop(tmu->cbus_base_va, phy, queue, (uint16_t)(parameters.max & UINT16_MAX));
                break;
            }

            case TMU_Q_MODE_WRED:
            {
                ret_val = pfe_tmu_q_mode_set_wred(tmu->cbus_base_va, phy, queue, (uint16_t)(parameters.min & UINT16_MAX), (uint16_t)(parameters.max & UINT16_MAX));
                break;
            }
...
errno_t pfe_tmu_queue_err051211_sync(const pfe_tmu_t *tmu)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        for (uint8_t phy_idx = 0u; phy_idx < sizeof(phys)/sizeof(phys[0]); phy_idx++)
        {
 ...
 */
uint32_t pfe_tmu_get_stat_value(const pfe_tmu_t* tmu, uint32_t stat_id)
{
    uint32_t stat_value;
#if defined(PFE_CFG_NULL_ARG_CHECK)
 ...
 */
errno_t pfe_tmu_get_queue_stats(const pfe_tmu_t* tmu, uint32_t phy_id, uint32_t queue_id, pfe_tmu_queue_stats* queue_stats)
{
    errno_t ret;
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        if(EOK == pfe_tmu_cfg_get_queue_stats(tmu->cbus_base_va, phy_id, queue_id, queue_stats))
        {
            ret = EOK;
        }
        else
