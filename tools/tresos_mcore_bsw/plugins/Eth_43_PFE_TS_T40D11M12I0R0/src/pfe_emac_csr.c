...
static inline void pfe_emac_cfg_clear_registers(addr_t base_va)
{
    ...
    hal_write32(MAC_ADDRESS0_HIGH_RESET_VAL, base_va + MAC_ADDRESS_HIGH(0U));
    hal_write32(MAC_ADDRESSX_LOW_RESET_VAL, base_va + MAC_ADDRESS_LOW(0U));
    for (uint8_t slot = 1U; slot < EMAC_CFG_INDIVIDUAL_ADDR_SLOTS_COUNT; slot++)
    {
        hal_write32(MAC_ADDRESSX_HIGH_RESET_VAL, base_va + MAC_ADDRESS_HIGH(slot));
        hal_write32(MAC_ADDRESSX_LOW_RESET_VAL, base_va + MAC_ADDRESS_LOW(slot));
    }

    /* Clear hash table */
    hal_write32(0U, base_va + MAC_HASH_TABLE_REG0);
    hal_write32(0U, base_va + MAC_HASH_TABLE_REG1);
...
void pfe_emac_cfg_get_ts_time(addr_t base_va, uint32_t *sec, uint32_t *nsec, uint16_t *sec_hi)
{
    ...
    do
    {
        ...
        *sec_hi = (uint16_t)hal_read32(base_va + MAC_STS_HIGHER_WORD);
        sec_tmp = hal_read32(base_va + MAC_SYSTEM_TIME_SECONDS);
    } while (*sec != sec_tmp);
}
...
{
    uint32_t top = hal_read32(base_va + MAC_ADDRESS_HIGH(slot));
    uint32_t bottom = hal_read32(base_va + MAC_ADDRESS_LOW(slot));
    
    /* Check whether the address slot is in use */
    if ((top & 0x80000000U) != 0U)
    {
        /* Address slot is in use */
        addr[0] = (uint8_t) bottom;
        addr[1] = (uint8_t) (bottom >> 8U);
        addr[2] = (uint8_t) (bottom >> 16U);
        addr[3] = (uint8_t) (bottom >> 24U);
        addr[4] = (uint8_t) top;
        addr[5] = (uint8_t) (top >> 8U);
    }
    else
    {
...
{
    uint32_t regval = hal_read32(base_va + MAC_PPS_CONTROL);
    ...
    hal_write32(regval, base_va + MAC_PPS_CONTROL);
}
...
{
    hal_write32(nanoseconds, base_va + MAC_PPS0_TARGET_TIME_NANOSECONDS);
    hal_write32(seconds, base_va + MAC_PPS0_TARGET_TIME_SECONDS);
}
...
{
    hal_write32(period, base_va + MAC_PPS0_INTERVAL);
}
...
{
    hal_write32(pulse_width, base_va + MAC_PPS0_WIDTH);
}
