		...
		uint32 size)
{
	...
	/* notify demo task to send an echo reply back */
	SetEvent(task_shm_demo, event_ping);
}
...
/**
 * task_shm_demo() - Autosar shared memory demo task
 *
...
TASK(task_shm_demo)
{
	...
	/* application: echo reply for every message received using IRQ notify */
	while (1) {
		/* wait for incoming message */
		WaitEvent(event_ping);
		ClearEvent(event_ping);
		...
		err = ipc_shm_tx(IPCF_INSTANCE, app.last_rx_ch, buf, ipc_strlen(buf));
		if (err) {
			handle_error(err);
		}
...
/**
 * ErrorHook_SHM_DEMO_APP() - callback for handling Autosar OS errors
 * @error: OS error code
 */
void ErrorHook_SHM_DEMO_APP(StatusType error)
{
	(void)error; /* silence compiler warning */

	while (1) {
		/* block indefinitely in case of error */
	}
}

/**
 * ErrorHook() - callback for handling Autosar OS errors
 * @error: OS error code
 */
