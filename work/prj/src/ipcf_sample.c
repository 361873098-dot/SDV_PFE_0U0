...
#define SAMPLE_TASK_STACK_SIZE 256 /* 4-byte words */
...
#define WAIT_TIMEOUT 1000/portTICK_PERIOD_MS
...
/* FreeRTOS variable used for synchronization */
static SemaphoreHandle_t shm_event_rx;

#if (configSUPPORT_STATIC_ALLOCATION == 1)
static StackType_t xStack[SAMPLE_TASK_STACK_SIZE];
		...
		uint32 size)
{
	...
	/* notify demo task it can resume sending another message */
...
void ctrl_chan_rx_cb(void *arg, const uint8 instance, uint8 chan_id, void *mem)
{
	...
	if (cb_arg_sample != &app || ipc_strlen(mem) > MAX_SAMPLE_MSG_LEN ||
		chan_id != CTRL_CHAN_ID || IPCF_INSTANCE0 != instance) {
		handle_error(-IPC_SHM_E_INVAL);
	}
...
void shm_demo(void *params)
{
	...
	while (1) {
		...
		if (xSemaphoreTake(shm_event_rx, WAIT_TIMEOUT)) {
			/* get buffer */
			buf = ipc_shm_acquire_buf(IPCF_INSTANCE0, app.last_rx_ch,
					MAX_SAMPLE_MSG_LEN);
			if (!buf)
				handle_error(-IPC_SHM_E_NOMEM);
			...
			err = ipc_shm_tx(IPCF_INSTANCE0, app.last_rx_ch, buf,
					ipc_strlen(buf));
			if (err) {
				handle_error(err);
			}
			...
			if (app.rx_count == app.expected_msgs) {
				reply_ctrl_msg(IPCF_INSTANCE0, CTRL_CHAN_ID);
			}
		}
	}
...
/**
 * prepare_demo() - prepare sample execution
 *
 * Apply demo prerequisites
 */
void prepare_demo(void)
{
	...
	os_status = xTaskCreate((TaskFunction_t) shm_demo,
					"sample",
					SAMPLE_TASK_STACK_SIZE,
					NULL,
					tskIDLE_PRIORITY + 1,
					NULL);
	if (os_status != pdPASS)
		handle_error(os_status);
#endif
#if (configSUPPORT_STATIC_ALLOCATION == 1)
	/* Initialize the semaphore for shm_event_rx */
	shm_event_rx = xSemaphoreCreateCountingStatic(0xFFu, 0, &xSemaphoreBuffer);

	/* create the demo task */
	xTaskCreateStatic((TaskFunction_t)shm_demo,
					"sample",
					SAMPLE_TASK_STACK_SIZE,
					NULL,
					tskIDLE_PRIORITY + 1,
