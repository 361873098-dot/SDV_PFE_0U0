...
static int32_t fci_command(FCI_CLIENT *client, uint16_t fcode, const uint16_t *cmd_buf, uint16_t cmd_len, uint16_t *rep_buf, uint16_t *rep_len);
...
int fci_cmd(FCI_CLIENT *client, unsigned short fcode, unsigned short *cmd_buf, unsigned short cmd_len, unsigned short *rep_buf, unsigned short *rep_len)
{
    ...
    return fci_command(client, fcode, cmd_buf, cmd_len, rep_buf, rep_len);
}
...
int fci_write(FCI_CLIENT *client, unsigned short fcode, unsigned short cmd_len, unsigned short *cmd_buf)
{
    ...
    return fci_command(client, fcode, cmd_buf, cmd_len, NULL, NULL);
}
...
static int32_t fci_command(FCI_CLIENT *client, uint16_t fcode, const uint16_t *cmd_buf, uint16_t cmd_len, uint16_t *rep_buf, uint16_t *rep_len)
{
    ...
    else
    {
        ...
        if (cmd_len > 0U)
        {
            ...
            (void)autolibc_memcpy(&msg.msg_cmd.payload, cmd_buf, cmd_len);
        }
        ...
        else
        {
            ...
            if ((NULL != rep_buf) && (NULL != rep_len) && (4U <= reply_msg.msg_cmd.length))
            {
#if (TRUE == FCI_CFG_FORCE_LEGACY_API)
                (void)autolibc_memcpy(rep_buf, reply_msg.msg_cmd.payload, reply_msg.msg_cmd.length);
                *rep_len = reply_msg.msg_cmd.length;
#else
                (void)autolibc_memcpy(rep_buf, (reply_msg.msg_cmd.payload + 4U), reply_msg.msg_cmd.length - 4U);
                *rep_len = (uint16_t)(reply_msg.msg_cmd.length) - 4U;
