...
#define QSPI_IP_CONTROLLER_SW_PATCH_VERSION_C                2
                                        ...
                                        )
{
    if (QSPI_IP_DATA_RATE_SDR == userConfigPtr->dataRate)
    {
        ...
        /* SDR SPI NOR flash has no DQS pin. Keep DQS disabled and leave
           MCR[DQS_FA_SEL] / SMPR[DLLFSMP] at their reset (0) values - this
           matches the working reference BL (MCR=0x000F404C, SMPR=0). The
           DQS+LOOPBACK+DLL-tap path previously enabled here broke RX sampling
           so every IP serial command (ReadSR ...) timed out with empty RX. */
        ...
        /* SDR: intentionally do NOT call SetDQSSourceA/B nor SetRxDLLTapA/B */
    }
