 ...
 */
#define HIF_CLIENTS_IHC_IDX                     ((uint8_t)PFE_PHY_IF_ID_MAX + 1U)
...
typedef struct
{
    uint32_t txq_num;   /* Number of TX queues */
    uint32_t rxq_num;   /* Number of RX queues */
...
static inline bool_t pfe_hif_pkt_is_last(const pfe_hif_pkt_t *pkt)
{
    ...
    {
        if (0U != ((uint32_t)pkt->flags.common & (uint32_t)HIF_LAST_BUFFER))
        {
            ret = TRUE;
        }
        else
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        if (0U != ((uint32_t)pkt->flags.specific.rx_flags & (uint32_t)HIF_RX_IPV4_CSUM))
        {
            ret = TRUE;
        }
        else
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        if (0U != ((uint32_t)pkt->flags.specific.rx_flags & (uint32_t)HIF_RX_UDPV4_CSUM))
        {
            ret = TRUE;
        }
        else
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        if (0U != ((uint32_t)pkt->flags.specific.rx_flags & (uint32_t)HIF_RX_UDPV6_CSUM))
        {
            ret = TRUE;
        }
        else
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        if (0U != ((uint32_t)pkt->flags.specific.rx_flags & (uint32_t)HIF_RX_TCPV4_CSUM))
        {
            ret = TRUE;
        }
        else
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        if (0U != ((uint32_t)pkt->flags.specific.rx_flags & (uint32_t)HIF_RX_TCPV6_CSUM))
        {
            ret = TRUE;
        }
        else
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        if (0U != ((uint16_t)pkt->flags.specific.rx_flags & (uint16_t)HIF_RX_ICMP_CSUM))
        {
            ret = TRUE;
        }
        else
