...
static errno_t pfe_l2br_table_compile_update_cmd(pfe_l2br_table_t *l2br, pfe_l2br_table_entry_t *entry, uint32_t *cmd);
static errno_t pfe_l2br_table_compile_del_cmd(pfe_l2br_table_t *l2br, pfe_l2br_table_entry_t *entry, uint32_t *cmd);
static errno_t pfe_l2br_table_compile_add_cmd(pfe_l2br_table_t *l2br, pfe_l2br_table_entry_t *entry, uint32_t *cmd);
static errno_t pfe_l2br_table_compile_search_cmd(pfe_l2br_table_t *l2br, pfe_l2br_table_entry_t *entry, uint32_t *cmd);
...
static errno_t pfe_l2br_table_col_entries_init(pfe_l2br_table_t *l2br)
{
    ...
    for (uint32_t ii=0U; ii < l2br->coll_space_depth; ii++)
    {
        if (PFE_L2BR_TABLE_MAC2F == l2br->type)
        {
        ...
        if (EOK == ret)
        {
            ...
            else
            {
                ...
                if (ret == EOK)
                {
                    ...
                    const uint32_t space_depth_total = (uint32_t)l2br->hash_space_depth + l2br->coll_space_depth; 
...
errno_t pfe_l2br_table_entry_set_mac_addr(pfe_l2br_table_entry_t *entry,const pfe_mac_addr_t mac_addr)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        if (PFE_L2BR_TABLE_MAC2F == entry->type)
        {
            ...
            entry->u.mac2f_entry.field_valids |= (uint32_t)MAC2F_ENTRY_MAC_VALID;
            ret = EOK;
        }
...
errno_t pfe_l2br_table_entry_set_vlan(pfe_l2br_table_entry_t *entry, uint16_t vlan)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        if (PFE_L2BR_TABLE_MAC2F == entry->type)
        {
            ...
            entry->u.mac2f_entry.field_valids |= (uint32_t) MAC2F_ENTRY_VLAN_VALID;
            ret = EOK;
        }
        else if (PFE_L2BR_TABLE_VLAN == entry->type)
        {
            entry->u.vlan_entry.vlan = ((uint32_t)vlan & (uint32_t)0x1fffU);
            entry->u.vlan_entry.field_valids |= (uint32_t) VLAN_ENTRY_VLAN_VALID;
            ret = EOK;
        }
        else
        {
...
uint32_t pfe_l2br_table_entry_to_str(const pfe_l2br_table_entry_t *entry, char_t *buf, uint32_t buf_len)
{
    ...
    if (PFE_L2BR_TABLE_MAC2F == entry->type)
    {
        len += (uint32_t)nxp_snprintf(buf + len, buf_len - len, "[MAC+VLAN Table Entry]\n");
        len += (uint32_t)nxp_snprintf(buf + len, buf_len - len, "MAC Address: %02x:%02x:%02x:%02x:%02x:%02x\n",
        ...
        len += (uint32_t)nxp_snprintf(buf + len, buf_len - len, "VLAN       : 0x%x\n", entry->u.mac2f_entry.vlan);
        len += (uint32_t)nxp_snprintf(buf + len, buf_len - len, "Action Data: 0x%x\n", entry->u.mac2f_entry.action_data);
#if 0
        /* Currently not used - action data stores the port information, FW does not have access to port field */
        len += (uint32_t)nxp_snprintf(buf + len, buf_len - len, "Port       : 0x%x\n", entry->u.mac2f_entry.port);
#endif
        len += (uint32_t)nxp_snprintf(buf + len, buf_len - len, "Col Ptr    : 0x%x\n", entry->u.mac2f_entry.col_ptr);
        len += (uint32_t)nxp_snprintf(buf + len, buf_len - len, "Flags      : 0x%x\n", entry->u.mac2f_entry.flags);
    }
    else if (PFE_L2BR_TABLE_VLAN == entry->type)
    {
        len += (uint32_t)nxp_snprintf(buf + len, buf_len - len, "[VLAN Table Entry]\n");
        len += (uint32_t)nxp_snprintf(buf + len, buf_len - len, "VLAN       : 0x%x\n", entry->u.vlan_entry.vlan);
        /*  Native type used to fix compiler warning */
        len += (uint32_t)nxp_snprintf(buf + len, buf_len - len, "Action Data: 0x%"PRINT64"x\n", (uint64_t)entry->u.vlan_entry.action_data);
#if 0
        /* Currently not used - action data stores the port information, FW does not have access to port field */
    ...
    {
        len += (uint32_t)nxp_snprintf(buf + len, buf_len - len, "Invalid entry type\n");
    }
    return len;
}
