...
static errno_t fci_internal_read_fw_feature(const char *feature_name, uint16_t *fci_ret, fpp_fw_features_cmd_t *reply_buf, uint32_t *reply_len);
static errno_t fci_internal_read_fw_feature_element_init(errno_t *element_cmd_ret, fpp_fw_features_element_cmd_t  *fp_cmd, uint16_t *fci_ret,  char **feature_name, uint32_t *reply_len);
static errno_t fci_internal_mng_fw_feat_elmnt_act_update(fpp_fw_features_element_cmd_t *fp_cmd, uint16_t *fci_ret, const char *feature_name);
static errno_t fci_internal_mng_fw_feat_elmnt_act_query(fpp_fw_features_element_cmd_t *fp_cmd,uint16_t *fci_ret, const char *feature_name, fpp_fw_features_element_cmd_t *reply_buf, uint32_t *reply_len);
static errno_t fci_internal_mng_fw_feat_elmnt_act_query_cont(fpp_fw_features_element_cmd_t *fp_cmd,uint16_t *fci_ret, const char *feature_name, fpp_fw_features_element_cmd_t *reply_buf, uint32_t *reply_len);
static errno_t fci_internal_read_fw_feature_table_element(fpp_fw_features_element_cmd_t *fp_cmd, uint16_t *fci_ret, const char *table_el_name, fpp_fw_features_element_cmd_t *reply_buf, uint32_t *reply_len);
#if defined(PFE_CFG_NULL_ARG_CHECK)
static bool_t null_arg_check(fci_msg_t *msg, uint16_t *fci_ret, fpp_fw_features_element_cmd_t *reply_buf, uint32_t *reply_len);
