...
static uint32_t pfe_rtable_entry_get_hash(pfe_rtable_entry_t *entry, pfe_ipv_type_t ipv_type, uint32_t hash_mask)
{
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        if (IPV4 == ipv_type)
        {
            ...
            sport = entry->phys_entry->ipv.v4.sip ^ oal_ntohl(oal_ntohs(entry->phys_entry->sport));
            temp = pfe_get_crc32_be(crc, (uint8_t *)&sport, 4);
            temp += oal_ntohl(entry->phys_entry->ipv.v4.dip);
            temp += entry->phys_entry->proto;
            temp += (uint32_t)oal_ntohs(entry->phys_entry->dport);
            ret = (temp & hash_mask);
        ...
        else if (IPV6 == ipv_type)
        {
            ...
            /*  CRC(SIP) + DIP + CRC(SPORT) + DPORT + PROTO */
            sport = crc_ipv6 ^ oal_ntohl(oal_ntohs(entry->phys_entry->sport));
            temp = pfe_get_crc32_be(crc,(uint8_t *)&sport, 4);
...
#if defined(PFE_CFG_NULL_ARG_CHECK)
    if (unlikely((NULL_ADDR == htable_base_va) || (NULL_ADDR == pool_base_va) || (NULL_PTR == class)))
    {
        NXP_LOG_ERROR("NULL argument received\n");
        rtable = NULL_PTR;
    }
...
errno_t pfe_rtable_get_stats(const pfe_rtable_t *rtable, pfe_ct_conntrack_stats_t *stat, uint8_t conntrack_index)
{
    uint32_t i = 0U;
    errno_t ret = EOK;
    pfe_ct_conntrack_stats_t stats;
    uint16_t offset = 0;
...
#endif /* PFE_CFG_NULL_ARG_CHECK */
    {
        ...
        {
            (void)autolibc_memset(&stats, 0, sizeof(pfe_ct_conntrack_stats_t));
