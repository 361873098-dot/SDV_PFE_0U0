...
[!NOCODE!]
    ...
    [!IF "$GptEcucPartitionRefCount != 0 "!]
        [!// Count instances
        [!LOOP "GptPit/*"!]
            [!IF "node:exists(as:modconf('Gpt')[1]/GptChannelConfigSet/GptChannelConfiguration/*[(node:refs(GptModuleRef)/../../GptPitModule = node:current()/GptPitModule) and (node:value(./GptChannelEcucPartitionRef/*[1]) = $GptEcucPartitionRefName)])"!]
                [!VAR "OutNumInstancesInPartition" = "$OutNumInstancesInPartition + 1"!]
            [!ENDIF!]
        [!ENDLOOP!]
        [!LOOP "GptStm/*"!]
            [!IF "node:exists(as:modconf('Gpt')[1]/GptChannelConfigSet/GptChannelConfiguration/*[(node:refs(GptModuleRef)/../../GptStmModule = node:current()/GptStmModule) and (node:value(./GptChannelEcucPartitionRef/*[1]) = $GptEcucPartitionRefName)])"!]
                [!VAR "OutNumInstancesInPartition" = "$OutNumInstancesInPartition + 1"!]
            [!ENDIF!]
        [!ENDLOOP!]
        [!LOOP "GptFtm/*"!]
            [!IF "node:exists(as:modconf('Gpt')[1]/GptChannelConfigSet/GptChannelConfiguration/*[(node:refs(GptModuleRef)/../../GptFtmModule = node:current()/GptFtmModule) and (node:value(./GptChannelEcucPartitionRef/*[1]) = $GptEcucPartitionRefName)])"!]
                [!VAR "OutNumInstancesInPartition" = "$OutNumInstancesInPartition + 1"!]
            [!ENDIF!]
        [!ENDLOOP!]

        [!IF "node:exists(as:modconf('Gpt')[1]/GptChannelConfigSet/GptChannelConfiguration/*[(GptHwIp = 'RTC') and (node:value(./GptChannelEcucPartitionRef/*[1]) = $GptEcucPartitionRefName)])"!]
            [!VAR "OutNumInstancesInPartition" = "$OutNumInstancesInPartition + 1"!]
        [!ENDIF!]
        ...
        [!LOOP "GptChannelConfiguration/*"!]
            ...
            [!LOOP "./GptChannelEcucPartitionRef/*"!]
                [!IF "node:value(.) = $GptEcucPartitionRefName"!]
                    ...
                    [!ELSEIF "contains($HardwareModule,'FTM')"!]
    ...
    [!VAR "OutNumInstances" = "$OutNumInstancesPit + $OutNumInstancesStm + $OutNumInstancesRtc + $OutNumInstancesFtm"!]


            [!LOOP "GptPit/*"!]
            [!IF "node:exists(as:modconf('Gpt')[1]/GptChannelConfigSet/GptChannelConfiguration/*[(node:refs(GptModuleRef)/../../GptPitModule = node:current()/GptPitModule)])"!]
                [!VAR "OutNumInstancesInPartition" = "$OutNumInstancesInPartition + 1"!]
            [!ENDIF!]
            [!ENDLOOP!]
            [!LOOP "GptStm/*"!]
            [!IF "node:exists(as:modconf('Gpt')[1]/GptChannelConfigSet/GptChannelConfiguration/*[(node:refs(GptModuleRef)/../../GptStmModule = node:current()/GptStmModule)])"!]
                [!VAR "OutNumInstancesInPartition" = "$OutNumInstancesInPartition + 1"!]
            [!ENDIF!]
            [!ENDLOOP!]
            [!LOOP "GptFtm/*"!]
            [!IF "node:exists(as:modconf('Gpt')[1]/GptChannelConfigSet/GptChannelConfiguration/*[(node:refs(GptModuleRef)/../../GptFtmModule = node:current()/GptFtmModule)])"!]
                [!VAR "OutNumInstancesInPartition" = "$OutNumInstancesInPartition + 1"!]
            [!ENDIF!]
            [!ENDLOOP!]

            [!IF "node:exists(as:modconf('Gpt')[1]/GptChannelConfigSet/GptChannelConfiguration/*[(GptHwIp = 'RTC')])"!]
                [!VAR "OutNumInstancesInPartition" = "$OutNumInstancesInPartition + 1"!]
            [!ENDIF!]
