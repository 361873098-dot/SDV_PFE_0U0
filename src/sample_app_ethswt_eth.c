...
Std_ReturnType SampleAppEthInit(void)
{
    ...
    bool_t         configStatus    = (bool_t)FALSE;
#endif /* USE_SJA1105P_DRIVER */

    Eth_43_PFE_Init(NULL_PTR);

#ifdef USE_SJA1105P_DRIVER
    EthSwt_43_SJA1105P_Init(NULL_PTR);

    stdRet |= EthSwt_43_SJA1105P_SwitchInit(EthSwtConfig_0);
    stdRet |= EthSwt_43_SJA1105P_VerifyConfig(EthSwtConfig_0, &configStatus);
    if ((bool_t)FALSE == configStatus)
    {
        stdRet = E_NOT_OK;
    }
#endif /* USE_SJA1105P_DRIVER */
