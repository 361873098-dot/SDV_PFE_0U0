            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            u8QueueIdx = Eth_43_PFE_InternalCfgPtr->
                                            pController[u8CtrlIdx]->
                                            EthCtrlEgressPrioToFifoIdx[Priority];
#if (STD_ON == ETH_43_PFE_SWT_MANAGEMENT_SUPPORT_API)
                            ...
                            if(TRUE == \
                                 Eth_PFE_LLD_ProvideBufferDataArea( \
                                     u8CtrlIdx, u8QueueIdx, BufIdxPtr, BufPtr, &u16TempLenByte \
                                                                  ) \
                              )
...
#endif /* STD_ON == ETH_43_PFE_DEV_ERROR_DETECT */
    {
        eReturnStatus = Eth_43_PFE_LLD_GetL2BridgeDomainStats(stat, index_vlan);
    }
    return eReturnStatus;
}
