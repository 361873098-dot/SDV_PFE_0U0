...
static errno_t pfe_feature_mgr_is_util_available(const char *feature_name, pfe_fw_feature_t **fw_feature_util, pfe_feature_mgr_state_info_t *mgr_state);
...
static errno_t pfe_feature_mgr_is_util_available(const char *feature_name, pfe_fw_feature_t **fw_feature_util, pfe_feature_mgr_state_info_t *mgr_state)
{
    ...
    /* Util is present */
    ret = pfe_util_get_feature(feature_mgr.util, fw_feature_util, feature_name);
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        ret_util = pfe_feature_mgr_is_util_available(feature_name, &fw_feature_util, &mgr_state);
                        if(EOK != ret_util)
                        {
                            /* Feature does not exist i.e. it is not available */
                            mgr_state.util_avail = FALSE;
...
static errno_t pfe_feature_mgr_table_cfg_get_payload(const char *feature_name, const char *table_el_name, uint8_t *payload)
{
...
#endif
    {
        ...
        if (EOK == ret)
        {
            ...
            if (EOK == ret)
            {
                (void)pfe_fw_feature_table_entry_get(fw_feature_table_cfg, payload,
...
static errno_t pfe_feature_mgr_table_stats_get_payload(const char *feature_name, const char *table_el_name, uint8_t *payload)
{
...
#endif
    {
        ...
        if (EOK == ret)
        {
            ...
            if (EOK == ret)
            {
                (void)pfe_fw_feature_table_entry_get(fw_feature_table_stats, payload,
