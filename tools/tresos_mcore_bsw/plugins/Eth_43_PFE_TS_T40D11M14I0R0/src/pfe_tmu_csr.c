...
static uint32_t pfe_tmu_reclaim_mem_wrkarnd(uint32_t drops, uint8_t queue, pfe_ct_phy_if_id_t phy);
static errno_t pfe_tmu_set_default_queue_mode(addr_t cbus_base_va, uint32_t ii, bool_t has_tlite_phy5);
static errno_t pfe_tmu_tdq_shp_on_phy5_g2(addr_t cbus_base_va, uint32_t ii);
...
void pfe_tmu_reclaim_init(addr_t cbus_base_va)
{
    ...
    for (ii = 0U; ii < (uint32_t)TLITE_PHYS_CNT; ii++)
    {
        ...
        {
            hal_write32(((ii & 0x1fUL) << 8U) | ((uint32_t)queue & 0x7UL), ADDR_BASE_OFFSET(cbus_base_va, TMU_PHY_QUEUE_SEL));
            hal_nop();
...
errno_t pfe_tmu_q_reset_tail_drop_policy(addr_t cbus_base_va)
{
    ...
    for (ii = 0U; ii < (uint32_t)TLITE_PHYS_CNT; ii++)
    {
        for (queue = 0; queue < (uint8_t)TLITE_PHY_QUEUES_CNT; queue++)
        {
            switch(ii)
            {
                ...
                case PFE_PHY_IF_ID_HIF:
                    ...
                    max = (uint16_t) TLITE_HIF_MAX_Q_SIZE;
                    break;
                default:
                    /* Other: UTIL, HIF_NOCPY */
                    max = (uint16_t) TLITE_MAX_Q_SIZE;
                    break;
            }
...
static errno_t pfe_tmu_cntx_mem_write(addr_t cbus_base_va, pfe_ct_phy_if_id_t phy, uint8_t loc, uint32_t data)
{
    ...
    if(ret == EOK)
    {
        hal_write32((((uint32_t)phy_temp & (uint32_t)0x1fU) << 16U) | (uint32_t)loc, ADDR_BASE_OFFSET(cbus_base_va, TMU_CNTX_ADDR));
...
static errno_t pfe_tmu_context_memory(addr_t cbus_base_va, pfe_ct_phy_if_id_t phy, uint8_t queue_temp, uint16_t min, uint16_t max)
{
    ...
    */
    ...
    ret = pfe_tmu_cntx_mem_write(cbus_base_va, phy, (uint8_t)(((8U * queue_temp) + 5U) & UINT8_MAX), reg);
    if (EOK == ret)
    {
        ret = pfe_tmu_cntx_mem_write(cbus_base_va, phy, (uint8_t)(((8U * queue_temp) + 6U) & UINT8_MAX), reg);
        if (EOK == ret)
        {
            /*  curQ_Qmax[8:0], curQ_Qmin[8:0], curQ_cfg[1:0] are @ position 4 per queue */
            reg = ((uint32_t)max << 11U) | ((uint32_t)min << 2U) | 0x2UL;
            ret = pfe_tmu_cntx_mem_write(cbus_base_va, phy, (uint8_t)(((8U * queue_temp) + 4U) & UINT8_MAX), reg);
        }
    }
    return ret;
}
...
errno_t pfe_tmu_q_cfg_get_fill_level(addr_t cbus_base_va, pfe_ct_phy_if_id_t phy, uint8_t queue, uint32_t *level)
{
    ...
    if (EOK == ret)
    {
        ...
        PfeDevAssert(queue_temp < 8U);
        ret = pfe_tmu_cntx_mem_read(cbus_base_va, phy, (uint8_t)(((8U * queue_temp) + 1U) & UINT8_MAX), level);
    }
    return ret;
}
...
errno_t pfe_tmu_q_cfg_get_drop_count(addr_t cbus_base_va, pfe_ct_phy_if_id_t phy, uint8_t queue, uint32_t *cnt)
{
    ...
    if (EOK == ret)
    {
        ...
        ret = pfe_tmu_cntx_mem_read(cbus_base_va, phy, (uint8_t)(((8U * temp) + 2U) & UINT8_MAX), &drops);
...
errno_t pfe_tmu_q_cfg_get_tx_count(addr_t cbus_base_va, pfe_ct_phy_if_id_t phy, uint8_t queue, uint32_t *cnt)
{
    ...
    if (EOK == ret)
    {
        ...
        PfeDevAssert(temp < 8U);
        ret = pfe_tmu_cntx_mem_read(cbus_base_va, phy, (uint8_t)(((8U * temp) + 3U) & UINT8_MAX), cnt);
    }
...
pfe_tmu_queue_mode_t pfe_tmu_q_get_mode(addr_t cbus_base_va, pfe_ct_phy_if_id_t phy, uint8_t queue, uint32_t *min, uint32_t *max)
{
    ...
    if (TMU_Q_MODE_DEFAULT == mode)
    {
        PfeDevAssert(temp < 8U);
        /*  curQ_Qmax[8:0], curQ_Qmin[8:0], curQ_cfg[1:0] are @ position 4 per queue */
        ret = pfe_tmu_cntx_mem_read(cbus_base_va, phy, (uint8_t)(((8U * temp) + 4U) & UINT8_MAX), &reg);
        if (EOK != ret)
        {
            mode = TMU_Q_MODE_INVALID;
        }
...
errno_t pfe_tmu_q_mode_set_default(addr_t cbus_base_va, pfe_ct_phy_if_id_t phy, uint8_t queue)
{
    ...
    if (EOK == ret)
    {
        ...
        hal_write32((uint32_t)0x0U | ((uint32_t)0x0U << 1), ADDR_BASE_OFFSET(cbus_base_va, TMU_TEQ_CTRL));

        /*  Put the queue to default mode */
        /*  curQ_Qmax[8:0], curQ_Qmin[8:0], curQ_cfg[1:0] are @ position 4 per queue */
        PfeDevAssert(temp < 8U);
        ret = pfe_tmu_cntx_mem_write(cbus_base_va, phy, (uint8_t)(((8U * temp) + 4U) & UINT8_MAX), 0U);
    }
    ...
    else
    {
        ...
        */
        if (EOK == ret)
        {
            ...
            ret = pfe_tmu_cntx_mem_read(cbus_base_va, phy, (uint8_t)(((8U * queue_temp) + pos) & UINT8_MAX), &reg);
            if (EOK == ret)
            {
                reg &= ~(0x1fUL << (5U * (zone % 6U)));
                reg |= (((0x1fU * (uint32_t)prob) / 100U) & 0x1fU) << (5U * (zone % 6U));

                ret = pfe_tmu_cntx_mem_write(cbus_base_va, phy, (uint8_t)(((8U * queue_temp) + pos) & UINT8_MAX), reg);
            }
        }
    }
        ...
        uint8_t shp, int32_t max_credit, int32_t min_credit)
{
    ...
    addr_t shp_base_va = ADDR_BASE_OFFSET(cbus_base_va, TLITE_PHYn_SHPm_BASE_ADDR((uint32_t)phy, shp));
    errno_t ret;
    ...
    {
        hal_write32((uint32_t)max_credit << 10, ADDR_BASE_OFFSET(shp_base_va, TMU_SHP_MAX_CREDIT));
        ...
        uint8_t shp, int32_t *max_credit, int32_t *min_credit)
{
    ...
    *max_credit = (int32_t)(hal_read32(ADDR_BASE_OFFSET(shp_base_va, TMU_SHP_MAX_CREDIT)) >> 10);
    const uint32_t min_credit_hw = hal_read32(ADDR_BASE_OFFSET(shp_base_va, TMU_SHP_MIN_CREDIT));
    PfeDevAssert(min_credit_hw < (1UL << 22)); /* HW register width is 22 bits */
    *min_credit = -(int32_t)min_credit_hw;
...
uint8_t pfe_tmu_shp_cfg_get_position(addr_t cbus_base_va, pfe_ct_phy_if_id_t phy, uint8_t shp)
{
    ...
    PfeDevAssert(shp < TLITE_SHP_CNT);
    addr_t shp_base_va = ADDR_BASE_OFFSET(cbus_base_va, TLITE_PHYn_SHPm_BASE_ADDR((uint32_t)phy, shp));
    return (uint8_t)((hal_read32(ADDR_BASE_OFFSET(shp_base_va, TMU_SHP_CTRL2)) >> 1U) & 0x1fU);
}
        ...
        pfe_ct_phy_if_id_t phy, uint8_t shp)
{
    ...
    return (uint32_t)(isl & UINT32_MAX);
}
...
void pfe_tmu_shp_cfg_init(addr_t cbus_base_va, pfe_ct_phy_if_id_t phy, uint8_t shp)
{
    ...
    hal_write32(((uint32_t)TLITE_SHP_INVALID_POS << 1), ADDR_BASE_OFFSET(shp_base_va, TMU_SHP_CTRL2));
...
void pfe_tmu_sch_cfg_init(addr_t cbus_base_va, pfe_ct_phy_if_id_t phy, uint8_t sch)
{
    ...
    addr_t sch_base_va = ADDR_BASE_OFFSET(cbus_base_va, TLITE_PHYn_SCHEDm_BASE_ADDR((uint32_t)phy, sch));
        ...
        pfe_ct_phy_if_id_t phy, uint8_t sch)
{
    ...
    uint32_t reg = hal_read32(ADDR_BASE_OFFSET(sch_base_va, TMU_SCH_BIT_RATE));
        ...
        pfe_ct_phy_if_id_t phy, uint8_t sch, uint8_t input, uint32_t weight)
{
    ...
    {
        hal_write32(weight, ADDR_BASE_OFFSET(sch_base_va, (uint32_t)(TMU_SCH_Qn_WGHT(input))));
        ret = EOK;
    }
...
uint32_t pfe_tmu_sch_cfg_get_input_weight(addr_t cbus_base_va, pfe_ct_phy_if_id_t phy, uint8_t sch, uint8_t input)
{
    ...
    {
        input_weight = hal_read32(ADDR_BASE_OFFSET(sch_base_va, (uint32_t)(TMU_SCH_Qn_WGHT(input))));
    }
        ...
        pfe_ct_phy_if_id_t phy, uint8_t sch, uint8_t input, uint8_t queue)
{
    ...
    else
    {
        ...
        reg = hal_read32(ADDR_BASE_OFFSET(sch_base_va, (uint32_t)(TMU_SCH_Q_ALLOCn((uint32_t)input / 4U))));
        reg &= ~(0xffUL << (8U * (input % 4U)));
        reg |= (((uint32_t)queue & 0x1fUL) << (8U * (input % 4U)));
        hal_write32(reg, ADDR_BASE_OFFSET(sch_base_va, (uint32_t)(TMU_SCH_Q_ALLOCn((uint32_t)input / 4U))));
        ret = EOK;
    }
...
uint8_t pfe_tmu_sch_cfg_get_bound_queue(addr_t cbus_base_va, pfe_ct_phy_if_id_t phy, uint8_t sch, uint8_t input)
{
    ...
    {
        reg = hal_read32(ADDR_BASE_OFFSET(sch_base_va, (uint32_t)(TMU_SCH_Q_ALLOCn((uint32_t)input / 4U))));
        queue = (uint8_t)((reg >> (8U * (input % 4U))) & UINT8_MAX);
...
errno_t pfe_tmu_sch_cfg_bind_sched_output(addr_t cbus_base_va, pfe_ct_phy_if_id_t phy, uint8_t src_sch, uint8_t dst_sch, uint8_t input)
{
    ...
    addr_t sch_base_va = ADDR_BASE_OFFSET(cbus_base_va, TLITE_PHYn_SCHEDm_BASE_ADDR((uint32_t)phy, src_sch));
    errno_t ret;
    ...
    else
    {
        ...
        else
        {
            ...
            hal_write32((uint32_t)input & (uint32_t)0xfU, ADDR_BASE_OFFSET(sch_base_va, TMU_SCH_POS));
            ret = EOK;
        }
    }
    return ret;
...
uint8_t pfe_tmu_sch_cfg_get_bound_sched_output(addr_t cbus_base_va, pfe_ct_phy_if_id_t phy, uint8_t sch, uint8_t input)
{
    PfeDevAssert(phy < PFE_PHY_IF_ID_INVALID);
    PfeDevAssert(sch < TLITE_SCH_CNT);
    addr_t sch_base_va = ADDR_BASE_OFFSET(cbus_base_va, TLITE_PHYn_SCHEDm_BASE_ADDR((uint32_t)phy, 0U));
    uint32_t reg;
    uint8_t sched_id;
...
uint32_t pfe_tmu_cfg_get_text_stat(addr_t base_va, char_t *buf, uint32_t size, uint8_t verb_level)
{
    ...
    for (ii=0U; ii < TLITE_PHYS_CNT; ii++)
    {
        ...
        for (queue=0U; queue<TLITE_PHY_QUEUES_CNT; queue++)
        {
            ...
            if (EOK != pfe_tmu_cntx_mem_read(base_va, phy_if_id_temp[ii], (uint8_t)(((8U * queue) + 4U) & UINT8_MAX), &reg))
            {
 ...
 */
uint32_t pfe_tmu_cfg_get_stat_value(addr_t base_va, uint32_t stat_id)
{
    uint32_t stat_value;
 ...
 * @return      EOK if possible to get queue-based stats, otherwise E_NOT_OK
 */
errno_t pfe_tmu_cfg_get_queue_stats(addr_t base_va, uint32_t phy_id, uint32_t queue_id, pfe_tmu_queue_stats* queue_stats)
{
    uint32_t reg;
    uint8_t zone, prob;
    ...
    /*  curQ_cfg is @ position 4 per queue */
    if(EOK == pfe_tmu_cntx_mem_read(base_va, phy_if_id_temp[phy_id], (uint8_t)(((8U * queue_id) + 4U) & UINT8_MAX), &reg))
    {
        /*  Configuration */
        switch (reg & 0x3U)
        {
