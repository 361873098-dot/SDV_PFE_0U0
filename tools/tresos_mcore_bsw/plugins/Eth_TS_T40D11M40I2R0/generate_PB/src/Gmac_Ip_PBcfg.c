...
#define GMAC_IP_PBCFG[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!]_SW_MAJOR_VERSION_C              4
#define GMAC_IP_PBCFG[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!]_SW_MINOR_VERSION_C              0
#define GMAC_IP_PBCFG[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!]_SW_PATCH_VERSION_C              2
    ...
    [!IF "$NumOfEthCtrls > 0"!]
        [!LOOP "node:order(EthConfigSet/EthCtrlConfig/*, 'EthCtrlIdx')"!]
            ...
            [!LOOP "EthCtrlConfigTimeAwareShaper/EthCtrlConfigTimeAwareShaper/*"!]
                ...
                [!VAR "EnablePreemtion" = "node:value(EthCtrlEnablePreemtion)"!]
...
static const Gmac_Ip_TxTimeAwareShaper GMAC_[!"num:i($CtrlIdx)"!]_pTxTimeShaperPB[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!] =
{
    ...
    /*.releaseAdvanceTime = */[!IF "$NumOfTimeShaper > 0"!][!IF "$EnablePreemtion = 'true'"!][!"$EthReleaseAdvanceTime"!]U[!ELSE!]0U[!ENDIF!][!ELSE!]0U[!ENDIF!],
    /*.holdAdvanceTime = */[!IF "$NumOfTimeShaper > 0"!][!IF "$EnablePreemtion = 'true'"!][!"$EthHoldAdvanceTime"!]U[!ELSE!]0U[!ENDIF!][!ELSE!]0U[!ENDIF!],
    /*.preemptionClassification = */[!IF "$NumOfTimeShaper > 0"!][!IF "$EnablePreemtion = 'true'"!][!"$PreempClassReg"!][!ELSE!]0U[!ENDIF!][!ELSE!]0U[!ENDIF!],
